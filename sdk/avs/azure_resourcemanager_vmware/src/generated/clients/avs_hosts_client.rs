...
use crate::generated::models::{
    AVSHostsClientGetOptions, AVSHostsClientListOptions, Host, HostListResult,
};
use azure_core::{
...
impl AVSHostsClient {
    ...
    #[tracing::function("Microsoft.AVS.Hosts.get")]
    pub async fn get(
        &self,
        ...
        cluster_name: &str,
        host_id: &str,
        options: Option<AVSHostsClientGetOptions<'_>>,
        ...
        if host_id.is_empty() {
            return Err(azure_core::Error::with_message(
                azure_core::error::ErrorKind::Other,
                "parameter host_id cannot be empty",
            ));
        }
        ...
        let mut path = String::from("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.AVS/privateClouds/{privateCloudName}/clusters/{clusterName}/hosts/{hostId}");
        path = path.replace("{clusterName}", cluster_name);
        path = path.replace("{hostId}", host_id);
    ...
    /// * `cluster_name` - Name of the cluster
    /// * `options` - Optional parameters for the request.
    #[tracing::function("Microsoft.AVS.Hosts.list")]
    pub fn list(
        &self,
        resource_group_name: &str,
        private_cloud_name: &str,
        cluster_name: &str,
        options: Option<AVSHostsClientListOptions<'_>>,
    ) -> Result<Pager<HostListResult>> {
        if cluster_name.is_empty() {
            return Err(azure_core::Error::with_message(
                azure_core::error::ErrorKind::Other,
                "parameter cluster_name cannot be empty",
        ...
        let mut path = String::from("/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.AVS/privateClouds/{privateCloudName}/clusters/{clusterName}/hosts");
        path = path.replace("{clusterName}", cluster_name);
        path = path.replace("{privateCloudName}", private_cloud_name);
        path = path.replace("{resourceGroupName}", resource_group_name);
        path = path.replace("{subscriptionId}", &self.subscription_id);
            ...
            move |next_link: PagerState, pager_options| {
                ...
                Box::pin({
                    ...
                    async move {
                        ...
                        let res: HostListResult = json::from_json(&body)?;
