...
use crate::http::{headers::Headers, DefaultFormat, Format, StatusCode};
use bytes::Bytes;
...
pub struct Response<T = ResponseBody, F: Format = DefaultFormat> {
    status: StatusCode,
    headers: Headers,
    body: ResponseBody,
...
impl<T: FormattedBy<F>, F: Format> Response<T, F> {
