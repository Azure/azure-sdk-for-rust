// Copyright (c) Microsoft Corporation. All rights reserved.
//
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) Rust Code Generator. DO NOT EDIT.

use crate::generated::models::{
    AzureQueueStorageQueueOperationGroupClientCreateOptions,
    AzureQueueStorageQueueOperationGroupClientDeleteOptions,
    AzureQueueStorageQueueOperationGroupClientGetAccessPolicyOptions,
    AzureQueueStorageQueueOperationGroupClientGetPropertiesOptions,
    AzureQueueStorageQueueOperationGroupClientSetAccessPolicyOptions,
    AzureQueueStorageQueueOperationGroupClientSetMetadataOptions, ListOfSignedIdentifier,
};
use azure_core::{
    http::{Context, Method, Pipeline, Request, Response, Url, XmlFormat},
    Result,
};

pub struct AzureQueueStorageQueueOperationGroupClient {
    pub(crate) api_version: String,
    pub(crate) endpoint: Url,
    pub(crate) pipeline: Pipeline,
}

impl AzureQueueStorageQueueOperationGroupClient {
    /// Returns the Url associated with this client.
    pub fn endpoint(&self) -> &Url {
        &self.endpoint
    }

    /// creates a new queue under the given account.
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn create(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientCreateOptions<'_>>,
    ) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Put);
        request.insert_header("accept", "application/json");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        if let Some(metadata) = options.metadata {
            for (k, v) in &metadata {
                request.insert_header(format!("x-ms-meta-{}", k), v);
            }
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }

    /// operation permanently deletes the specified queue
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn delete(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientDeleteOptions<'_>>,
    ) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Delete);
        request.insert_header("accept", "application/json");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }

    /// returns details about any stored access policies specified on the queue that
    /// may be used with Shared Access Signatures.
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn get_access_policy(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientGetAccessPolicyOptions<'_>>,
    ) -> Result<Response<ListOfSignedIdentifier, XmlFormat>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut().append_pair("comp", "acl");
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Get);
        request.insert_header("accept", "application/xml");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }

    /// Retrieves user-defined metadata and queue properties on the specified queue.
    /// Metadata is associated with the queue as name-values pairs.
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn get_properties(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientGetPropertiesOptions<'_>>,
    ) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut().append_pair("comp", "metadata");
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Get);
        request.insert_header("accept", "application/json");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }

    /// sets stored access policies for the queue that may be used with Shared Access
    /// Signatures
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn set_access_policy(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientSetAccessPolicyOptions<'_>>,
    ) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut().append_pair("comp", "acl");
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Put);
        request.insert_header("accept", "application/json");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        if let Some(list_of_signed_identifier) = options.list_of_signed_identifier {
            request.set_body(list_of_signed_identifier);
        }
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }

    /// sets user-defined metadata on the specified queue. Metadata is associated with
    /// the queue as name-value pairs.
    ///
    /// # Arguments
    ///
    /// * `queue_name` - The queue name.
    /// * `options` - Optional parameters for the request.
    pub async fn set_metadata(
        &self,
        queue_name: &str,
        options: Option<AzureQueueStorageQueueOperationGroupClientSetMetadataOptions<'_>>,
    ) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join(queue_name)?;
        url.query_pairs_mut().append_pair("comp", "metadata");
        url.query_pairs_mut()
            .append_pair("api-version", &self.api_version);
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut()
                .append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Put);
        request.insert_header("accept", "application/json");
        if let Some(request_id) = options.request_id {
            request.insert_header("request-id", request_id);
        }
        if let Some(metadata) = options.metadata {
            for (k, v) in &metadata {
                request.insert_header(format!("x-ms-meta-{}", k), v);
            }
        }
        request.insert_header("x-ms-version", self.api_version.to_string());
        self.pipeline.send(&ctx, &mut request).await.map(Into::into)
    }
}
