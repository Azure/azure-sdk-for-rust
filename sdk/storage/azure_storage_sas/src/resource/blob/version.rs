...
pub struct BlobVersionResource {
    pub container: String,
    pub blob: String,
    pub version_id: OffsetDateTime,
    pub options: Option<BlobResourceOptions>,
}

impl Resource for BlobVersionResource {
    type Permissions = BlobSasPermissions;
}

impl sealed::BlobService for BlobVersionResource {}

impl sealed::Resource for BlobVersionResource {
    ...
    fn sas_url(
        ...
        q.append_pair("sv", params.version)
            .append_pair("sr", "bv")
            .append_pair("svid", &version_id_str);
...
mod tests {
    ...
    fn make_resource() -> BlobVersionResource {
        BlobVersionResource {
            container: CONTAINER.to_string(),
            blob: BLOB.to_string(),
            version_id: datetime!(2024-03-10 08:30:00 UTC),
            options: None,
        }
    }

    #[test]
    fn test_blob_version_string_to_sign_has_24_fields() {
        let key = make_key();
        ...
        assert_eq!(
            parts.len(),
            28,
            "blob version string-to-sign must have 28 fields"
        );
        assert_eq!(parts[18], "bv", "[18] signedResource");
        assert_eq!(
            parts[19], "",
            "[19] signedSnapshotTime — empty for versions"
        );
    }

    #[test]
    fn test_blob_version_sas_url_has_sr_bv_and_svid() {
        let key = make_key();
        let resource = make_resource();
        let canon = resource.canonicalized_resource(ACCOUNT);
        let ctx = SasSigningContext {
        ...
        assert_eq!(params["sr"], "bv", "version SAS must include sr=bv");
        assert_eq!(
            params["svid"], "2024-03-10T08:30:00.0000000Z",
            "version SAS must include svid"
        );
        assert!(
            !params.contains_key("sst"),
            "version SAS must not include sst"
        );
        assert_eq!(
            params.len(),
            14,
            "blob version SAS must have 14 parameters (sr + svid)"
        );
    }
}
