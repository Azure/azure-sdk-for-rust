...
///
/// Derefs to `str` for direct access to string methods. Implements
...
///
/// Derefs to [`url::Url`] for direct access to URL methods. Implements
...
impl<S: SasResource> SasUrlBuilder<'_, S> {
    ...
    pub fn build(&self) -> azure_core::Result<SasUrl> {
        ...
        let endpoint = self
            .endpoint
            .clone()
            .unwrap_or_else(|| S::default_endpoint(&self.inner.common.account));
