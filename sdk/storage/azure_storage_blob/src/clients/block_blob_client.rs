...
impl BlockBlobClient {
    ...
    /// Creates a new BlockBlobClient with session token authentication configured.
    ///
    /// This additive constructor exists because [`SessionOptions`](crate::SessionOptions)
    /// cannot yet be carried on the generated [`BlockBlobClientOptions`]. Use
    /// [`BlockBlobClient::new`] when session authentication is not needed.
    ///
    /// # Arguments
    ///
    /// * `blob_url` - The full URL of the block blob, for example `https://myaccount.blob.core.windows.net/mycontainer/myblob`.
