...
impl PageBlobClient {
    ...
    /// Creates a new PageBlobClient with session token authentication configured.
    ///
    /// This additive constructor exists because [`SessionOptions`](crate::SessionOptions)
    /// cannot yet be carried on the generated [`PageBlobClientOptions`]. Use
    /// [`PageBlobClient::new`] when session authentication is not needed.
    ///
    /// # Arguments
    ///
    /// * `blob_url` - The full URL of the page blob, for example `https://myaccount.blob.core.windows.net/mycontainer/myblob`.
