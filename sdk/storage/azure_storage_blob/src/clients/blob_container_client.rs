...
<EnumerationResults ServiceEndpoint="https://example.blob.core.windows.net/" ContainerName="container">
  ...
  <NextMarker>page-2</NextMarker>
    ...
    const LIST_BLOBS_FINAL_PAGE: &[u8] = br#"<?xml version="1.0" encoding="utf-8"?>
<EnumerationResults ServiceEndpoint="https://example.blob.core.windows.net/" ContainerName="container">
    <Blobs>
        <Blob>
            <Name>blob1</Name>
    ...
    #[tokio::test]
    async fn list_blobs_defaults_to_arrow_with_xml_fallback() -> Result<()> {
        let mock_client = Arc::new(MockHttpClient::new(|req| {
            assert_eq!(
                req.headers()
                    .get_optional_str(&azure_core::http::headers::ACCEPT),
            ...
            async move {
                Ok(AsyncRawResponse::from_bytes(
                    StatusCode::Ok,
                    Headers::new(),
                    Bytes::from_static(LIST_BLOBS_FINAL_PAGE),
                ))
            }
            .boxed()
        }));
        ...
        let blobs: Vec<_> = client.list_blobs(None)?.try_collect().await?;
