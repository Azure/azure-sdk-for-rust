...
impl Serialize for BlobMetadata {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
        ...
        let mut map = serializer.serialize_map(Some(1 + match &self.additional_properties {
            Some(additional_properties) => {
                additional_properties.len()
            },
            None => {
                0
            },
...
pub mod option_offset_date_time_rfc3339_fixed_width {
    ...
    use azure_core::time::{OffsetDateTime, parse_rfc3339, };
    ...
    pub fn serialize<S>(to_serialize: &Option<OffsetDateTime>, serializer: S) -> Result<S::Ok, S::Error>
    where S: Serializer
    {
        let format = Iso8601::<{iso8601::Config::DEFAULT.set_time_precision(iso8601::TimePrecision::Second {decimal_digits: NonZero::new(7)}).encode()}>;
        if let Some(to_serialize) = to_serialize {
            let encoded0 = to_serialize.format(&format).map_err(serde::ser::Error::custom)?;
...
pub mod option_vec_encoded_bytes_std {
    ...
    pub fn serialize<S>(to_serialize: &Option<Vec<Vec<u8>>>, serializer: S) -> Result<S::Ok, S::Error>
    where S: Serializer
    {
        if let Some(to_serialize) = to_serialize {
