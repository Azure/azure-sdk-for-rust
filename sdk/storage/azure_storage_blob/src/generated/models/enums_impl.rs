...
use super::{
    AccessTier, AccountKind, ArchiveStatus, AuthenticationType, BlobCopySourceTags, BlobDeleteType,
    BlobType, BlockListType, CopyStatus, DeleteSnapshotsOptionType, DownloadHint,
    ...
    ListBlobsHierarchicalInternalResponseContentType, ListBlobsIncludeItem,
    ListBlobsInternalResponseContentType, ListContainersIncludeType, PremiumPageBlobAccessTier,
...
impl<'a> From<&'a AuthenticationType> for &'a str {
    fn from(e: &'a AuthenticationType) -> Self {
        match e {
            AuthenticationType::Hmac => "HMAC",
            AuthenticationType::UnknownValue(s) => s.as_ref(),
        }
    }
}
...
impl FromStr for AuthenticationType {
    ...
    fn from_str(s: &str) -> ::core::result::Result<Self, <Self as FromStr>::Err> {
        Ok(match s {
            "HMAC" => AuthenticationType::Hmac,
            _ => AuthenticationType::UnknownValue(s.to_string()),
        })
    }
}
...
impl AsRef<str> for AuthenticationType {
    fn as_ref(&self) -> &str {
        match self {
            AuthenticationType::Hmac => "HMAC",
            AuthenticationType::UnknownValue(s) => s.as_str(),
        }
    }
}
...
impl Display for AuthenticationType {
    fn fmt(&self, f: &mut Formatter<'_>) -> ::std::fmt::Result {
        match self {
            AuthenticationType::Hmac => f.write_str("HMAC"),
            AuthenticationType::UnknownValue(s) => f.write_str(s.as_str()),
        }
    }
}
