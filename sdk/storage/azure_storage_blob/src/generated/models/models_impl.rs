...
use super::{
    BlobItem, BlobServiceProperties, BlobTags, BlockLookupList, ContainerItem,
    CreateSessionConfiguration, FilterBlobItem, FilteredBlobResponse, KeyInfo,
    ListBlobsHierarchicalResponse, ListBlobsResponse, ListContainersResponse, SignedIdentifiers,
};
...
impl TryFrom<CreateSessionConfiguration> for RequestContent<CreateSessionConfiguration, XmlFormat> {
    type Error = azure_core::Error;
    fn try_from(value: CreateSessionConfiguration) -> Result<Self> {
        Ok(to_xml(&value)?.into())
    }
}
