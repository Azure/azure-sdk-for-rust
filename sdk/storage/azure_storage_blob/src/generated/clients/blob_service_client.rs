...
impl BlobServiceClient {
    ...
    pub async fn get_account_info(&self, options: Option<BlobServiceClientGetAccountInfoOptions<'_>>) -> Result<Response<()>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join("")?;
...
/// when read-access geo-redundant replication is enabled for the storage account.
    ...
    pub async fn get_user_delegation_key(&self, start: String, expiry: String, options: Option<BlobServiceClientGetUserDelegationKeyOptions<'_>>) -> Result<Response<UserDelegationKey>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join("")?;
...
/// Resource Sharing) rules
    ...
    pub async fn submit_batch(&self, content_length: i64, body: RequestContent<Bytes>, options: Option<BlobServiceClientSubmitBatchOptions<'_>>) -> Result<Response> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        url = url.join("")?;
