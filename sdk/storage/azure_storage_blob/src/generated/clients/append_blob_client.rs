...
use azure_core::{Bytes, Result, base64::encode, credentials::TokenCredential, error::CheckSuccessOptions, fmt::SafeDebug, http::{ClientOptions, Method, NoFormat, Pipeline, PipelineSendOptions, Request, RequestContent, Response, Url, UrlExt, policies::{Policy, auth::BearerTokenAuthorizationPolicy, }, }, time::to_rfc7231, tracing, };
use crate::generated::models::{AppendBlobClientAppendBlockFromUrlOptions, AppendBlobClientAppendBlockFromUrlResult, AppendBlobClientAppendBlockOptions, AppendBlobClientAppendBlockResult, AppendBlobClientCreateOptions, AppendBlobClientCreateResult, AppendBlobClientSealOptions, AppendBlobClientSealResult, };
use std::sync::Arc;
...
/// * `options` - Optional configuration for the client.
    ...
    pub fn new(endpoint: &str, credential: Arc<dyn TokenCredential>, options: Option<AppendBlobClientOptions>) -> Result<Self> {
...
/// * `options` - Optional parameters for the request.
    ...
    pub async fn append_block_from_url(&self, source_url: String, content_length: u64, options: Option<AppendBlobClientAppendBlockFromUrlOptions<'_>>) -> Result<Response<AppendBlobClientAppendBlockFromUrlResult, NoFormat>> {
        ...
        let mut url = self.endpoint.clone();
        let mut query_builder = url.query_builder();
        query_builder.append_pair("comp", "appendblock").append_key_only("fromUrl");
        if let Some(timeout) = options.timeout {
            query_builder.set_pair("timeout", timeout.to_string());
        }
        query_builder.build();
