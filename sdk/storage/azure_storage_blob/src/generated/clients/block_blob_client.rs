...
/// * `options` - Optional configuration for the client.
    pub fn new(endpoint: &str, credential: Arc<dyn TokenCredential>, container_name: String, blob_name: String, options: Option<BlockBlobClientOptions>) -> Result<Self> {
...
/// * `options` - Optional parameters for the request.
    pub async fn get_block_list(&self, list_type: BlockListType, options: Option<BlockBlobClientGetBlockListOptions<'_>>) -> Result<Response<BlockList>> {
        ...
        url = url.join(&path)?;
        url.query_pairs_mut().append_pair("comp", "blocklist");
        url.query_pairs_mut().append_pair("blocklisttype", list_type.as_ref());
...
/// * `options` - Optional parameters for the request.
    pub async fn put_blob_from_url(&self, content_length: u64, copy_source: &str, options: Option<BlockBlobClientPutBlobFromUrlOptions<'_>>) -> Result<Response<BlockBlobClientPutBlobFromUrlResult>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        let mut path = String::from("{containerName}/{blobName}");
...
/// * `options` - Optional parameters for the request.
    pub async fn query(&self, query_request: RequestContent<QueryRequest>, options: Option<BlockBlobClientQueryOptions<'_>>) -> Result<Response<BlockBlobClientQueryResult>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        let mut path = String::from("{containerName}/{blobName}");
...
/// * `options` - Optional parameters for the request.
    pub async fn stage_block(&self, block_id: Vec<u8>, content_length: u64, body: RequestContent<Bytes>, options: Option<BlockBlobClientStageBlockOptions<'_>>) -> Result<Response<BlockBlobClientStageBlockResult>> {
        ...
        url.query_pairs_mut().append_pair("blockid", &base64::encode(block_id));
        if let Some(timeout) = options.timeout {
            url.query_pairs_mut().append_pair("timeout", &timeout.to_string());
        }
        let mut request = Request::new(url, Method::Put);
...
/// * `options` - Optional parameters for the request.
    pub async fn stage_block_from_url(&self, block_id: Vec<u8>, content_length: u64, source_url: &str, options: Option<BlockBlobClientStageBlockFromUrlOptions<'_>>) -> Result<Response<BlockBlobClientStageBlockFromUrlResult>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        let mut path = String::from("{containerName}/{blobName}");
...
/// * `content_length` - The length of the request.
/// * `options` - Optional parameters for the request.
    pub async fn upload(&self, body: RequestContent<Bytes>, content_length: u64, options: Option<BlockBlobClientUploadOptions<'_>>) -> Result<Response<BlockBlobClientUploadResult>> {
        let options = options.unwrap_or_default();
        let ctx = Context::with_context(&options.method_options.context);
        let mut url = self.endpoint.clone();
        let mut path = String::from("{containerName}/{blobName}");
