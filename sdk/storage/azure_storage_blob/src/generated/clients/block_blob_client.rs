...
/// * `options` - Optional configuration for the client.
    ...
    pub fn new(endpoint: &str, credential: Arc<dyn TokenCredential>, options: Option<BlockBlobClientOptions>) -> Result<Self> {
...
/// * `options` - Optional parameters for the request.
    ...
    pub async fn query(&self, query_request: RequestContent<QueryRequest, XmlFormat>, options: Option<BlockBlobClientQueryOptions<'_>>) -> Result<AsyncResponse<BlockBlobClientQueryResult>> {
        let options = options.unwrap_or_default();
        let ctx = options.method_options.context.to_borrowed();
        let mut url = self.endpoint.clone();
        let mut query_builder = url.query_builder();
        ...
        let rsp = self.pipeline.stream(&ctx, &mut request, Some(PipelineStreamOptions {
            check_success: CheckSuccessOptions{ success_codes: &[200, 206]},
            ..Default::default()
        })).await?;
        Ok(rsp.into())
...
/// * `options` - Optional parameters for the request.
    ...
    pub async fn stage_block_from_url(&self, block_id: &[u8], content_length: u64, source_url: String, options: Option<BlockBlobClientStageBlockFromUrlOptions<'_>>) -> Result<Response<BlockBlobClientStageBlockFromUrlResult, NoFormat>> {
        ...
        let mut url = self.endpoint.clone();
        let mut query_builder = url.query_builder();
        query_builder.append_pair("comp", "block").append_key_only("fromURL");
        query_builder.set_pair("blockid", encode(block_id));
        if let Some(timeout) = options.timeout {
            query_builder.set_pair("timeout", timeout.to_string());
        }
...
/// * `options` - Optional parameters for the request.
    ...
    pub async fn upload(&self, body: RequestContent<Bytes, NoFormat>, content_length: u64, options: Option<BlockBlobClientUploadOptions<'_>>) -> Result<Response<BlockBlobClientUploadResult, NoFormat>> {
        let options = options.unwrap_or_default();
        let ctx = options.method_options.context.to_borrowed();
        let mut url = self.endpoint.clone();
        let mut query_builder = url.query_builder();
...
/// * `options` - Optional parameters for the request.
    ...
    pub async fn upload_blob_from_url(&self, copy_source: String, options: Option<BlockBlobClientUploadBlobFromUrlOptions<'_>>) -> Result<Response<BlockBlobClientUploadBlobFromUrlResult, NoFormat>> {
        ...
        let mut url = self.endpoint.clone();
        let mut query_builder = url.query_builder();
        query_builder.append_key_only("BlockBlob").append_key_only("fromUrl");
        if let Some(timeout) = options.timeout {
            query_builder.set_pair("timeout", timeout.to_string());
        }
        query_builder.build();
