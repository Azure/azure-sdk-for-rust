...
- Changed `BlobContainerClient::list_blobs()` to return `Result<ItemIterator<ListBlobsPageResponse>>` instead of `Result<Pager<ListBlobsResponse, XmlFormat>>`. Each page is now a `ListBlobsPageResponse` that selects XML or Apache Arrow based on the response headers.
