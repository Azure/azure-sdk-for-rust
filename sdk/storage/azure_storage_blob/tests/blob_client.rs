...
use azure_storage_blob::{
    models::{
        ...
        ImmutabilityPolicyMode, LeaseState, ListBlobsIncludeItem,
    },
...
#[recorded::test]
async fn test_list_blob_versions(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    // Recording Setup
    let recording = ctx.recording();
    let container_client = get_container_client(recording, true, StorageAccount::Versioned).await?;
...
async fn test_blob_version_error_cases(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    if result.is_err() {
        ...
        // Should be NotFound (404) or Bad Request (400)
