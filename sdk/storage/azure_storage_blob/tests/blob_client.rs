...
use azure_storage_blob::{
    models::{
        AccessConditions, AccessTier, AccountKind, BlobClientAcquireLeaseResultHeaders,
...
#[recorded::test]
async fn test_upload_blob_access_conditions(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    // Recording Setup
    let recording = ctx.recording();
    let container_client = get_container_client(recording, true).await?;
    let blob_client = container_client.blob_client(&get_blob_name(recording));
    ...
    blob_client
        .upload(
            RequestContent::from(data.to_vec()),
            u64::try_from(data.len())?,
            AccessConditions::if_not_exists(),
            None,
        )
        .await?;
    ...
    // Error Case (Should fail because we don't want to overwrite)
    let response = blob_client
        .upload(
            RequestContent::from(new_data.to_vec()),
            u64::try_from(new_data.len())?,
    ...
    // Working Case (Overwrite True, matches Service Default)
    let overwrite_response = blob_client
        .upload(
            RequestContent::from(new_data.to_vec()),
            u64::try_from(new_data.len())?,
            AccessConditions::allow_overwrite(),
            None,
        )
        .await?;
    let response = blob_client.download(None).await?;
