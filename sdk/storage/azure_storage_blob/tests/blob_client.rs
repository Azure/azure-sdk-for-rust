...
async fn test_encoding_edge_cases(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut listed_blob_names: Vec<String> = Vec::new();
    // Check name equality for all test cases
    let mut list_blobs_response = container_client.list_blobs(None)?;
    while let Some(blob) = list_blobs_response.try_next().await.unwrap() {
        listed_blob_names.push(blob.name.unwrap().content.unwrap().to_string());
    }

    // Ensure we have the expected number of blobs
    assert_eq!(test_cases.len(), listed_blob_names.len());
