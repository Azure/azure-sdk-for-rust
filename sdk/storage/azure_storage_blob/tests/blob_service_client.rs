...
async fn test_list_containers(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    // Assert
    let mut list_containers_response = service_client.list_containers(None)?;
    while let Some(container_result) = list_containers_response.next().await {
        let container = container_result?;
...
async fn test_list_containers_with_continuation(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    // Assert
    let list_containers_response = service_client.list_containers(Some(list_containers_options))?;
    let mut pages = list_containers_response.into_pages();
