...
// <AuthenticationErrorDetail>Authentication scheme Bearer is not allowed with HTTP.</AuthenticationErrorDetail>\n</Error>") }

// Aside from that, I tried launching Azurite w/o OAuth (which apparently allows anonymous access) --> didn't work
// Also launched Azurite w/ --loose flag --> didn't work
// So seems like the only workable methods right now are:

// 1.) Add in a custom SharedKey policy to use the well-defined Storage sharedkey Azurite information, gate it for Azurite only
// 2.) Use HTTPS, which means you have to mkcert even just for local testing and then just use regular OAuth
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Create OAuth credential using Azure CLI or Azure Developer CLI
