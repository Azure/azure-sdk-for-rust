...
pub struct ReadEventOptions {
    ...
    ///
    /// If `None`, the reader will wait forever for items to be available unless reading is canceled. Empty items will
    /// not be returned.
    ...
    /// from a local cache rather than waiting on a service request.
    ///
    /// When set to `None`, the option is considered disabled; otherwise, it will be considered enabled and take
    /// precedence over any value specified for the `prefetch_count` The `prefetch_size_in_bytes` is an
    /// advanced control that developers can use to help tune performance in some scenarios; it is recommended to prefer using
    /// the `prefetch_count` over this option where possible for more accurate control and more predictable throughput.
    ///
    /// This size should be considered a statement of intent rather than a guaranteed limit; the local cache may be larger or
    /// smaller than the number of bytes specified, and will always contain at least one event when the `prefetch_size_in_bytes`
    ...
    /// as traffic passes through the system.  Consequently, the resulting resource use will fluctuate as well.
    ...
    /// for the same partition, the one having a larger `owner_level` value will "win".
    ///
