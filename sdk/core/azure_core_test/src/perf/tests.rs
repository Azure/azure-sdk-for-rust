...
fn test_perf_runner_new_with_empty_tests() {
    let tests = vec![];
    let result = PerfRunner::with_command_line(tests, vec!["perf-tests"]);
    ...
    assert_eq!(runner.options.duration, 30);
    assert_eq!(runner.options.warmup, 5);
    assert_eq!(runner.options.test_results_filename, "./tests/results.json");
    assert!(!runner.options.no_cleanup);
    assert!(runner.options.test.is_none());
}
...
fn test_perf_runner_with_command_line_default_args() {
    ...
    let result = PerfRunner::with_command_line(tests, args);
    assert!(
        result.is_ok(),
...
fn test_perf_runner_with_command_line_custom_duration() {
    ...
    let runner = result.unwrap();
    assert_eq!(runner.options.duration, 60);
}
...
fn test_perf_runner_with_command_line_custom_warmup() {
    ...
    let runner = result.unwrap();
    assert_eq!(runner.options.warmup, 10);
}
...
#[test]
fn test_perf_runner_with_command_line_test_name() {
    let tests = vec![create_basic_test_metadata()];
    let args = vec!["perf-tests", "--test", "my_test"];

    let result = PerfRunner::with_command_line(tests, args);
    assert!(
        result.is_ok(),
        "PerfRunner::with_command_line should succeed with test name"
    );

    let runner = result.unwrap();
    assert_eq!(runner.options.test.as_ref().unwrap(), "my_test");
}
...
fn test_perf_runner_with_command_line_all_options() {
    ...
    assert_eq!(runner.options.duration, 120);
    assert_eq!(runner.options.warmup, 15);
    assert_eq!(runner.options.test_results_filename, "/custom/results.json");
    assert_eq!(runner.options.test.as_ref().unwrap(), "specific_test");
    assert!(runner.options.no_cleanup);
}
...
fn test_perf_runner_with_subcommand() {
    ...
    let runner = result.unwrap();
    let option_value: Option<&String> = runner
        .try_get_one_subcommand("basic_test", "test-option")
        .ok()
        .flatten();
...
fn test_perf_runner_with_complex_subcommand() {
    ...
    println!(
        "Help: {}",
        PerfRunner::with_command_line(tests.clone(), vec!["perf-tests", "--help"])
            .unwrap_err()
            .source()
            .unwrap()
    );
    println!(
        "Help2 : {}",
        PerfRunner::with_command_line(tests.clone(), vec!["perf-tests", "complex_test", "--help"])
            .unwrap_err()
            .source()
            .unwrap()
    );
    ...
    let mandatory_value: Result<Option<&String>, _> =
        runner.try_get_one_subcommand("complex_test", "mandatory-option");
    ...
    let sensitive_value: Option<&String> = runner
        .try_get_one_subcommand("complex_test", "sensitive-option")
        .ok()
        .flatten();
    assert!(sensitive_value.is_some());
    assert_eq!(sensitive_value.unwrap(), "secret_value");

    let flag_value = runner
        .try_get_one_subcommand("complex_test", "flag-option")
        .ok()
        .flatten();
...
fn test_perf_runner_with_no_short_activator() {
    ...
    let runner = result.unwrap();
    let option_value: Option<&String> = runner
        .try_get_one_subcommand("no_short_test", "long-only")
        .ok()
        .flatten();
    assert!(option_value.is_some());
    assert_eq!(option_value.unwrap(), "value");
...
fn test_perf_runner_get_one_nonexistent() {
    ...
    let runner = PerfRunner::with_command_line(tests, args).unwrap();
    let result: Result<Option<&String>, _> = runner.try_get_one("nonexistent");
...
fn test_perf_runner_get_one_different_types() {
    ...
    let iterations: Option<&u32> = runner.try_get_one("iterations").ok().flatten();
    assert!(iterations.is_some());
    assert_eq!(*iterations.unwrap(), 42);

    // Test getting wrong type returns None
    let iterations_as_string: Option<&String> = runner.try_get_one("iterations").ok().flatten();
...
fn test_perf_runner_with_multiple_tests_and_subcommands() {
    ...
    let result = PerfRunner::with_command_line(tests.clone(), args);
    assert!(result.is_ok());
    ...
    let runner = result.unwrap();
    let mandatory_value: Option<&String> = runner
        .try_get_one_subcommand("complex_test", "mandatory-option")
        .ok()
        .flatten();
