...
#[derive(Clone)]
pub struct PagerOptions<'a, C: AsRef<str>> {
    ...
    C: AsRef<str> + Clone + ConditionalSend + 'static,
{
    ...
    fn poll_next(
        ...
        loop {
            ...
            *this.continuation_token = iter.continuation_token.clone();
    ...
    C: AsRef<str> + Clone + ConditionalSend + 'static,
{
    ...
    fn reset(&mut self) {
        self.state = match self.continuation_token {
    ...
    fn poll_next(
        ...
        if *this.state == State::Init {
            ...
            if let Some(s) = span {
                *this.added_span = true;
                let previous_options = std::mem::take(this.options);
                *this.options = PagerOptions {
                    context: previous_options.context.with_value(s),
                    continuation_token: previous_options.continuation_token,
                };
            }
        }
        ...
        match result {
            ...
            Ok(PagerResult::More {
                ...
                *this.continuation_token = Some(continuation_token.clone());
