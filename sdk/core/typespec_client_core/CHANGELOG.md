...
- Added `backtrace` field to `Error` that captures a `std::backtrace::Backtrace` at construction time when `RUST_BACKTRACE=1` is set. The backtrace is available when calling `Debug::fmt` e.g., `{:?}`.
