...
use azure_messaging_servicebus::{
    CompleteMessageOptions, CreateReceiverOptions, CreateSenderOptions, DeferMessageOptions,
    Message, ReceiveDeferredMessagesOptions, ReceiveMessageOptions, ReceiveMode,
    SendMessageOptions, ServiceBusClient,
};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut message = Message::from_string("This message will be deferred");
    ...
    if let Some(received_message) = receiver.receive_message(Some(receive_options)).await? {
        ...
        let defer_options = DeferMessageOptions::default();
        receiver
            .defer_message(&received_message, Some(defer_options))
            .await?;
        ...
        if let Some(seq_num) = sequence_number {
            ...
            match receiver
                .receive_deferred_message(seq_num, Some(ReceiveDeferredMessagesOptions::default()))
                .await?
            {
                Some(deferred_message) => {
                    ...
                    receiver
                        .complete_message(&deferred_message, Some(complete_options))
                        .await?;
        ...
        let deferred_messages = receiver
            .receive_deferred_messages(
                &sequence_numbers,
                Some(ReceiveDeferredMessagesOptions::default()),
            )
            .await?;
        println!(
