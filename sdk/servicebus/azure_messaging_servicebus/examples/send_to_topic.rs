...
use azure_messaging_servicebus::{
    CreateSenderOptions, Message, SendMessageOptions, ServiceBusClient,
};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sender = client
        .create_sender(&topic_name, Some(CreateSenderOptions::default()))
        .await?;

    // Send multiple messages
    for i in 1..=5 {
        let mut message = Message::from_string(format!("Message {} to topic", i));
