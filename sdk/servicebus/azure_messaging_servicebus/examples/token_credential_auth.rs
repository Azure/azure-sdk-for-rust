...
use azure_identity::{
    AzureCliCredential, ClientSecretCredential, DefaultAzureCredential, ManagedIdentityCredential,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Example 1: DefaultAzureCredential (Recommended for production)
    ...
    println!("\n=== Example 1: DefaultAzureCredential (Builder Pattern) ===");
    let default_credential = DefaultAzureCredential::new()?;
    let client = ServiceBusClient::builder()
        .open(&namespace, default_credential.clone())
        .await?;

    match send_test_message(&client, &queue_name, "DefaultAzureCredential").await {
        Ok(_) => println!("✅ DefaultAzureCredential authentication successful!"),
        Err(e) => println!("❌ DefaultAzureCredential authentication failed: {}", e),
    }
...
=== Example 3: ManagedIdentityCredential (Builder Pattern) ==="
    ...
    println!(
        "
=== Example 5: DefaultAzureCredential (Builder Pattern) ==="
    );
    ...
    println!("- DefaultAzureCredential: Should work in most environments");
...
async fn send_test_message(
    ...
    let mut message = Message::from_string(format!(
...
async fn receive_test_messages(
    ...
    for (i, message) in messages.iter().enumerate() {
        ...
        if let Some(credential_type) = message.get_property("credential_type") {
            println!("  - Credential type: {}", credential_type);
        }
        if let Some(timestamp) = message.get_property("timestamp") {
