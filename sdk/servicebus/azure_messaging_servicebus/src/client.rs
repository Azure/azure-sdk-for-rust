...
impl ServiceBusClient {
    ...
    /// Creates a new Service Bus client using a connection string with custom options.
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    /// #[tokio::main]
    /// async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ///     let connection_string = std::env::var("SERVICEBUS_CONNECTION_STRING")?;
    ...
    pub async fn from_connection_string(
        ...
        let _endpoint_url = Url::parse(&parsed.endpoint).map_err(|e| {
            ServiceBusError::new(
        ...
        let namespace = extract_namespace_from_endpoint(&parsed.endpoint)?;

        // Convert sb:// to amqps:// with port 5671 for AMQP connection
        let amqp_endpoint = format!("amqps://{}:5671", namespace);
        let amqp_url = Url::parse(&amqp_endpoint).map_err(|e| {
            ServiceBusError::new(
                ErrorKind::InvalidRequest,
                format!("Invalid AMQP endpoint URL: {}", e),
            )
        })?;

        let connection = Arc::new(AmqpConnection::new());
        let default_options = ServiceBusClientOptions::default();
        let options_ref = options.as_ref().unwrap_or(&default_options);
        let connection_options = Self::build_connection_options(options_ref);
        connection
            .open(
        ...
        Ok(Self {
            ...
            authorizer: None, // No credential provided, so no authorizer needed
        })
    }
    ...
    pub(crate) async fn new_internal(
        ...
        let connection_options = Self::build_connection_options(&options);
        connection
            .open(
                "servicebus-client".to_string(),
                endpoint_url,
...
fn parse_connection_string(connection_string: &str) -> Result<ParsedConnectionString> {
    ...
    let shared_access_key_name = shared_access_key_name.ok_or_else(|| {
        ServiceBusError::new(
            ErrorKind::InvalidRequest,
            "Missing SharedAccessKeyName in connection string",
        )
    })?;
    let shared_access_key = shared_access_key.ok_or_else(|| {
        ServiceBusError::new(
            ErrorKind::InvalidRequest,
            "Missing SharedAccessKey in connection string",
        )
    })?;
...
fn extract_namespace_from_endpoint(endpoint: &str) -> Result<String> {
    ...
    let host = url.host_str().ok_or_else(|| {
        ServiceBusError::new(ErrorKind::InvalidRequest, "Missing host in endpoint")
    })?;
...
impl ServiceBusClientBuilder {
    ...
    pub async fn open(
        ...
        ServiceBusClient::new_internal(fully_qualified_namespace, credential, options).await
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_connection_string_success() {
        let connection_string = "Endpoint=sb://test.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=test_key";
        ...
        assert_eq!(parsed.shared_access_key, "test_key");
    }

    #[test]
    fn parse_connection_string_missing_endpoint() {
        let connection_string =
            "SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=test_key";
        let result = parse_connection_string(connection_string);
    ...
    #[test]
    fn extract_namespace_success() {
        let endpoint = "sb://test.servicebus.windows.net/";
        let namespace = extract_namespace_from_endpoint(endpoint).unwrap();

        assert_eq!(namespace, "test.servicebus.windows.net");
    }
    ...
    fn test_build_connection_options_with_application_id() {
        ...
        let connection_options = ServiceBusClient::build_connection_options(&options);
