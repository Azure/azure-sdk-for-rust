...
pub struct SendMessageOptions {
    // Currently no options, but preserves API for future extensibility
}
...
///
/// // Create with default maximum size (determined by Service Bus)
...
impl Sender {
    ...
    /// sender.send_message(message, Some(options)).await?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// ];
    /// let options = SendMessagesOptions::default();
    /// sender.send_messages(messages, Some(options)).await?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///
    /// let message1 = Message::from_string("Hello");
    /// let message2 = Message::from_string("World");
    ///
    ...
    /// let messages = vec![
    ///     Message::from_string("Message 1"),
    ///     Message::from_string("Message 2"),
    ...
    /// let client = ServiceBusClient::builder().open("myservicebus.servicebus.windows.net", credential.clone()).await?;
    /// let sender = client.create_sender("queue_name", None).await?;
    /// let message = Message::from_string("Hello, World!");
...
mod tests {
    ...
    fn test_message_creation() {
        ...
        let string_message = Message::from_string("Hello, String!");
    ...
    fn test_message_custom_properties() {
        ...
        assert_eq!(
            message.get_property("custom_key"),
            Some(&"custom_value".to_string())
        );
        assert_eq!(message.get_property("number"), Some(&"42".to_string()));
        assert_eq!(message.get_property("non_existent"), None);
