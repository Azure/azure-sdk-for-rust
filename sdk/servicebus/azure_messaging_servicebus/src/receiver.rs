...
//! let client = ServiceBusClient::from_connection_string("connection_string", Default::default()).await?;
...
//!
//! let options = None;
//! if let Some(message) = receiver.receive_message(Some(options)).await? {
...
/// use azure_core::time::Duration;
///
/// // Default options - receive 1 message with 60 second timeout
/// let default_options = None;
///
...
/// use std::collections::HashMap;
///
/// // Basic dead letter without additional information
/// let basic_options = None;
///
...
/// let receiver = client.create_receiver("my-queue", None).await?;
///
/// // Receive a single message with default options
...
mod tests {
    ...
    async fn test_renew_message_lock_with_different_options() {
        ...
        let renew_options = RenewMessageLockOptions::default();

        let result = receiver
            .renew_message_lock(&message, Some(renew_options))
            .await;
    ...
    async fn test_receive_deferred_messages_basic() {
        ...
        let sequence_numbers = vec![12345, 67890];
        let options = ReceiveDeferredMessagesOptions::default();
        let result = receiver
            .receive_deferred_messages(&sequence_numbers, Some(options))
            .await;
    ...
    async fn test_peek_messages_basic() {
        ...
        let peek_options = PeekMessagesOptions::default();
        ...
        let result = receiver.peek_messages(10, Some(peek_options)).await;
    ...
    async fn test_peek_messages_zero_count() {
        ...
        let result = receiver.peek_messages(0, Some(peek_options)).await;
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
    ...
    async fn test_peek_messages_both_modes_allowed() {
        ...
        let peek_options = PeekMessagesOptions::default();
        let result = receiver_peek_lock
            .peek_messages(1, Some(peek_options.clone()))
            .await;
        ...
        let result = receiver_receive_delete
            .peek_messages(1, Some(peek_options.clone()))
            .await;
        // Should fail at AMQP level, not validation level
        assert!(result.is_err());
