...
use azure_messaging_servicebus::{
    CompleteMessageOptions, CreateMessageBatchOptions, CreateReceiverOptions, CreateSenderOptions,
    Message, ReceiveMessageOptions, ReceiveMode, SendMessagesOptions, ServiceBusClient,
};
...
async fn test_token_credential_batch_operations(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..message_count {
        ...
        let mut message = Message::from_string(format!("TokenCredential batch message {}", i));
        message.set_message_id(&message_id);
        message.set_property("credential_type", "DefaultAzureCredential");
        message.set_property("test_name", "test_token_credential_batch_operations");
        message.set_property("batch_index", i.to_string());
        messages.push(message);
    }

    let send_options = SendMessagesOptions::default();
    sender.send_messages(messages, Some(send_options)).await?;
    println!(
    ...
    let received_messages = receiver
        .receive_messages(message_count, Some(ReceiveMessageOptions::default()))
        .await?;
    ...
    for message in received_messages.iter() {
        if let Some(cred_type) = message.get_property("credential_type") {
            assert_eq!(cred_type, "DefaultAzureCredential");
        }

        receiver
            .complete_message(message, Some(CompleteMessageOptions::default()))
            .await?;
    }
...
async fn test_message_batch_send_receive(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..5 {
        ...
        let mut message = Message::from_string(&message_body);
        message.set_message_id(&message_id);
    ...
    while received_messages.len() < 5 && attempts < 10 {
        let messages = receiver
            .receive_messages(5, Some(ReceiveMessageOptions::default()))
            .await?;
        for msg in messages {
    ...
    for (expected_id, expected_body) in expected_messages {
        ...
        assert_eq!(msg.get_property("batch_id"), Some(&batch_id));

        // Complete the message
        receiver
            .complete_message(msg, Some(CompleteMessageOptions::default()))
            .await?;
...
async fn test_batch_size_limits(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    if !batch.is_empty() {
        ...
        let receiver = client
            ...
            .await?;
        let messages = receiver
            .receive_messages(added_count as usize, Some(ReceiveMessageOptions::default()))
            .await?;
...
async fn test_batch_with_different_options(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Test message for batch options");
    message.set_message_id(Uuid::new_v4().to_string());
    message.set_property("test_type", "batch_options");

    // Clone message for both batches
    let message_copy = Message::from_string("Test message for batch options");
    ...
    for msg in messages {
        if msg.get_property("test_type") == Some(&"batch_options".to_string()) {
            receiver
                .complete_message(&msg, Some(CompleteMessageOptions::default()))
                .await?;
        }
...
async fn test_batch_overflow_handling(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..20 {
        ...
        if !batch.try_add_message(message) {
            ...
            let mut retry_message = Message::from_string(&message_body);
...
async fn test_multiple_senders_receivers(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message1 = Message::from_string("Message from sender 1");
    message1.set_message_id(&message_id_1);

    let mut message2 = Message::from_string("Message from sender 2");
    ...
    let messages1 = receiver1
        .receive_messages(1, Some(ReceiveMessageOptions::default()))
        .await?;
    let messages2 = receiver2
        .receive_messages(1, Some(ReceiveMessageOptions::default()))
        .await?;
...
async fn test_receiver_lifecycle(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3 {
        ...
        let mut message = Message::from_string(format!("Receiver lifecycle test {}", i));
        message.set_message_id(&message_id);
        ...
        if let Some(received_message) = messages.first() {
            receiver
                .complete_message(received_message, Some(CompleteMessageOptions::default()))
                .await?;
        }
...
async fn test_basic_send_receive_round_trip(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Hello, Service Bus!");
    message.set_message_id(&message_id);
    ...
    let receiver = client
        ...
        .await?;
    let messages = receiver
        .receive_messages(10, Some(ReceiveMessageOptions::default()))
        .await?; // Try to get up to 10 messages
    ...
    assert_eq!(
        received_message.get_property("test_property"),
...
async fn test_batch_send(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3 {
        ...
        let mut message = Message::from_string(format!("Batch message {}", i));
        message.set_message_id(&message_id);
        message.set_correlation_id(&batch_id);
...
async fn test_batch_partitioned_queue(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..5 {
        ...
        let mut message = Message::from_string(format!("Partitioned message {}", i));
        message.set_message_id(&message_id);
...
async fn test_batch_size_limits_alt(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..50 {
        ...
        let mut message = Message::from_string(&message_text);
...
async fn test_batch_single_message_batch(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Single message in batch test");
