...
async fn test_message_properties_preservation(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Properties preservation test");
    ...
    assert_eq!(
        received_message.get_property("custom_prop_1"),
        Some("value1".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("custom_prop_2"),
        Some("value2".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("number_prop"),
...
async fn test_message_properties_comprehensive(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Comprehensive properties test message");
