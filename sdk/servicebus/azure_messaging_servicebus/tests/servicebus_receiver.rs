...
async fn test_receive_single_message(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Single receive test message");
...
async fn test_receive_multiple_messages(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..message_count {
        ...
        let mut message = Message::from_string(format!("Multiple receive test message {}", i));
        message.set_message_id(&message_id);
    ...
    let messages = receiver
        .receive_messages(
            ...
            Some(ReceiveMessageOptions::default()),
        )
        .await?;
...
async fn test_receive_and_delete_mode(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let sender = client
        ...
        .await?;
    let message_id = Uuid::new_v4().to_string();
    let mut message = Message::from_string("ReceiveAndDelete test message");
    message.set_message_id(&message_id);
