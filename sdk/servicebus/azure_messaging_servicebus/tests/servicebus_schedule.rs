...
use azure_messaging_servicebus::{
    CancelScheduledMessagesOptions, CreateSenderOptions, Message, ScheduleMessageOptions,
    ServiceBusClient,
};
...
async fn test_scheduled_message_send(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Scheduled message test");
    message.set_message_id(&message_id);
    message.set_property("test_type", "scheduled_send");

    let schedule_options = ScheduleMessageOptions::default();
    let sequence_number = sender
        .schedule_message(message, scheduled_time, Some(schedule_options))
        .await?;
...
async fn test_schedule_message_then_cancel(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Cancel scheduled message test");
    message.set_message_id(message_id);
    ...
    let cancel_options = CancelScheduledMessagesOptions::default();
    sender
        .cancel_scheduled_message(sequence_number, Some(cancel_options))
        .await?;
...
async fn test_schedule_multiple_messages(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3 {
        ...
        let mut message = Message::from_string(format!("Scheduled message {}", i));
        message.set_message_id(&message_id);
        message.set_property("test_type", "schedule_multiple");
        message.set_property("sequence", i.to_string());

        let scheduled_time = base_time + Duration::seconds(i as u64 * 60); // 1 minute apart
        let schedule_options = ScheduleMessageOptions::default();
        let sequence_number = sender
            .schedule_message(message, scheduled_time, Some(schedule_options))
            .await?;
