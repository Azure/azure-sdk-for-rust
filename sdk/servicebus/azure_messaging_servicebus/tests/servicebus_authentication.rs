...
use common::{get_connection_string, get_queue_name, get_servicebus_namespace};
...
async fn test_token_credential_message_properties(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("TokenCredential properties test");
    ...
    assert_eq!(
        received_message.get_property("credential_type"),
        Some("DefaultAzureCredential".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("test_name"),
        Some("test_token_credential_message_properties".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("environment"),
        Some("live_test".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("number_value"),
        Some("123".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("boolean_value"),
...
#[recorded::test(live)]
async fn test_client_from_connection_string(_ctx: TestContext) -> Result<(), Box<dyn Error>> {
    let connection_string = get_connection_string()?;
    let queue_name = get_queue_name()?;

    println!(
        "Testing client creation with connection string for queue: {}",
        queue_name
    );

    // Create client with connection string
    let client = ServiceBusClient::from_connection_string(&connection_string, None).await?;
    ...
    println!("Client connection string test completed successfully");
    Ok(())
}
...
async fn test_client_with_token_credential(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Testing client creation with DefaultAzureCredential for namespace: {}",
        namespace
    );
