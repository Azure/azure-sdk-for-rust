...
use azure_messaging_servicebus::{
    ...
    Message, ReceiveMessageOptions, ReceiveMode, SendMessageOptions, ServiceBusClient,
};
...
async fn test_client_lifecycle(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3 {
        ...
        let mut message = Message::from_string(format!("Lifecycle test message {}", i));
...
async fn test_peek_lock_operations(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("PeekLock test message");
    message.set_message_id(&message_id);
...
async fn test_resend(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Resend test message");
    message.set_message_id(message_id);
    ...
    sender
        .send_message(message.clone(), Some(send_options))
        .await?;
