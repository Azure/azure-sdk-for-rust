...
use azure_messaging_servicebus::{
    AbandonMessageOptions, CompleteMessageOptions, CreateReceiverOptions, CreateSenderOptions,
...
async fn test_basic_send_receive_round_trip(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        let purge_options = ReceiveMessageOptions {
            ...
            max_wait_time: Some(Duration::from_secs(2)), // Short timeout for cleanup
        };
...
async fn test_send_verify_receive_verify_empty(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let empty_check_options = ReceiveMessageOptions {
        max_message_count: 1,
        max_wait_time: Some(Duration::from_secs(2)), // Short timeout to check if empty
    };
    ...
    let mut message = Message::from_string("Test message for send-verify-receive-verify cycle");
    message.set_message_id(&message_id);
    ...
    let verify_options = ReceiveMessageOptions {
        ...
        max_wait_time: Some(Duration::from_secs(10)), // Reasonable timeout
    };
    ...
    for msg in &verification_messages {
        peek_receiver
            .abandon_message(msg, Some(AbandonMessageOptions::default()))
            .await?;
    }
    ...
    completion_receiver
        .complete_message(message_to_complete, Some(complete_options))
        .await?;
    ...
    let final_verify_options = ReceiveMessageOptions {
        max_message_count: 20, // Get multiple messages to search through
        max_wait_time: Some(Duration::from_secs(5)), // Reasonable timeout
    };
...
async fn test_send_single_message(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let message_id = Uuid::new_v4().to_string();
    let mut message = Message::from_string("Single message test");
    message.set_message_id(&message_id);
...
async fn test_send_multiple_messages(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..message_count {
        ...
        let mut message = Message::from_string(format!("Multiple send test message {}", i));
        message.set_message_id(&message_id);
...
async fn test_send_message_with_properties(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Message with comprehensive properties");
    message.set_message_id(&message_id);
...
async fn test_sender_lifecycle(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..3 {
        ...
        let mut message = Message::from_string(format!("Sender lifecycle test {}", i));
        message.set_message_id(&message_id);
...
async fn test_send_message_id(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Hello world with custom ID");
...
async fn test_send_amqp_annotated_message(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("AMQP annotated message with all properties");
...
async fn test_send_amqp_value_body(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string(structured_data.to_string());
...
async fn test_send_amqp_sequence_body(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string(&serde_json::to_string(&sequence_data)?);
