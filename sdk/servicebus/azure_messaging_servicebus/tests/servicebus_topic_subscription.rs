...
use azure_messaging_servicebus::{
    ...
    Message, ReceiveMessageOptions, ReceiveMode, ServiceBusClient,
};
...
async fn test_topic_subscription_messaging(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Topic subscription test message");
    message.set_message_id(&message_id);
    ...
    if let Some(test_name) = received_message.get_property("test_name") {
...
async fn test_topic_multiple_messages(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    for i in 0..message_count {
        ...
        let mut message = Message::from_string(format!("Topic multiple message {}", i));
        message.set_message_id(&message_id);
...
async fn test_topic_subscription_with_properties(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Topic message with properties");
    message.set_message_id(&message_id);
    ...
    assert_eq!(
        received_message.get_property("category"),
        Some("important".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("region"),
        Some("global".to_string()).as_ref()
    );
    assert_eq!(
        received_message.get_property("priority"),
...
async fn test_topic_subscription_peek_lock(ctx: TestContext) -> Result<(), Box<dyn Error>> {
    ...
    let mut message = Message::from_string("Topic abandon test message");
    message.set_message_id(&message_id);
    ...
    receiver
        .abandon_message(received_message, Some(AbandonMessageOptions::default()))
        .await?;

    // Try to receive it again (it should be available since we abandoned it)
    let messages_after_abandon = receiver
        .receive_messages(1, Some(ReceiveMessageOptions::default()))
        .await?;
    ...
    if !messages_after_abandon.is_empty() {
        ...
        receiver
            .complete_message(re_received, Some(CompleteMessageOptions::default()))
            .await?;
    }
