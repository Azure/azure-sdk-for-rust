...
fn router(
    ...
    Router::new().fallback(any(dispatch)).with_state(GatewayV2State {
        emulator,
        base_url,
        metrics,
        #[cfg(test)]
...
async fn execute(
    ...
    if request.method() != axum::http::Method::POST {
        return Err((StatusCode::METHOD_NOT_ALLOWED, "Gateway 2.0 requires POST".to_owned()));
    }
    ...
    let response = state.emulator.execute_gateway_v2_request(&request).await.map_err(|error| {
        let status = StatusCode::from_u16(u16::from(error.status().status_code())).unwrap_or(
...
mod tests {
    ...
    async fn connectivity_probe_requires_http2_and_returns_ok() {
        ...
        let region = VirtualRegion::new(
            "East US",
            Url::parse("http://127.0.0.1:18081/").unwrap()
    ...
    async fn real_driver_uses_hosted_gateway20_over_h2c() {
        ...
        let gateway_url = Url::parse(
            &format!("http://{}/", gateway_listener.local_addr().unwrap())
        ).unwrap();
        let thin_url = Url::parse(
            &format!("http://{}/", thin_listener.local_addr().unwrap())
        ).unwrap();
        ...
        let gateway_task = tokio::spawn(async move {
            axum::serve(gateway_listener, gateway_router).await
        });
        ...
        let driver = runtime.create_driver(DriverOptions::builder(account).build()).await.unwrap();
        let container = driver
            .resolve_container("db", "coll", OperationOptions::default()).await
            .unwrap();
        ...
        assert_eq!(response.status().status_code(), azure_core::http::StatusCode::Ok);
        assert!(
