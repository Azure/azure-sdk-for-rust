...
The examples are minimal — they skip exhaustive error handling and the
production "receive-loop" thread pattern. See
...
public final class CosmosSample {
    ...
    static final MethodHandle OP_CREATE_ITEM    = h("cosmos_operation_create_item", FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS, ADDRESS, ADDRESS));
    static final MethodHandle OP_WITH_BODY      = h("cosmos_operation_with_body",   FunctionDescriptor.of(JAVA_INT, ADDRESS, ADDRESS, JAVA_LONG));
    static final MethodHandle OP_FREE           = h("cosmos_operation_free",        FunctionDescriptor.ofVoid(ADDRESS));
    static final MethodHandle SUBMIT            = h("cosmos_driver_submit",         FunctionDescriptor.of(ADDRESS, ADDRESS, ADDRESS, ADDRESS, ADDRESS, ADDRESS, ADDRESS));
    ...
    public static void main(String[] args) throws Throwable {
        try (Arena arena = Arena.ofConfined()) {
            ...
            MemorySegment outOp = arena.allocate(ADDRESS);
            OP_CREATE_ITEM.invokeExact(coll, cstr(arena, "doc1"), pk, outOp);
            MemorySegment op = outOp.get(ADDRESS, 0);
            OP_WITH_BODY.invokeExact(op, bodySeg, (long) body.length);

            MemorySegment preErr = arena.allocate(JAVA_INT);
            MemorySegment hdl = (MemorySegment) SUBMIT.invokeExact(drv, op, MemorySegment.NULL, q, MemorySegment.NULL, preErr);
            MemorySegment comp = (MemorySegment) CQ_WAIT.invokeExact(q, 30_000);
            OP_HND_FREE.invokeExact(hdl);
            OP_FREE.invokeExact(op);

            int outcome = (int) COMP_OUTCOME.invokeExact(comp);
            MemorySegment resp = (MemorySegment) COMP_TAKE_R.invokeExact(comp);
            System.out.printf("CREATE outcome=%d status=%d ru=%.2f%n",
                outcome,
            ...
            // 5/6. READ + DELETE — same shape; omitted for brevity.
...
func mustCstr(s string) *C.char { return C.CString(s) }

func submitAndWait(drv, op, q unsafe.Pointer) unsafe.Pointer {
    var pre C.int32_t
    h := C.cosmos_driver_submit(
        (*C.struct_cosmos_driver_t)(drv),
        (*C.struct_cosmos_operation_t)(op),
        nil,
        (*C.struct_cosmos_cq_t)(q),
        nil,
        &pre,
    )
    if h == nil {
        panic(fmt.Sprintf("submit pre-flight failed: %d", pre))
    }
    c := C.cosmos_cq_wait((*C.struct_cosmos_cq_t)(q), 30000)
    C.cosmos_operation_handle_free(h)
    C.cosmos_operation_free((*C.struct_cosmos_operation_t)(op))
...
func main() {
    ...
    ua := mustCstr("go-sample"); defer C.free(unsafe.Pointer(ua))
    C.cosmos_runtime_builder_with_user_agent_suffix(rb, ua)
    var rt *C.struct_cosmos_runtime_t
    C.cosmos_runtime_builder_build(rb, &rt, nil)
    q := C.cosmos_cq_create(rt, nil)

    // 2. Account → driver → container
    endp := mustCstr("https://localhost:8081/"); defer C.free(unsafe.Pointer(endp))
    key  := mustCstr("C2y6yDjf5/R+ob0N8A7Cgv30VRDJIWEHLM+4QDU5DE2nQ9nDuVTqobD4b8mGGyPMbIZnqyMsEcaGQy67XIw/Jw=="); defer C.free(unsafe.Pointer(key))
    var acct *C.struct_cosmos_account_ref_t
    C.cosmos_account_ref_with_master_key(endp, key, &acct, nil)
    var drv *C.struct_cosmos_driver_t
    C.cosmos_driver_get_or_create_blocking(rt, acct, nil, &drv, nil)
    db   := mustCstr("sample-db");   defer C.free(unsafe.Pointer(db))
    coll := mustCstr("sample-coll"); defer C.free(unsafe.Pointer(coll))
    var container *C.struct_cosmos_container_ref_t
    C.cosmos_driver_resolve_container_blocking(rt, drv, db, coll, &container, nil)

    // 3. Partition key
    pkb := C.cosmos_partition_key_builder_new()
    pkVal := mustCstr("tenant-42"); defer C.free(unsafe.Pointer(pkVal))
    C.cosmos_partition_key_builder_add_string(pkb, pkVal)
    var pk *C.struct_cosmos_partition_key_t
    C.cosmos_partition_key_builder_build(pkb, &pk)

    // 4. CREATE
    body, _ := json.Marshal(Doc{ID: "doc1", Pk: "tenant-42", Name: "hello"})
    docId := mustCstr("doc1"); defer C.free(unsafe.Pointer(docId))
    var op *C.struct_cosmos_operation_t
    C.cosmos_operation_create_item(container, docId, pk, &op)
    C.cosmos_operation_with_body(op, (*C.uint8_t)(unsafe.Pointer(&body[0])), C.size_t(len(body)))
    comp := (*C.struct_cosmos_completion_t)(submitAndWait(unsafe.Pointer(drv), unsafe.Pointer(op), unsafe.Pointer(q)))
    ...
    C.cosmos_response_free(resp); C.cosmos_completion_free(comp)

    // 5. READ
    C.cosmos_operation_read_item(container, docId, pk, &op)
    comp = (*C.struct_cosmos_completion_t)(submitAndWait(unsafe.Pointer(drv), unsafe.Pointer(op), unsafe.Pointer(q)))
    resp = C.cosmos_completion_take_response(comp)
    var dataPtr *C.uint8_t; var dataLen C.size_t
    ...
    // 6. DELETE
    C.cosmos_operation_delete_item(container, docId, pk, &op)
    comp = (*C.struct_cosmos_completion_t)(submitAndWait(unsafe.Pointer(drv), unsafe.Pointer(op), unsafe.Pointer(q)))
    resp = C.cosmos_completion_take_response(comp)
    ...
    // 7. Tear-down
    C.cosmos_partition_key_free(pk)
    C.cosmos_container_ref_free(container)
    C.cosmos_driver_free(drv)
    C.cosmos_account_ref_free(acct)
    C.cosmos_cq_free(q)
    C.cosmos_runtime_free(rt)
}
```
...
_runtime_builder_ua    = _decl("cosmos_runtime_builder_with_user_agent_suffix", [void_p, ctypes.c_char_p], ctypes.c_int32)
...
_acct_with_key         = _decl("cosmos_account_ref_with_master_key", [ctypes.c_char_p, ctypes.c_char_p, ctypes.POINTER(void_p), ctypes.POINTER(void_p)], ctypes.c_int32)
...
_resolve_container_blk = _decl("cosmos_driver_resolve_container_blocking", [void_p, void_p, ctypes.c_char_p, ctypes.c_char_p, ctypes.POINTER(void_p), ctypes.POINTER(void_p)], ctypes.c_int32)
_container_free        = _decl("cosmos_container_ref_free", [void_p], None)
_pkb_new               = _decl("cosmos_partition_key_builder_new", [], void_p)
_pkb_add_s             = _decl("cosmos_partition_key_builder_add_string", [void_p, ctypes.c_char_p], ctypes.c_int32)
...
_op_create_item        = _decl("cosmos_operation_create_item", [void_p, ctypes.c_char_p, void_p, ctypes.POINTER(void_p)], ctypes.c_int32)
_op_read_item          = _decl("cosmos_operation_read_item",   [void_p, ctypes.c_char_p, void_p, ctypes.POINTER(void_p)], ctypes.c_int32)
_op_delete_item        = _decl("cosmos_operation_delete_item", [void_p, ctypes.c_char_p, void_p, ctypes.POINTER(void_p)], ctypes.c_int32)
_op_with_body          = _decl("cosmos_operation_with_body",   [void_p, u8_p, size_t], ctypes.c_int32)
_op_free               = _decl("cosmos_operation_free", [void_p], None)
_driver_submit         = _decl("cosmos_driver_submit", [void_p, void_p, void_p, void_p, void_p, ctypes.POINTER(ctypes.c_int32)], void_p)
...
def submit_and_wait(drv, op, q):
    pre = ctypes.c_int32(0)
    h = _driver_submit(drv, op, None, q, None, ctypes.byref(pre))
    if not h:
...
def main() -> int:
    ...
    rt = void_p(); _runtime_builder_build(rb, ctypes.byref(rt), None)
    ...
    drv = void_p(); _driver_goc_blk(rt, acct, None, ctypes.byref(drv), None)
    container = void_p()
    _resolve_container_blk(rt, drv, b"sample-db", b"sample-coll", ctypes.byref(container), None)

    # 3. Partition key
    pkb = _pkb_new()
    _pkb_add_s(pkb, b"tenant-42")
    pk = void_p(); _pkb_build(pkb, ctypes.byref(pk))

    # 4. CREATE
    body = json.dumps({"id": "doc1", "pk": "tenant-42", "name": "hello"}).encode("utf-8")
    body_buf = (ctypes.c_uint8 * len(body)).from_buffer_copy(body)
    op = void_p(); _op_create_item(container, b"doc1", pk, ctypes.byref(op))
    _op_with_body(op, body_buf, len(body))
    comp = submit_and_wait(drv, op, q)
    resp = _comp_take_resp(comp)
    print(f"CREATE outcome={_comp_outcome(comp)} status={_resp_status(resp)} ru={_resp_ru(resp):.2f}")
    _resp_free(resp); _comp_free(comp)

    # 5. READ
    op = void_p(); _op_read_item(container, b"doc1", pk, ctypes.byref(op))
    comp = submit_and_wait(drv, op, q)
    resp = _comp_take_resp(comp)
    data_ptr = u8_p(); data_len = size_t()
    _resp_body(resp, ctypes.byref(data_ptr), ctypes.byref(data_len))
    print(f"READ status={_resp_status(resp)} body={ctypes.string_at(data_ptr, data_len.value).decode('utf-8')}")
    _resp_free(resp); _comp_free(comp)

    # 6. DELETE
    op = void_p(); _op_delete_item(container, b"doc1", pk, ctypes.byref(op))
    comp = submit_and_wait(drv, op, q)
    resp = _comp_take_resp(comp)
...
1. **Error handling is elided for brevity.** Production code should check
   every return code, check `outcome != OK`, pull `cosmos_completion_take_error`
   to read the rich `cosmos_error_t`, and decide whether to retry / surface /
   log.
