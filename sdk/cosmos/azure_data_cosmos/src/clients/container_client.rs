...
use crate::{
    ...
    DeleteContainerOptions, FeedPager, ItemOptions, PartitionKey, Query, QueryChangeFeedOptions,
    ReadFeedRangesOptions, ReplaceContainerOptions, ThroughputOptions,
};
use std::sync::Arc;
...
impl ContainerClient {
    ...
    /// }
    /// # let container_client: azure_data_cosmos::clients::ContainerClient = panic!("this is a non-running example");
    /// let options = QueryChangeFeedOptions {
    ///     start_from: Some(ChangeFeedStartFrom::Beginning),
    ///     ..Default::default()
    /// };
    ...
    pub fn query_items_change_feed<T: DeserializeOwned + Send + 'static>(
        &self,
        options: Option<QueryChangeFeedOptions<'_>>,
    ) -> azure_core::Result<FeedPager<T>> {
    ...
    pub async fn read_feed_ranges(
        &self,
        options: Option<ReadFeedRangesOptions<'_>>,
