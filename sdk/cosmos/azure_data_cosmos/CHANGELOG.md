...
- Replaced `CosmosClientBuilder::with_partition_key_range_cache_enabled` with `PartitionFailoverOptions::with_partition_topology_cache_mode`; partition topology caching can no longer be disabled, and `PartitionTopologyCacheMode::{Eager, Lazy}` controls only when the cache is loaded.
- Container resolution now loads the complete partition topology by default and fails if it cannot load a valid routing map. `PartitionTopologyCacheMode::Lazy` preserves the previous first-use loading behavior as a compatibility escape hatch. ([#5279](https://github.com/Azure/azure-sdk-for-rust/issues/5279))
