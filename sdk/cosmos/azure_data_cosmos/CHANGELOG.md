...
- RID-addressed container feed operations now retain Gateway 2.0 routing for create, upsert, query, change feed, and transactional batch workloads instead of silently falling back to standard Gateway. ([#5071](https://github.com/Azure/azure-sdk-for-rust/pull/5071))
- `ContainerClient::read_item`, `replace_item`, `delete_item`, and `patch_item` now accept a [`ResourceId`] for `item_id`, addressing an item by its service-assigned `_rid` instead of its `id`. This matches the existing `database_client`/`container_client` addressing idiom. RID-addressed point operations route through Gateway 2.0, and the driver rejects an item RID that is malformed or belongs to another container before the request is sent. ([#5071](https://github.com/Azure/azure-sdk-for-rust/pull/5071))
...
- `ContainerClient::read_item`, `replace_item`, `delete_item`, and `patch_item` now take `item_id: impl Into<ResourceIdentity>` instead of `&str`, so an item can be addressed by its service-assigned `_rid` as well as its `id`. `&str`, `&String`, and `String` all work directly, but the change removes deref coercion: any type that previously reached `&str` via `Deref` — `Cow<str>`, `Box<str>`, `Rc<str>`/`Arc<str>`, and `&&str` (common when iterating a collection of ids) — now needs `&*value` or `.as_ref()`. ([#5071](https://github.com/Azure/azure-sdk-for-rust/pull/5071))
