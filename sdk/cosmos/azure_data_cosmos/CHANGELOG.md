...
- Added `CosmosError` and the `CosmosResult<T>` type alias, which wrap `azure_core::Error` with per-operation `status_code`, `sub_status`, and `diagnostics` fields. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- Per-operation `DiagnosticsContext` (ActivityId, region, transport shard, per-attempt event history, final status) is now attached to the error path. On any failed operation the context is reachable through the `CosmosError::diagnostics` field. Response-body parse failures (`CosmosResponse::into_model`, `offers_client::find_offer`), `ContainerClient::begin_replace_throughput` empty-offer errors, and routing-map lookup failures behind `ContainerClient::read_feed_ranges` / `feed_range_from_partition_key` also surface the originating operation's diagnostics. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
