...
- Added `CosmosError` — a transparent record with public fields — and the `CosmosResult<T>` type alias. The struct exposes the wrapped `Arc<azure_core::Error>` plus the per-operation `status_code: Option<u16>`, `sub_status: Option<u32>`, and `diagnostics: Option<Arc<DiagnosticsContext>>` as plain fields so callers can pattern-match, project into ADX columns, or serialize directly. Convenience methods `kind()`, `http_status()`, `status_code()`, `sub_status()`, `diagnostics()`, `as_azure_error()`, `into_source()`, and `into_azure_error_lossy()` are provided for ergonomics. The struct is `#[non_exhaustive]` and derives `Clone` via the `Arc` source handle (`azure_core::Error` is itself `!Clone`). Hand-rolled `Debug` projects the error through `ErrorKind`'s `Display` impl rather than its derived `Debug`, so `format!("{err:?}")` cannot leak `ErrorKind::HttpResponse::raw_response` (captured response headers, session tokens, `WWW-Authenticate` challenges, masked-key reason text) into logs. `From<serde_json::Error>` is also provided since serialize/deserialize is the dominant `?` site; other conversion errors require `.map_err(azure_core::Error::from)?`. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
