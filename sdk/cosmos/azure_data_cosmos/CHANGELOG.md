...
- Change feed items are now surfaced as the wire-format envelope. Callers bind `T = ChangeFeedItem<YourDoc>` on `ContainerClient::query_change_feed()` and read the post-change document via `ChangeFeedItem::current()`. The envelope also exposes the pre-change document (`previous()`) and per-change `metadata()` (populated by full-fidelity reads; absent for `LatestVersion`). A backend that does not envelope change feed items (such as an emulator build without the wire format) returns the bare document, which is mapped onto `current()` so no data is lost. Added the `models` types `ChangeFeedItem<T>`, `ChangeFeedMetadata`, and `ChangeFeedOperationType`. ([#4723](https://github.com/Azure/azure-sdk-for-rust/pull/4723))
