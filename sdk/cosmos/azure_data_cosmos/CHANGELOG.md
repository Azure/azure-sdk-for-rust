...
- Added `FeedRange` type with `ContainerClient::read_feed_ranges()` and `ContainerClient::feed_range_from_partition_key()`. Supports hierarchical partition keys (MultiHash) including prefix partition keys that return multiple feed ranges. ([#3987](https://github.com/Azure/azure-sdk-for-rust/pull/3987))
