...
- Change feed items are now surfaced as the wire-format envelope. Callers bind `T = ChangeFeedItem<YourDoc>` on `ContainerClient::query_change_feed()` and read the post-change document via `ChangeFeedItem::current()`. The envelope also exposes the pre-change document (`previous()`) and per-change `metadata()` (populated by full-fidelity reads; absent for `LatestVersion`). Added the `models` types `ChangeFeedItem<T>`, `ChangeFeedMetadata`, and `ChangeFeedOperationType`. ([#4723](https://github.com/Azure/azure-sdk-for-rust/pull/4723))
- For backends that do not envelope change feed items (such as an emulator build without the wire format), the SDK now maps the bare document payload onto `ChangeFeedItem::current()` so no data is lost. ([#4723](https://github.com/Azure/azure-sdk-for-rust/pull/4723))
- Added `ChangeFeedMode::AllVersionsAndDeletes` ("full fidelity") to `ContainerClient::query_change_feed()`. In this mode the service returns every intermediate version and delete, and the `ChangeFeedItem<T>` envelope's `previous()` and `metadata()` (operation type, LSN, commit timestamp, previous-image LSN, and a time-to-live-expiry flag) are populated. `ChangeFeedStartFrom::Beginning` is rejected for this mode because intermediate versions and deletes are only retained within the container's retention window; use `Now` or an in-retention `PointInTime`. The continuation token does not persist the feed mode, so callers must re-pass `AllVersionsAndDeletes` on resume. Added the `models` type `ChangeFeedPolicy` and `ContainerProperties::with_change_feed_policy()` to configure a container's full-fidelity retention window (required to read in this mode). ([#4706](https://github.com/Azure/azure-sdk-for-rust/pull/4706))
