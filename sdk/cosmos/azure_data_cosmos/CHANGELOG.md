...
- Added `CosmosError` newtype (`pub struct CosmosError(azure_core::Error)`) and `CosmosResult<T>` type alias. `CosmosError` exposes Cosmos-specific accessors — `kind`, `status_code` (final outcome after retries/failovers, diagnostics-aware, returns `Option<u16>`), `sub_status` (`Option<u32>`), and `diagnostics` (`Option<Arc<CosmosDiagnosticsContext>>`) — and skips an internal diagnostics carrier from its `source()` chain so callers never observe driver-internal error types. Hand-rolled `Debug` strips `raw_response` from `ErrorKind::HttpResponse` so `format!("{err:?}")` cannot leak captured response headers (session tokens, `WWW-Authenticate` challenges, masked-key reason text) into application logs. `From<azure_core::Error>` and `From<CosmosError> for azure_core::Error` provide bidirectional `?`-conversion across the SDK boundary; `From<serde_json::Error>` is also provided since serialize/deserialize is the dominant `?` site (other conversion errors require `.map_err(azure_core::Error::from)?`). ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- Per-operation `DiagnosticsContext` (ActivityId, region, transport shard, per-attempt event history, final status) is now attached to the error path. On any failed operation the context is recoverable via `CosmosError::diagnostics()`. Driver-side parse failures during response deserialization (`CosmosResponse::into_model`, `offers_client::find_offer`) now also attach the originating operation's diagnostics. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))

### Breaking Changes

- Every public Cosmos API now returns `CosmosResult<T>` (i.e. `Result<T, CosmosError>`) instead of `azure_core::Result<T>`. `?`-chained calls inside functions returning `CosmosResult` keep working unchanged thanks to `From<azure_core::Error> for CosmosError`. Direct-return patterns like `fn my_fn() -> azure_core::Result<X> { client.read_item(...).await }` need either `.map_err(Into::into)` or a return-type change to `CosmosResult<X>` — there is no automatic upward coercion. The `azure_core::Error` is still reachable via `err.as_azure_error()` / `err.into_azure_error()` for interop with code that holds the generic Azure SDK error. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
