...
- Added `CosmosError` — a transparent record (à la `azure_storage_blob::StorageError`) with public fields — and the `CosmosResult<T>` type alias. The struct exposes the wrapped `Arc<azure_core::Error>` plus the per-operation `status_code: Option<u16>`, `sub_status: Option<u32>`, and `diagnostics: Option<Arc<CosmosDiagnosticsContext>>` as plain fields so callers can pattern-match, project into ADX columns, or serialize directly without going through accessor methods. Convenience methods `status_code()`, `sub_status()`, `diagnostics()`, `as_azure_error()`, and `into_source()` are provided for ergonomics but are zero-cost field-access shims. The struct is `#[non_exhaustive]` (preview-friendly) and derives `Clone` via the `Arc<…>` source handle (`azure_core::Error` is itself `!Clone`). Hand-rolled `Debug` projects the error through `ErrorKind`'s `Display` impl rather than its derived `Debug`, so `format!("{err:?}")` cannot leak `ErrorKind::HttpResponse::raw_response` (captured response headers / session tokens / `WWW-Authenticate` challenges / masked-key reason text) into logs. The bridge from the driver's `azure_core::Error` (which transports diagnostics via a `pub(crate) ErrorWithDiagnostics` carrier) to `CosmosError` happens exactly once, in `From<azure_core::Error>`: the diagnostics are extracted and the carrier is stripped from the source chain so `CosmosError::source().source()...` walks contain only original wrapped errors. `From<serde_json::Error>` is also provided since serialize/deserialize is the dominant `?` site; other conversion errors (`base64::DecodeError`, `url::ParseError`, …) require `.map_err(azure_core::Error::from)?` per call site (documented policy in the `error.rs` module docs). ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- Per-operation `DiagnosticsContext` (ActivityId, region, transport shard, per-attempt event history, final status) is now attached to the error path. On any failed operation the context is reachable through the `CosmosError::diagnostics` field. Response-body parse failures (`CosmosResponse::into_model`, `offers_client::find_offer`) also attach the originating operation's diagnostics. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))

### Breaking Changes

- Every public Cosmos API now returns `CosmosResult<T>` (i.e. `Result<T, CosmosError>`) instead of `azure_core::Result<T>`. `?`-chained calls inside functions returning `CosmosResult` keep working unchanged thanks to `From<azure_core::Error> for CosmosError`. Direct-return patterns like `fn my_fn() -> azure_core::Result<X> { client.read_item(...).await }` need either `.map_err(Into::into)` or a return-type change to `CosmosResult<X>` — there is no `From<CosmosError> for azure_core::Error` impl. To bridge a `CosmosError` upward, use `err.as_azure_error()` (borrow) or `err.into_source()` (owned `Arc<azure_core::Error>`). ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- `fault_injection::CustomResponseBuilder::with_sub_status` now takes `u32` instead of `impl Into<SubStatusCode>`. The change keeps the fault-injection surface free of driver-internal types; the most common sub-status codes are documented in the method's rustdoc. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- Removed the `try_extract_diagnostics`, `ErrorWithDiagnostics`, `CosmosStatus`, and `SubStatusCode` re-exports from `azure_data_cosmos` and `azure_data_cosmos::models`. The recommended replacement is `CosmosError::diagnostics` / `CosmosError::status_code` / `CosmosError::sub_status`, none of which return driver-internal types. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
