...
- Support for simple cross-partition queries with `SELECT` projections and `WHERE` filters. Cross-partition queries are now done through fan-out in the client, and provide a client-generated continuation token that can be used to resume the query. See `ContainerClient::query_items()` and `FeedScope` for details. ([#4391](https://github.com/Azure/azure-sdk-for-rust/pull/4391))
...
- `ContainerClient::query_items()` now takes a `FeedScope` (`FeedScope::partition(...)`, `FeedScope::feed_range(...)`, or `FeedScope::full_container()`) instead of a partition key where `()` represented cross-partition queries. ([#4391](https://github.com/Azure/azure-sdk-for-rust/pull/4391))
