...
- Added finite cross-partition `ORDER BY VectorDistance(...)` queries with `TOP` or `OFFSET`/`LIMIT`. Results are fully buffered before the first page, bounded by `QueryOptions::max_non_streaming_order_by_buffered_items` (50,000 candidate rows by default), and cannot be resumed from continuation tokens. Hybrid/full-text vector ranking remains unsupported.
