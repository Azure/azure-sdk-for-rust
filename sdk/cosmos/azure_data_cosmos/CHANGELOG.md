...
- `ContainerClient::query_items()` now takes a `FeedScope` (`FeedScope::partition(...)`, `FeedScope::range(...)`, or `FeedScope::full_container()`) instead of a partition key where `()` represented cross-partition queries. ([#4391](https://github.com/Azure/azure-sdk-for-rust/pull/4391))
