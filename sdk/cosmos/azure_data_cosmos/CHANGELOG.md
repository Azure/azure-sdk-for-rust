...
- Added `QueryOptions::with_populate_index_metrics(bool)`, `with_populate_query_metrics(bool)`, and `with_max_item_count(MaxItemCountHint)` setters. These replace the previous pattern of passing raw `x-ms-cosmos-populateindexmetrics`, `x-ms-documentdb-populatequerymetrics`, and `x-ms-max-item-count` values through `OperationOptions::with_custom_headers` for query execution. `max_item_count` takes the new `MaxItemCountHint` enum with `ServerDecides` and `Limit(NonZeroI32)` variants, so callers don't have to traffic in the `-1` wire sentinel directly.

### Breaking Changes

- Response wrappers (`ItemResponse`, `ResourceResponse`, `BatchResponse`) no longer expose `azure_core` HTTP types for status, headers, and body. `ItemResponse<T>` drops its type parameter (the payload type is user-defined and not known to the SDK): existing `container.read_item::<MyItem>(...).await?.into_model()` becomes `container.read_item(...).await?.into_model::<MyItem>()`, or use `response.into_body().single_item::<MyItem>()`. `ResourceResponse<T>` keeps its type parameter — every `ResourceResponse`-returning client method statically knows its model (`DatabaseProperties`, `ContainerProperties`, `ThroughputProperties`, …), so `.into_model()?` works without a turbofish. All three return SDK-owned wrapper types: `status() -> CosmosStatus`, `headers() -> &ResponseHeaders`, and `into_body() -> ResponseBody`. `ResponseBody` provides `single()`, `single_item::<T>()`, `into_items::<T>()`, and `into_string()` helpers; legacy concatenating helpers (`into_bytes` / `as_contiguous_bytes`) are intentionally not provided. `CosmosStatus` is re-exported from the driver and already implements `PartialEq<StatusCode>` and `From<CosmosStatus> for StatusCode/u16`, so existing comparisons and conversions continue to work. `ResponseHeaders` exposes individual fields through dedicated accessor methods (`etag()`, `request_charge()`, `session_token()`, `continuation()`, `activity_id()`, `substatus()`, `index_metrics()`, `query_metrics()`, `offer_replace_pending()`, `server_duration_ms()`, `lsn()`, `item_lsn()`, `item_count()`). The driver header struct is intentionally hidden behind these accessors so the driver remains free to evolve its internal representation; the *value* types returned (`ETag`, `RequestCharge`, `SessionToken`, `SubStatusCode`, `ActivityId`) are stable Cosmos primitives re-exported from the driver.
- `FeedPage::headers()` / `QueryFeedPage::headers()` now return `&ResponseHeaders` (the SDK-owned wrapper) instead of `&azure_core::http::headers::Headers`. Callers should reach individual fields through the typed accessors (`headers().etag()`, `headers().request_charge()`, …).
- Removed the `ItemResponse::etag()` convenience accessor. Use `response.headers().etag()` instead (returns `Option<&ETag>`).
- `ResponseBody` and `ResponseHeaders` are SDK-owned types defined in `azure_data_cosmos::models` and re-exported at the crate root. `CosmosStatus` continues to be a re-export of the driver type.

### Other Changes

- The SDK no longer uses `OperationOptions::with_custom_headers` to set well-known Cosmos query, changefeed, and offer headers (`x-ms-documentdb-isquery`, `Content-Type: application/query+json`, `x-ms-max-item-count`, `A-IM: Incremental Feed`, `x-ms-cosmos-populateindexmetrics`, `x-ms-documentdb-populatequerymetrics`, `x-ms-documentdb-query-enablecrosspartition`). These are now set via typed fields on `CosmosRequestHeaders` and emitted by the driver pipeline. No public API change for end users.
