...
- Gateway 2.0 transport (a regional proxy forwarding RNTBD-over-HTTP/2) is selected automatically when the account advertises thin-client endpoints and the connectivity probe confirms them. Transport selection is fully server-driven — there is no client API or environment variable to opt a client in or out — with HTTP/2 as the only client-side prerequisite. ([#4319](https://github.com/Azure/azure-sdk-for-rust/pull/4319))
