...
- Added `QueryOptions::with_populate_index_metrics(bool)`, `with_populate_query_metrics(bool)`, and `with_max_item_count(NonZeroI32)` setters. These replace the previous pattern of passing raw `x-ms-cosmos-populateindexmetrics`, `x-ms-documentdb-populatequerymetrics`, and `x-ms-max-item-count` values through `OperationOptions::with_custom_headers` for query execution. `max_item_count` uses `Option<NonZeroI32>` so callers must construct the value explicitly (e.g. `NonZeroI32::new(100).unwrap()`); `-1` is the documented sentinel for "server decides".

### Breaking Changes

- Response wrappers (`ItemResponse`, `ResourceResponse`, `BatchResponse`) no longer expose `azure_core` HTTP types for status, headers, and body. They now return SDK-owned wrapper types: `status() -> CosmosStatus`, `headers() -> &ResponseHeaders`, and `into_body() -> ResponseBody`. `ResponseBody` provides `single()`, `single_item::<T>()`, `into_items::<T>()`, `json_single::<T>()`, and `into_string()` helpers; legacy concatenating helpers (`into_bytes` / `as_contiguous_bytes`) are intentionally not provided. `CosmosStatus` is re-exported from the driver and already implements `PartialEq<StatusCode>` and `From<CosmosStatus> for StatusCode/u16`, so existing comparisons and conversions continue to work. `ResponseHeaders` exposes individual fields through dedicated accessor methods (`etag()`, `request_charge()`, `session_token()`, `continuation()`, `activity_id()`, `substatus()`, `index_metrics()`, `query_metrics()`, `offer_replace_pending()`, `server_duration_ms()`, `lsn()`, `item_lsn()`, `item_count()`) and intentionally does not expose the underlying driver header struct.
