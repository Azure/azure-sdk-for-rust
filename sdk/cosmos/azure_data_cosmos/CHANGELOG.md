...
- `ContainerClient::patch_item` can now execute a patch server-side in a single request instead of always running the driver's read-modify-write loop. This halves the round trips and, on a multi-write-region account, lets the service resolve concurrent patches at the path level so two writers touching different properties of the same item both survive. `PatchItemOptions::strategy` selects the path; the default `PatchStrategy::Auto` runs server-side when the operation list is safe to resend and within the service's 10-operation limit, and falls back to the loop otherwise. The response body contract is unchanged: a server-side patch defaults `content_response_on_write` to `Enabled` when the caller left it unset, so `patch_item` still returns the post-image whichever path runs. `PatchItemOptions::max_attempts` continues to apply to the loop only. ([#5111](https://github.com/Azure/azure-sdk-for-rust/pull/5111))
