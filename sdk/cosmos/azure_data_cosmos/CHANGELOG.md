...
- Gateway 2.0 transport is enabled by default. Gateway 2.0 routes data-plane requests through a regional proxy that forwards RNTBD-over-HTTP/2 to the backend. To keep traffic on the standard gateway (Gateway 1.0), build a `CosmosRuntime` whose connection pool sets `ConnectionPoolOptions::with_gateway20_disabled(true)` (or set the `AZURE_COSMOS_CONNECTION_POOL_GATEWAY20_DISABLED` environment variable) and attach it via `CosmosClientBuilder::with_runtime` — useful for A/B testing transport behavior, isolating a transport-specific issue, or staying on the standard gateway during a controlled rollout. ([#4319](https://github.com/Azure/azure-sdk-for-rust/pull/4319))
