...
| `hedging_enabled` | `Option<bool>` | `AZURE_COSMOS_HEDGING_ENABLED` | Master switch for cross-region read hedging. `None` (unset) is treated as **enabled**, using the built-in default threshold of `min(1000ms, request_timeout / 2)`. When set, it is the **source of truth** and takes precedence over the programmatic `AvailabilityStrategy` in both directions: `false` disables hedging even when an explicit `AvailabilityStrategy::Hedging(..)` is configured, and `true` enables hedging even when an explicit `AvailabilityStrategy::Disabled` is configured (a programmatic `Hedging(..)` still supplies its custom threshold). |
