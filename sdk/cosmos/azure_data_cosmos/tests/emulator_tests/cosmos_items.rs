...
pub async fn item_crud() -> Result<(), Box<dyn Error>> {
    TestClient::run_with_shared_db(
        async |run_context, _db_client| {
            ...
            let response = container_client.create_item(&pk, &item_id, &item, None).await?;
            assert_response(
                &response,
                StatusCode::Created,
...
pub async fn item_read_system_properties() -> Result<(), Box<dyn Error>> {
    TestClient::run_with_shared_db(
        async |run_context, _db_client| {
            ...
            let create_response = container_client.create_item(&pk, &item_id, &item, None).await?;
            assert_response(
                &create_response,
                StatusCode::Created,
                &get_effective_hub_endpoint(),
...
pub async fn item_upsert_new() -> Result<(), Box<dyn Error>> {
    TestClient::run_with_shared_db(
        async |run_context, _db_client| {
            ...
            let upsert_response = container_client.upsert_item(&pk, &item_id, &item, None).await?;
            assert_response(
                &upsert_response,
                StatusCode::Created,
                &get_effective_hub_endpoint(),
...
pub async fn create_item_duplicate_returns_conflict() -> Result<(), Box<dyn Error>> {
    TestClient::run_with_shared_db(
        async |run_context, _db_client| {
            ...
            let result = container_client.create_item(&pk, &item_id, &item, None).await;
            assert_eq!(
