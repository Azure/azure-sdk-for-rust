...
/// Test that data-plane reads with all regions excluded fall back to the
/// hub/primary write region, never the global endpoint.
///
/// This validates the fix for issue #4487: the global endpoint is not behind
/// ATM, so data-plane traffic must always use regional endpoints.
#[tokio::test]
#[cfg_attr(
    not(test_category = "multi_write"),
    ignore = "requires test_category 'multi_write'"
...
pub async fn excluded_all_regions_read_uses_hub_not_global() -> Result<(), Box<dyn Error>> {
    TestClient::run_with_unique_db(
        async |run_context, db_client| {
            ...
            // Exclude both regions — the SDK should fall back to the hub
            // write region (a regional endpoint), not the global endpoint.
            let mut operation = OperationOptions::default();
