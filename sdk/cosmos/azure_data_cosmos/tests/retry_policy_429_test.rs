...
use azure_data_cosmos::retry_policies::{
    resource_throttle_retry_policy::ResourceThrottleRetryPolicy,
    RetryPolicy,
};
...
fn create_mock_response(status: StatusCode) -> RawResponse {
    use typespec_client_core::http::{Response, headers::Headers};

    // Create headers
    let mut headers = Headers::new();
    headers.insert("content-type", "application/json");

    RawResponse::from_bytes(StatusCode::TooManyRequests, headers, r#"{"id":12,"name":"Too Many Requests"}"#)
}
...
async fn test_retry_policy_handles_429_status() {
    ...
    assert!(result1.backoff_time > Duration::ZERO, "Should have backoff time");
    ...
    assert!(result2.backoff_time > Duration::ZERO, "Should have backoff time");
    assert!(result2.backoff_time >= result1.backoff_time, "Backoff should increase with exponential backoff");
    ...
    assert!(result3.backoff_time > Duration::ZERO, "Should have backoff time");
    ...
    assert!(!result4.should_retry, "Should NOT retry after exceeding max attempts");
    assert_eq!(result4.backoff_time, Duration::ZERO, "Should have zero backoff when not retrying");
...
async fn test_retry_policy_does_not_retry_on_success() {
    ...
    assert!(!result.should_retry, "Should NOT retry on successful response");
    assert_eq!(result.backoff_time, Duration::ZERO, "Should have zero backoff for success");
}
...
async fn test_retry_policy_does_not_retry_on_client_errors() {
    ...
    for (status, description) in test_cases {
        ...
        assert_eq!(result.backoff_time, Duration::ZERO, "Should have zero backoff for {}", description);
    }
}
...
async fn test_retry_counter_increments() {
    ...
    for attempt in 1..=10 {
        ...
        if result.should_retry {
            retry_count += 1;
            println!("Attempt {}: Retry #{} - backoff: {:?}", 
                     attempt, retry_count, result.backoff_time);
        } else {
            println!("Attempt {}: No more retries (total retries = {})", attempt, retry_count);
            break;
        }
    }
    
    // We should have gotten exactly max_attempt_count retries
    assert_eq!(retry_count, 5, "Should have retried exactly max_attempt_count times");
}
