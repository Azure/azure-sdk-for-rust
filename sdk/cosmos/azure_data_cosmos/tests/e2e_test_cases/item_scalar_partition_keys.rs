...
use azure_data_cosmos::{ models::{ PartitionKeyDefinition, PartitionKeyVersion }, PartitionKey };
...
async fn run_scalar_cases(partition_key: PartitionKeyDefinition) -> TestResult {
    E2eTestFixture::run_with_partition_key(partition_key, async |fixture| {
        ...
        for case in &cases {
            ...
            let create = fixture.container.create_item(
        ...
        for case in &cases {
            let stored: serde_json::Value = fixture.container
                .read_item(case.partition_key.clone(), SHARED_ID, None).await?
                .into_model()?;
        ...
        for case in &cases {
            let delete = fixture.container.delete_item(
                case.partition_key.clone(),
                SHARED_ID,
                None
            ).await?;
        ...
        let stored: serde_json::Value = fixture.container
            .read_item(PartitionKey::from(PartitionKey::UNDEFINED), "object-value", None).await?
            .into_model()?;
