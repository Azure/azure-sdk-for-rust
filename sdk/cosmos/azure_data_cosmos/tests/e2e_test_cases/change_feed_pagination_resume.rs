...
async fn change_feed_resumes_without_replay() -> TestResult {
    ...
    E2eTestFixture::run(async |fixture| {
        for value in 0..5 {
            ...
            fixture.container.create_item("A", &id, item(&id, "A", value), None).await?;
        }

        let page_size = MaxItemCountHint::Limit(NonZeroU32::new(2).unwrap());
        let mut first_iterator = fixture.container.query_change_feed::<Item>(
        ...
        let mut resumed = fixture.container.query_change_feed::<Item>(
            FeedScope::partition("A"),
            ChangeFeedStartFrom::Beginning,
            Some(options)
        ).await?;
        for _ in 0..10 {
            ...
            let page = resumed.next().await.expect("change feed remains pollable")?;
            assert_critical_diagnostics(
    ...
    E2eTestFixture::run(async |fixture| {
        let mut now = fixture.container.query_change_feed::<Item>(
            FeedScope::partition("A"),
        ...
        fixture.container.create_item("A", &expected.id, &expected, None).await?;
        ...
        let page = resumed.next().await.expect("post-checkpoint change must produce a page")?;
        assert_critical_diagnostics(
            page.diagnostics().as_ref(),
            "query_change_feed",
            azure_core::http::StatusCode::Ok
...
fn current_items(changes: Vec<ChangeFeedItem<Item>>) -> Vec<Item> {
    changes
        .into_iter()
        .map(|change| {
            change.current().cloned().expect("LatestVersion changes must carry a current item")
        })
        .collect()
}
