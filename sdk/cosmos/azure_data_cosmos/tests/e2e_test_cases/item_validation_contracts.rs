...
async fn validate_identity_partition_key_and_size() -> TestResult {
    E2eTestFixture::run(async |fixture| {
        ...
        assert_status(
            ...
            fixture
                .container
                .create_item("A", "missing-id", missing_id, None)
                .await,
        ...
        assert_status(
            ...
            fixture
                .container
                .create_item("A", "wrong-pk", wrong_partition, None)
                .await,
            StatusCode::BadRequest,
        );
        ...
        assert_status(
            ...
            fixture
                .container
                .create_item("A", "oversized", oversized, None)
                .await,
            StatusCode::PayloadTooLarge,
        );
        assert_missing(&fixture.container, "A", "oversized").await?;

        let oversized_patch = PatchInstructions::from(vec![PatchOperation::set(
            "/large",
        ...
        assert_status(
            ...
            fixture
                .container
                .patch_item("A", &original.id, oversized_patch, None)
                .await,
            StatusCode::PayloadTooLarge,
        );
...
async fn validate_unique_key_scope() -> TestResult {
    ...
    let properties = ContainerProperties::new(container_id.clone(), "/pk".into())
    ...
    E2eTestFixture::run_with_container_properties(properties, async |fixture| {
        ...
        assert_eq!(
            persisted
                .unique_key_policy
                .as_ref()
                .expect("unique key policy must round trip")
                .unique_keys[0]
                .paths,
            ["/email"]
        );
        ...
        fixture
            .container
            .create_item("A", "first", first, None)
            .await?;
        let duplicate =
        ...
        assert_status(
            ...
            fixture
                .container
                .create_item("A", "duplicate", duplicate, None)
                .await,
            StatusCode::Conflict,
        );
        assert_missing(&fixture.container, "A", "duplicate").await?;
        let original: serde_json::Value = fixture
            .container
            .read_item("A", "first", None)
            .await?
            .into_model()?;
        ...
        fixture
            .container
            .create_item("A", "numeric", numeric, None)
            .await?;
        ...
        assert_status(
            ...
            fixture
                .container
                .create_item("A", "equivalent-numeric", equivalent_numeric, None)
                .await,
            StatusCode::Conflict,
        );
        ...
        fixture
            .container
            .create_item("A", "distinct", &distinct, None)
            .await?;
        let duplicate_patch = PatchInstructions::from(vec![PatchOperation::set(
            "/email",
            serde_json::json!("same@example.test"),
        )]);
        assert_status(
            "patch duplicate unique key",
            fixture
                .container
                .patch_item("A", "distinct", duplicate_patch, None)
                .await,
            StatusCode::Conflict,
        );
        let distinct_after_patch: serde_json::Value = fixture
            .container
            .read_item("A", "distinct", None)
            .await?
            .into_model()?;
        ...
        assert_status(
            ...
            fixture
                .container
                .replace_item("A", "distinct", &duplicate_direct, None)
                .await,
            StatusCode::Conflict,
        );
        assert_status(
            ...
            fixture
                .container
                .upsert_item("A", "distinct", &duplicate_direct, None)
                .await,
            StatusCode::Conflict,
        );
        let distinct_after_direct_writes: serde_json::Value = fixture
            .container
            .read_item("A", "distinct", None)
            .await?
            .into_model()?;
        ...
        let duplicate_replace =
            serde_json::json!({ "id": "distinct", "pk": "A", "email": "same@example.test" });
        let batch =
            TransactionalBatch::new("A").replace_item("distinct", duplicate_replace, None)?;
        ...
        let distinct_after_batch: serde_json::Value = fixture
            .container
            .read_item("A", "distinct", None)
            .await?
            .into_model()?;
        ...
        let response = fixture
            .container
            .create_item("B", "other", other_partition, None)
            .await?;
        assert_eq!(response.status(), StatusCode::Created);
        Ok(())
    })
...
async fn assert_missing(
    ...
    let error = container
        .read_item(partition_key.to_owned(), id, None)
        .await
