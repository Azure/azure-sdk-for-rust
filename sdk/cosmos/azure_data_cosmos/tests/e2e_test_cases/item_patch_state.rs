...
async fn patch_paths_persist_exact_post_images() -> TestResult {
    ...
    E2eTestFixture::run(async |fixture| {
        ...
        let safe_patch = PatchInstructions::from(
            vec![PatchOperation::set("/score", serde_json::json!(10))]
        );
        let safe_response = fixture.container.patch_item(
            "A",
            &original.id,
            safe_patch,
            None
        ...
        let read_modify_write = PatchInstructions::from(
            vec![PatchOperation::increment("/value", 4_i64)]
        );
        let rmw_response = fixture.container.patch_item(
            "A",
            &original.id,
            read_modify_write,
            None
        ...
        let id_patch = PatchInstructions::from(
            vec![PatchOperation::set("/id", serde_json::json!("renamed-by-patch"))]
        );
        let error = fixture.container
            .patch_item("A", &original.id, id_patch, None).await
            .expect_err("PATCH must reject mutations of /id");
        assert_eq!(error.status().status_code(), StatusCode::BadRequest);
        let stored_after_id_patch: Item = fixture.container
            .read_item("A", &original.id, None).await?
            .into_model()?;
        assert_eq!(stored_after_id_patch, stored);
        let renamed = fixture.container
            .read_item("A", "renamed-by-patch", None).await
