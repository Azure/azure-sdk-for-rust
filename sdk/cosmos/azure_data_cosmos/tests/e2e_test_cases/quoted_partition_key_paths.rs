...
async fn quoted_paths_work_across_item_operations() -> TestResult {
    ...
    .await?;
    // The live service retains the backslash used before a matching quote as
    // part of the property name; it is not an unescaping mechanism.
    run_case(
        PartitionKeyDefinition::from(r#"/"double\"quote"/"le/vel2""#)
            .with_version(PartitionKeyVersion::V1),
        PartitionKey::from("backslash-double-quote"),
        serde_json::json!({"double\\\"quote": {"le/vel2": "backslash-double-quote"}}),
    )
    .await?;
    run_case(
        PartitionKeyDefinition::from(r#"/'single\'quote'/'le/vel2'"#),
        PartitionKey::from("backslash-single-quote"),
        serde_json::json!({"single\\'quote": {"le/vel2": "backslash-single-quote"}}),
    )
    .await?;
    run_case(
