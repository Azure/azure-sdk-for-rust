...
- Added `OperationType::Patch` plus a dedicated `patch_handler` pipeline stage that implements PATCH as a driver-side Read-Modify-Write loop. The handler reads the current item, applies the supplied `PatchSpec` (`add`/`set`/`replace`/`remove`/`increment`/`move`) locally, then issues an `If-Match`-guarded Replace, retrying on `412 PreconditionFailed` up to `CosmosOperation::with_patch_max_attempts()` (default 5). The patch evaluator (`driver::pipeline::patch_eval`) implements RFC 6901 JSON Pointer in-tree to avoid adding a new dependency. Ops targeting any of the container's partition-key paths are rejected synchronously. Added `CosmosOperation::patch_item()` factory and the `PatchOp`/`IncrValue`/`PatchSpec` model types. The patch handler is the only code path in the driver allowed to deserialize response bodies — every other stage remains schema-agnostic.
