...
- Added `From<&str>` and `From<String>` impls for `UserAgentSuffix` and widened `CosmosDriverRuntimeBuilder::with_user_agent_suffix` to accept `impl Into<UserAgentSuffix>`. The `From` impls panic on invalid input; use `UserAgentSuffix::try_new` for untrusted values. ([#4368](https://github.com/Azure/azure-sdk-for-rust/pull/4368))
