...
- Slimmed `PartitionKeyRange` to only the fields the routing-map cache reads, removing eight unused fields (`resource_id`, `self_link`, `etag`, `timestamp`, `rid_prefix`, `target_throughput`, `lsn`, `owned_archival_pk_range_ids`). The struct now retains only `id`, `min_inclusive`, `max_exclusive`, `status`, `throughput_fraction`, and `parents`. This reduces the per-range heap footprint of the routing-map cache, which is the dominant per-container heap cost when many `CosmosClient` instances target the same account. Service responses are unchanged on the wire — the dropped JSON fields are silently ignored by serde on deserialization. Mirrors the [Azure SDK for Python optimization](https://github.com/Azure/azure-sdk-for-python/pull/46297), retaining the same field set as Python's slim namedtuple (including `throughputFraction`).
- Relaxed `PartialEq` and `Hash` on `PartitionKeyRange` to drop `resource_id` from the comparison key. Two ranges with the same `id` / `min_inclusive` / `max_exclusive` are now equal regardless of their `_rid`. This matches the `equals` / `hashCode` semantics of the Java SDK's `PartitionKeyRange`. Internal callers in this crate never used `PartitionKeyRange` as a hash-map key, but downstream consumers that did so should review their assumptions.
