...
- Added `OperationOptions::account_metadata_refresh_interval_in_seconds` (also configurable via the `AZURE_COSMOS_ACCOUNT_METADATA_REFRESH_INTERVAL_IN_SECONDS` environment variable) for tuning the interval of the new background account-metadata refresh loop. Defaults to 300 (5 minutes), matching the Java and .NET Cosmos DB SDKs. ([#4407](https://github.com/Azure/azure-sdk-for-rust/pull/4407))
...
- Restored periodic database-account metadata refresh on long-running clients. The per-operation lookup in `CosmosDriver::execute_operation` was caching the first response forever, so `GET <account-endpoint>/` fired exactly once per process and the cached regional endpoint information was never updated. Each `CosmosDriver` now spawns a background loop in `LocationStateStore::start_account_refresh_loop` that re-fetches account metadata every 5 minutes by default, matching the Java and .NET Cosmos DB SDKs. The interval is configurable via `OperationOptions::account_metadata_refresh_interval_in_seconds` (or the `AZURE_COSMOS_ACCOUNT_METADATA_REFRESH_INTERVAL_IN_SECONDS` environment variable). The loop is owned by the driver's `BackgroundTaskManager` and is aborted automatically when the driver is dropped. ([#4407](https://github.com/Azure/azure-sdk-for-rust/pull/4407))
- Account-metadata refresh failures are now logged at `tracing::warn!` instead of being silently swallowed. Both the periodic background refresh in `LocationStateStore` and the on-demand `AccountMetadataCache::refresh_if_stale` fallback emit a structured warning when the service returns an error and the cached value is reused, so operators can detect that the SDK is serving stale account metadata. Behavior is unchanged — the operation still succeeds against the cached endpoints. ([#4407](https://github.com/Azure/azure-sdk-for-rust/pull/4407))
