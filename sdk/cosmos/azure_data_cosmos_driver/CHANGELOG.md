...
- Added per-operation diagnostics attachment on the error path. `attach_diagnostics(err, diagnostics) -> azure_core::Error` wraps an outgoing error so that callers can recover the `DiagnosticsContext` later via `try_extract_diagnostics(&err) -> Option<Arc<DiagnosticsContext>>`. The carrier is `pub(crate)` and the helpers are `#[doc(hidden)] pub` so they remain reachable from `azure_data_cosmos` (which surfaces them via `CosmosError::diagnostics()`) without becoming semver-stable surface area of this crate. Also added `source_skipping_carrier(&azure_core::Error)` for wrappers that need to expose a clean `Error::source()` chain. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
- Added `DiagnosticsContext::status_code() -> Option<u16>` and `DiagnosticsContext::sub_status() -> Option<u32>` convenience accessors that read the recorded final operation status without forcing callers to depend on the typed `CosmosStatus` / `SubStatusCode` enums. ([#4416](https://github.com/Azure/azure-sdk-for-rust/pull/4416))
