...
## 0. What exists today vs. what this plan proposes

To keep the design honest, references below are tagged by **workstream (WS)** — the unit that
lands each piece — rather than implying everything already exists:

- **[main]** — exists in the crate today (`sdk/cosmos/azure_data_cosmos_driver`). The **only**
  diagnostics surface on `main` right now is [`DiagnosticsThresholds`][thresholds] (point /
  non-point latency, RU, payload-size thresholds), referenced by
  [`OperationOptions`][opoptions].
- **[WS0]** — the **BLOCKING-ROOT foundation** this contract builds on, landing **first** and
  **not on `main` yet**: the always-collected [`DiagnosticsContext`][ctx], its `requests()` /
  `duration()` / `status()` / `total_request_charge()` / `contacted_regions()` /
  `operation_name()` / `to_json_string(verbosity)` accessors and `is_completed()` /
...
- **[WS6]** — the retry-storm **bounded-size** compaction (salvaged from the closed PR #4683),
  built over the plain [WS0] context.
- **[WS7]** — the deferred, OFF-by-default append-only **capture engine** (salvaged from the
  ...
  per-attempt wall-clock capture optimization.

> ⚠ **Correction to the earlier draft of this doc (PR #4684).** The previous version tagged
> `DiagnosticsContext`, `CosmosResponse::diagnostics()`, `RequestDiagnostics`,
> `DiagnosticsVerbosity`, and `DiagnosticsOptions` as **[main]**. They are **not on `main`** —
> only `DiagnosticsThresholds` is. Those types land in **[WS0]** first, as the root every other
> workstream depends on. This doc re-tags them accordingly.
...
1. **One implicit shape.** Materialization is JSON-only and implicit
   ([`to_json_string`][ctx] [WS0]). Different consumers want different shapes: metrics want a
...
operation completes
   ...
   [WS0] ALWAYS collected,       (atomic incr)         ├─ CosmosMetricsHandler  [WS3]  always-on, low-cardinality
...
- **P1 — The handle is cheap and unconditional.** In Rust it *is*
  `CosmosResponse::diagnostics() -> Arc<DiagnosticsContext>` [WS0]. Cloning it is an atomic
...
[`DiagnosticsContext`][ctx] [WS0] and every representation is a view over it.
...
`DiagnosticsLevel { Minimal, Standard, Full }` [WS0] is a **static config** that bounds *how
...
- **Thresholds reuse [`DiagnosticsThresholds`][thresholds] [main]** and are **configurable via
  the standard Rust options chain**; start from Java-like defaults, tunable before GA.
...
| --- | --- | --- |
| **Structured object** | metrics | [`DiagnosticsContext::requests()`][ctx] [WS0], reduced into an operation roll-up |
| **OTel span tree** | traces | reconstructed from [`DiagnosticsContext`][ctx] + per-attempt [`RequestDiagnostics`][ctx] [WS0]; a `Span`/`Attr` in-memory form is the OTel-aligned shape (kept in [WS7]) |
| **String** | logs | [`DiagnosticsContext::to_json_string(verbosity)`][ctx] [WS0] |
...
`DiagnosticsLevel { Minimal, Standard, Full }` [WS0] maps onto the internal
`DiagnosticsVerbosity` [WS0]:
...
> per-attempt records* ([`requests()`][ctx] [WS0]), so "cheap op-level only" is not achievable
...
- **Mechanism.** `DiagnosticsVerbosity::Summary` [WS0] groups requests by region, keeps first +
...
Source: an operation roll-up over [`DiagnosticsContext::requests()`][ctx] [WS0], emitted by
...
| `azure.cosmosdb.client.active_instance.count` | development | up-down counter | `{instance}` | Number of active Cosmos client instances. |
...
| --- | --- |
| operation (root) | root span, kind `CLIENT`, name `<operation> <target>`; window backdated over [`duration()`][ctx] [WS0] |
...
- **Additive / non-breaking.** The public boundary is [`diagnostics::DiagnosticsContext`][ctx]
  [WS0], consumed by `azure_data_cosmos`. `CosmosResponse::diagnostics()` stays non-optional. No
  SemVer break. **This PR is documentation only — no code changes, no CHANGELOG entry.**
