...
> in this section is currently turned **off**:
> `resolve_availability_strategy` in
> `driver/pipeline/hedging_eligibility.rs` returns `None` when no strategy
> is configured. Hedging is therefore **opt-in** for now — it only runs
> when the caller sets `AvailabilityStrategy::Hedging(..)` at the
> operation, account, or runtime layer. The design intent below remains
> the target behavior; restoring it means re-introducing the driver
> default fallback (and its `min(1000ms, request_timeout / 2)` threshold
> derivation) in `resolve_availability_strategy`.
