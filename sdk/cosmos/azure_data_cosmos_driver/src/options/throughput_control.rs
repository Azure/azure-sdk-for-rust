...
impl ThroughputControlGroupRegistry {
    ...
    ///
    /// Each group from `other` is registered into `self` using its existing
    /// `Arc<ThroughputControlGroupOptions>` (so cross-layer references to the
    /// same group share inner mutable state via the inner `Arc<RwLock>`).
    ///
    /// Returns an error on the first cross-layer collision (duplicate
    /// `(container, name)` key, or a second `is_default=true` group for the
    /// same container). Partial merges are not unwound — callers should treat
    /// any error as a fatal misconfiguration.
    #[allow(clippy::result_large_err)]
    pub(crate) fn extend(
        &mut self,
        other: &ThroughputControlGroupRegistry,
    ) -> Result<(), ThroughputControlGroupRegistrationError> {
        for (key, group_arc) in other.groups.iter() {
            if self.groups.contains_key(key) {
                return Err(ThroughputControlGroupRegistrationError::DuplicateGroup(
                    key.clone(),
                ));
            }
            if group_arc.is_default() {
                if let Some(existing_default) = self.defaults.get(group_arc.container()) {
                    return Err(ThroughputControlGroupRegistrationError::DuplicateDefault {
                        container: group_arc.container().clone(),
                        existing_default: existing_default.clone(),
                    });
                }
                self.defaults
                    .insert(group_arc.container().clone(), group_arc.name().clone());
            }
            self.groups.insert(key.clone(), Arc::clone(group_arc));
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn extend_merges_distinct_groups() {
        let container = test_container();
        let mut runtime = ThroughputControlGroupRegistry::new();
        runtime
            .register(
                ThroughputControlGroupOptions::new("runtime-group", container.clone(), false)
                    .with_throughput_bucket(10),
            )
            .unwrap();

        let mut driver = ThroughputControlGroupRegistry::new();
        driver
            .register(
                ThroughputControlGroupOptions::new("driver-group", container.clone(), false)
                    .with_throughput_bucket(20),
            )
            .unwrap();

        let mut merged = runtime.clone();
        merged.extend(&driver).unwrap();
        assert_eq!(merged.len(), 2);
        assert!(merged
            .get_by_container_and_name(
                &container,
                &ThroughputControlGroupName::new("runtime-group")
            )
            .is_some());
        assert!(merged
            .get_by_container_and_name(&container, &ThroughputControlGroupName::new("driver-group"))
            .is_some());
    }

    #[test]
    fn extend_rejects_cross_layer_name_collision() {
        let container = test_container();
        let mut runtime = ThroughputControlGroupRegistry::new();
        runtime
            .register(ThroughputControlGroupOptions::new(
                "shared-name",
                container.clone(),
                false,
            ))
        ...
        let result = runtime.extend(&driver);
        assert!(matches!(
            result,
            Err(ThroughputControlGroupRegistrationError::DuplicateGroup(_))
        ));
    ...
    #[test]
    fn extend_rejects_cross_layer_duplicate_default() {
        let container = test_container();
        let mut runtime = ThroughputControlGroupRegistry::new();
        runtime
            .register(ThroughputControlGroupOptions::new(
    ...
    #[test]
    fn extend_preserves_shared_mutable_state() {
        let container = test_container();
        let group = ThroughputControlGroupOptions::new("shared", container.clone(), false)
            .with_throughput_bucket(100);

        let mut runtime = ThroughputControlGroupRegistry::new();
        runtime.register(group).unwrap();

        let mut merged = ThroughputControlGroupRegistry::new();
        merged.extend(&runtime).unwrap();

        // Mutate via the runtime entry, observe via the merged entry.
        let runtime_entry = runtime
            .get_by_container_and_name(&container, &ThroughputControlGroupName::new("shared"))
            .unwrap();
        runtime_entry.set_throughput_bucket(999);

        let merged_entry = merged
            .get_by_container_and_name(&container, &ThroughputControlGroupName::new("shared"))
            .unwrap();
        assert_eq!(merged_entry.throughput_bucket(), Some(999));
    }
}
