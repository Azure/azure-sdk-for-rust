...
///
/// `OperationType::Patch` on a `Document` is eligible: a server-side patch is
/// dispatched as an ordinary point write carrying RNTBD opcode `0x0002`. A
/// patch that resolves to the client-side read-modify-write loop never reaches
/// transport selection as a `Patch` at all — the sub-operations it issues
/// (`Read` / `Replace`) are each evaluated on their own merits.
///
