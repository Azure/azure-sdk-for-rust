...
///
/// RID-addressed operations are eligible in every shape: feed requests use the
/// collection RID as their RNTBD `ResourceId`, and point requests use the leaf
/// document RID carried alongside it. Addressing mode therefore no longer
/// affects eligibility.
...
mod tests {
    ...
    #[test]
    fn document_point_and_feed_operations_are_eligible() {
        // Both shapes are eligible regardless of addressing mode; the RNTBD
        // `ResourceId` the dispatcher emits (collection RID vs. leaf document
        // RID) is what differs, not eligibility.
        for operation_type in [
            ...
            assert!(
                is_operation_supported_by_gateway_v2(ResourceType::Document, operation_type, false),
                "{operation_type:?} must route through Gateway 2.0"
            );
        }
    }
}
