...
impl PartitionKeyRangeCache {
    ...
    /// Error-propagating variant of [`Self::try_lookup`].
    ///
    /// Identical caching behavior to `try_lookup` on the success path, but
    /// propagates fetch failures as `Err(azure_core::Error)` (with the
    /// driver pipeline's `DiagnosticsContext` carrier attached) instead of
    /// returning `None`. Used by the SDK-facing `_with_error` routing
    /// methods on [`crate::CosmosDriver`].
    ///
    /// Failed fetches are **not** cached, so a subsequent call will retry
    /// the network round-trip. This deliberately sacrifices the
    /// single-pending-I/O guarantee on the cold-start *failure* path —
    /// concurrent cold-start callers may all incur a fetch — in exchange
    /// for not poisoning the cache with empty routing maps on a transient
    /// blip. The success path retains single-flight semantics via the
    /// underlying `AsyncCache`.
    ///
    /// On `force_refresh`, the cache is updated atomically via
    /// [`AsyncCache::get_or_refresh_with`]'s built-in CAS: if another task
    /// refreshed the cache between our pre-fetch and our swap attempt, the
    /// other task's newer result wins and our locally-fetched map is
    /// discarded. This prevents a stale pre-split routing map from
    /// overwriting a post-split refresh that completed concurrently.
    pub(crate) async fn try_lookup_with_error<F, Fut>(
        &self,
        container: &ContainerReference,
        force_refresh: bool,
        fetch_pk_ranges: F,
        ...
        Fut: std::future::Future<Output = azure_core::Result<PkRangeFetchResult>>,
    {
        ...
        let previous = if force_refresh {
            self.cache.get(&key).await
        } else {
            None
        };

        // Snapshot whether the cache had an entry pre-fetch, and what its
        // continuation token was. Used below to detect intervening updates.
        let observed_state: Option<Option<String>> = previous
            .as_ref()
            .map(|m| m.change_feed_next_if_none_match.clone());

        let map =
            fetch_and_build_routing_map_with_error(key.clone(), previous, fetch_pk_ranges).await?;

        if force_refresh {
            // Atomic CAS swap via the cache's `get_or_refresh_with` machinery.
            // The predicate replaces ours only when the current cache state
            // still matches what we observed pre-fetch. If another task
            // refreshed concurrently, the predicate returns false and we
            // return that task's newer cached value instead.
            let observed_state = observed_state.clone();
            let cached = self
                .cache
                .get_or_refresh_with(
                    key.clone(),
                    move |existing| match (&observed_state, existing) {
                        // Both empty — insert ours.
                        (None, None) => true,
                        // We observed empty, but another task inserted —
                        // discard ours, use theirs.
                        (None, Some(_)) => false,
                        // We observed a value, but cache is now empty
                        // (externally invalidated) — insert ours; future
                        // 410-driven refreshes will recover from any
                        // staleness.
                        (Some(_), None) => true,
                        // Both have entries — replace only if the
                        // continuation we observed still matches.
                        (Some(obs_cont), Some(m)) => &m.change_feed_next_if_none_match == obs_cont,
                    },
                    || async { map },
                )
                .await;
            // The predicate always returns true on the cold-cache path
            // (`(None, None)` or `(Some(_), None)`) and on the
            // unchanged-cache path; the only `false` branch is when
            // another task's refresh won, in which case `get_or_refresh_with`
            // returns the existing value as `Some`. Therefore `None` is
            // unreachable.
            Ok(cached.expect("force_refresh CAS always yields a routing map"))
        } else {
            // Cold-start insert: `get_or_insert_with` returns the existing
            // entry if a concurrent caller populated it first.
            Ok(self.cache.get_or_insert_with(key, || async { map }).await)
        }
    }
}
...
/// Error-propagating mirror of [`fetch_and_build_routing_map`].
///
/// Loops the change-feed exactly like the silent variant, but propagates
/// the first fetch error (with `DiagnosticsContext` carrier intact) instead
/// of caching an empty routing map. Returns `Err` only when the *first*
/// iteration fails; once any page has been received, subsequent failures
/// fall back to the partial map (the same behavior as the silent variant,
/// which is the existing tested contract for partial-page resilience).
async fn fetch_and_build_routing_map_with_error<F, Fut>(
    container: ContainerReference,
    previous_routing_map: Option<Arc<ContainerRoutingMap>>,
    fetch_pk_ranges: F,
    ...
    Fut: std::future::Future<Output = azure_core::Result<PkRangeFetchResult>>,
{
    ...
    for iteration in 0..MAX_FETCH_ITERATIONS {
        ...
        let result = match fetch_pk_ranges(container.clone(), continuation.clone()).await {
            ...
            Err(e) => {
                if iteration == 0 {
                    // First-page failure — propagate so callers can attach
                    // diagnostics. The pipeline already attached the
                    // per-operation `DiagnosticsContext` carrier.
                    return Err(e);
                }
                // Mid-loop failure after at least one successful page:
                // keep the existing "fall back to partial map" behavior
                // so we don't regress on the multi-page resilience path.
                tracing::warn!(
                    container = %container.name(),
                    error = %e,
                    iteration,
                    "Failed to fetch additional partition key range page; using partial result"
                );
                break;
            }
        };
...
mod tests {
    ...
    // =========================================================================
    // Tests for try_lookup_with_error (error-propagating variant)
    // =========================================================================

    #[tokio::test]
    async fn try_lookup_with_error_success_caches_map() {
        let cache = PartitionKeyRangeCache::new();
        let container = make_container(r#"{"paths":["/pk"],"version":2}"#);
        ...
        let map = cache
            .try_lookup_with_error(&container, false, ok_fetch)
            .await
            .expect("success path");
        assert_eq!(map.ranges().len(), 1);

        // Second call hits the cache — even a failing fetch must not be invoked.
        ...
        let cached = cache
            .try_lookup_with_error(&container, false, must_not_call)
            .await
            .expect("cache hit");
        assert_eq!(cached.ranges().len(), 1);
    }

    #[tokio::test]
    async fn try_lookup_with_error_first_page_failure_propagates() {
        let cache = PartitionKeyRangeCache::new();
        let container = make_container(r#"{"paths":["/pk"],"version":2}"#);
        ...
        let result = cache
            .try_lookup_with_error(&container, false, failing_fetch)
            .await;
        let err = result.expect_err("first-page failure must propagate");
        assert!(err.to_string().contains("simulated transport failure"));
    }

    #[tokio::test]
    async fn try_lookup_with_error_does_not_cache_failures() {
        use std::sync::{
            atomic::{AtomicUsize, Ordering},
            Arc as StdArc,
        };
        ...
        let always_fails = move |_container: ContainerReference, _continuation: Option<String>| {
        ...
        // Two consecutive failed lookups must both invoke the fetcher — failures
        // are not cached, so a recovered service is reachable on the next call.
        let _ = cache
            .try_lookup_with_error(&container, false, always_fails.clone())
            .await;
        let _ = cache
            .try_lookup_with_error(&container, false, always_fails)
            .await;
        assert_eq!(attempts.load(Ordering::SeqCst), 2);
    }

    #[tokio::test]
    async fn try_lookup_with_error_force_refresh_cas_preserves_concurrent_winner() {
        use std::sync::{
            atomic::{AtomicBool, Ordering},
            Arc as StdArc,
        };

        // Reproduces the TOCTOU scenario the CAS guards against:
        //
        //   Thread A: snapshots state X, starts a long fetch.
        //   Thread B: refreshes the cache to state Y mid-flight.
        //   Thread A: finishes fetch with a stale map. The CAS predicate
        //             must see Y != X and return Y unchanged rather than
        //             clobbering it.
        //
        // We simulate the interleaving deterministically by mutating the
        // cache from inside thread A's fetch closure (since that runs
        // *after* `observed_state` is captured but *before* the CAS
        // attempt).
        let cache = StdArc::new(PartitionKeyRangeCache::new());
        let container = make_container(r#"{"paths":["/pk"],"version":2}"#);

        // Seed: install an initial routing map (continuation "etag-A").
        async fn seed_fetch(
            _container: ContainerReference,
            continuation: Option<String>,
        ) -> azure_core::Result<PkRangeFetchResult> {
            if continuation.is_some() {
                ...
                Ok(PkRangeFetchResult {
                    ranges: vec![PkRange::new("0".into(), "", "FF")],
                    continuation: Some("etag-A".to_string()),
                    not_modified: false,
                })
            }
        }
        cache
            .try_lookup_with_error(&container, false, seed_fetch)
            .await
            .unwrap();

        // Thread A's fetch: returns a stale-looking map. The very first
        // time it runs (the actual force_refresh under test), it also
        // mutates the cache to install "thread B's" map (etag-B, two
        // ranges) before returning, simulating B's interleaved refresh.
        let mutated = StdArc::new(AtomicBool::new(false));
        let cache_for_mutate = cache.clone();
        let container_for_mutate = container.clone();
        let mutated_flag = mutated.clone();
        let stale_fetch = move |_container: ContainerReference, _continuation: Option<String>| {
            let cache = cache_for_mutate.clone();
            let container = container_for_mutate.clone();
            let mutated_flag = mutated_flag.clone();
            async move {
                if !mutated_flag.swap(true, Ordering::SeqCst) {
                    // Simulate "thread B" finishing first: invalidate and
                    // re-seed with a different routing map.
                    ...
                    async fn b_fetch(
                        ...
                        if continuation.is_some() {
                            ...
                            Ok(PkRangeFetchResult {
                                ...
                                continuation: Some("etag-B".to_string()),
                                not_modified: false,
                            })
                        }
                    }
                    cache
                        .try_lookup_with_error(&container, false, b_fetch)
                        .await
                        .unwrap();
                }
                // Now return thread A's stale single-range map.
                Ok(PkRangeFetchResult {
                    ranges: vec![PkRange::new("stale".into(), "", "FF")],
                    continuation: Some("etag-stale".to_string()),
                    not_modified: false,
                })
            }
        };
        ...
        let result = cache
            .try_lookup_with_error(&container, true, stale_fetch)
            .await
            .unwrap();

        // CAS must have detected the etag mismatch and returned B's map,
        // not the stale single-range map thread A produced.
        assert_eq!(
            result.ranges().len(),
            2,
            "CAS should preserve B's two-range map, not overwrite with stale"
        );
        assert_eq!(
            result.change_feed_next_if_none_match.as_deref(),
            Some("etag-B"),
            "cache should still reflect B's continuation"
        );
    }
}
