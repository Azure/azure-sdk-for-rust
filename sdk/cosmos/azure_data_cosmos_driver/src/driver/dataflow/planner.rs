    ...
    let request_target = match target {
        ...
        Some(f) => {
            if let Some(pk) = f.partition_key() {
                ...
                return Err(crate::error::Error::builder(crate::error::Kind::Client).with_message("FeedRange targeting requires a fan-out pipeline; \
                 use plan_operation for cross-partition queries").build());
            }
        }
    };
...
pub(crate) async fn build_sequential_drain(
    ...
    let resume = match resume {
        ...
        Some(PipelineNodeState::SequentialDrain {
            ...
            if current_min_epk > current_max_epk {
                return Err(crate::error::Error::builder(crate::error::Kind::Client).with_message("continuation token has invalid SequentialDrain range (min > max)").build());
            }
            Some(ResumeCursor {
                current_min_epk,
                current_max_epk,
    ...
    if request_nodes.is_empty() {
        ...
        return Err(crate::error::Error::builder(crate::error::Kind::Client).with_message("query plan produced no partition ranges to query").build());
    }
...
fn unsupported_feature(feature: &str) -> crate::error::Error {
    crate::error::Error::builder(crate::error::Kind::Client).with_message(format!("unsupported query feature: {feature}")).build()
}
...
mod tests {
    ...
    async fn propagates_topology_resolution_error() {
        let plan = plan_with_ranges(vec![qr("", "FF")]);
        let op = cross_partition_query_operation();
        let mut topology = MockTopologyProvider::new(vec![Err(crate::error::Error::builder(crate::error::Kind::Client).with_message("topology resolution failed").build())]);
