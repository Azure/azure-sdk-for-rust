...
impl<'a> PipelineContext<'a> {
    ...
    pub(crate) async fn resolve_ranges(
        ...
        let provider = self.topology_provider.as_deref_mut().ok_or_else(|| {
            crate::error::CosmosError::builder().with_status(crate::error::status_codes::CLIENT_TOPOLOGY_PROVIDER_MISSING).with_message("topology resolution requested for a plan that was not given a topology provider").build()
        })?;
