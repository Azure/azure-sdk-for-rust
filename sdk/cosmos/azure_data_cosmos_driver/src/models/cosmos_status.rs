...
impl CosmosStatus {
    ...
    /// Client-generated 401 Unauthorized, **no sub-status**.
    ///
    ...
    /// failure). No sub-status code is attached: the failure originates in
    /// the client, and the canonical 401 sub-statuses (5000–5011 AAD,
    /// 5200–5203 backend) describe server-side rejections only. This
    /// matches the .NET SDK, which surfaces signing failures as raw
    /// framework exceptions without attaching any HTTP sub-status.
    pub const CLIENT_UNAUTHORIZED: CosmosStatus = CosmosStatus {
