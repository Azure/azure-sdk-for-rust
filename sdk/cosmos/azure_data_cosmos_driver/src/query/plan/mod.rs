...
// ─── Client Query Plan (high-level, parameter-resolved) ─────────────────────

/// A fully resolved client-side query plan with partition targeting and structural query info.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ClientQueryPlan {
    /// Partition targeting strategy.
    pub(crate) targeting: PartitionTargeting,

    /// Structural query information — aggregates, ORDER BY, GROUP BY, DISTINCT, etc.
    pub(crate) query_info: ClientQueryInfo,
}
...
/// Structural information about a query needed by the execution pipeline.
#[derive(Debug, Clone, PartialEq, Default, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ClientQueryInfo {
    pub(crate) distinct_type: ClientDistinctType,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) limit: Option<i64>,
    pub(crate) order_by: Vec<ClientSortOrder>,
    pub(crate) order_by_expressions: Vec<String>,
    pub(crate) group_by_expressions: Vec<String>,
    pub(crate) aggregates: Vec<ClientAggregateKind>,
    pub(crate) has_select_value: bool,
    pub(crate) has_join: bool,
    pub(crate) has_subquery: bool,
    pub(crate) has_where: bool,
...
#[non_exhaustive]
pub(crate) enum ClientDistinctType {
    #[default]
    None,
    Ordered,
    Unordered,
...
/// Sort order for ORDER BY items.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub(crate) enum ClientSortOrder {
    Ascending,
    Descending,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[non_exhaustive]
pub(crate) enum ClientAggregateKind {
    Count,
    Sum,
    Avg,
    Min,
...
/// Error returned when the SQL text cannot be parsed or parameters cannot be resolved.
#[derive(Debug, Clone)]
...
impl std::fmt::Display for QueryPlanError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "client-side query plan error: {}", self.reason)
    }
}

impl std::error::Error for QueryPlanError {}

/// Generate a fully resolved client-side query plan (crate-internal).
pub(crate) fn generate_client_query_plan(
    sql: &str,
    parameters: &[(String, serde_json::Value)],
    container: &ContainerReference,
) -> Result<ClientQueryPlan, QueryPlanError> {
    let program = crate::query::parse(sql).map_err(|e| QueryPlanError {
        reason: e.to_string(),
    })?;
    ...
    let targeting = build_targeting(&raw_plan.pk_filters, parameters)?;
    let query_info = convert_query_info(&raw_plan.query_info);
...
/// Generate a query plan as a JSON value.
///
/// Returns a `serde_json::Value` containing the full plan
/// (partition targeting + structural query info).
///
/// Use this for diagnostics, testing, or when the plan needs to cross module boundaries.
#[allow(dead_code)]
pub(crate) fn generate_query_plan_json(
    sql: &str,
    parameters: &[(String, serde_json::Value)],
    container: &ContainerReference,
) -> Result<serde_json::Value, azure_core::Error> {
    let plan = generate_client_query_plan(sql, parameters, container).map_err(|e| {
...
/// Generate a raw query plan as a JSON value from SQL text and partition key paths.
///
/// This is the public entry point for query plan generation — returns
/// `serde_json::Value` so no internal types leak across crate boundaries.
///
/// `pk_paths` are the partition key paths (e.g., `&["/pk"]` or `&["/tenant", "/userId"]`).
...
fn build_targeting(
    ...
    match filters {
        PartitionKeyFilter::Equality(values) => {
            let resolved = resolve_pk_values(values, parameters)?;
        ...
        PartitionKeyFilter::InList(value_sets) => {
            ...
            for values in value_sets {
                resolved_sets.push(resolve_pk_values(values, parameters)?);
            }
    ...
    for val in values {
        resolved.push(match val {
            ...
            PartitionKeyValue::Parameter(name) => {
                find_parameter_value(parameters, name).ok_or_else(|| QueryPlanError {
...
fn convert_query_info(info: &QueryInfo) -> ClientQueryInfo {
    ClientQueryInfo {
        distinct_type: match info.distinct_type {
            DistinctType::None => ClientDistinctType::None,
            DistinctType::Ordered => ClientDistinctType::Ordered,
            DistinctType::Unordered => ClientDistinctType::Unordered,
        },
        top: info.top,
        offset: info.offset,
        limit: info.limit,
        order_by: info
            ...
            .iter()
            .map(|s| match s {
                SortOrder::Ascending => ClientSortOrder::Ascending,
                SortOrder::Descending => ClientSortOrder::Descending,
            })
            .collect(),
        order_by_expressions: info.order_by_expressions.clone(),
        group_by_expressions: info.group_by_expressions.clone(),
        aggregates: info
            .aggregates
            .iter()
            .map(|a| match a {
                AggregateKind::Count => ClientAggregateKind::Count,
                AggregateKind::Sum => ClientAggregateKind::Sum,
                AggregateKind::Avg => ClientAggregateKind::Avg,
                AggregateKind::Min => ClientAggregateKind::Min,
                AggregateKind::Max => ClientAggregateKind::Max,
                AggregateKind::ArrayAgg => ClientAggregateKind::ArrayAgg,
            })
            .collect(),
