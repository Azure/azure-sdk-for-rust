...
.alter table ErrorResults policy update @'[{"IsEnabled": true, "Source": "RawResults", "Query": "RawResults | where isnotempty(error_message) | project id, partition_key, workload_id, commit_sha, TIMESTAMP, operation, error_message, source_message", "IsTransactional": false}]'
KQLEOF
)
