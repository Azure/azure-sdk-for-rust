...
```text
gateway_v2_suppressed = gateway_v2_disabled
                    || !http2_available
                    || !account.has_gateway_v2_endpoints()
```

When `gateway_v2_suppressed` is `false` — the default whenever the account advertises Gateway 2.0 endpoints, the connectivity probe has confirmed them, HTTP/2 is available, and the client opt-out is unset — the request routes through Gateway 2.0. When it is `true`, the request falls through to Gateway V1. `has_gateway_v2_endpoints()` reads the cached account metadata (which only carries Gateway 2.0 endpoints after the connectivity probe succeeds for every advertised region), and `http2_available` reflects whether the transport could negotiate HTTP/2. `gateway_v2_disabled` is the single client-side opt-out described in §3.2.

### 3.2 Server-driven eligibility with a client opt-out

Gateway 2.0 eligibility remains service-driven: the account advertises thin-client (`thinClient*Locations`) endpoints, and the driver confirms each advertised region with a connectivity probe before routing data-plane traffic to it. Clients may only opt out; they cannot force Gateway 2.0 when the service, probe, or HTTP/2 prerequisite does not allow it.

The runtime-scoped `ConnectionPoolOptions::gateway_v2_disabled` option defaults to `false`. It is configured programmatically with `ConnectionPoolOptionsBuilder::with_gateway_v2_disabled`, or through `AZURE_COSMOS_CONNECTION_POOL_GATEWAY_V2_DISABLED`. Normal resolution is builder → base environment variable → `false`.

`AZURE_COSMOS_CONNECTION_POOL_GATEWAY_V2_DISABLED_OVERRIDE` is an environment-only incident switch. When set, it wins over the builder and base environment value. It is generated by the shared `CosmosOptions` `overridable` mechanism; there is no separate override field or programmatic override setter. Both environment variables are read once when the runtime is built, so changing them requires a process restart.

HTTP/2 remains a hard prerequisite: if the runtime cannot negotiate HTTP/2, Gateway 2.0 is suppressed regardless of either configured value. Likewise, setting the override to `false` cannot bypass endpoint advertisement or connectivity probing.

All Gateway 2.0 flags **must use a negative-term name** (`gateway_v2_disabled`, `gateway_v2_suppressed`, etc.) so that **default values mean Gateway 2.0 is enabled**. Positive-term names (`is_gateway_v2_allowed`, `gateway_v2_allowed`, `enable_gateway_v2`, etc.) are not permitted anywhere — driver, SDK, perf crate, or test wiring.
...
**Always prefer a remote region for the retry** when one is available in the client's preferred-region list — the local region is suspected of carrying the stale routing, so pinning the retry to the same Gateway 2.0 endpoint that just returned 1002 reproduces the bug. **PLF takes precedence**: if PLF (per `0008-partition-level-failover.md`) has already pinned a region for this PKRangeId, the PLF region wins over the "prefer remote" hint.
...
Beyond 449 and 404/1002, Gateway 2.0 follows the timeout/408 handling defined in `0005-operation-and-transport-pipelines.md` — no Gateway-2.0-specific override is introduced.
...
Keep the regional-retry behavior. A single attempt against an unreachable G2 endpoint fails, the endpoint is marked `TransportError`, the retry tries the next region's G2 endpoint, and the operation eventually fails with a transport error if all G2 regions are unreachable. The error and diagnostics surface a clear, actionable hint that the Gateway 2.0 endpoints are unreachable. The connectivity probe or account advertisement can automatically converge the client to G1; operators can also set `AZURE_COSMOS_CONNECTION_POOL_GATEWAY_V2_DISABLED_OVERRIDE=true` and restart the process for immediate fleet-wide mitigation.
...
- **Simplicity.** No new state, no concurrency contract, no recovery logic in the data path. Customers behind firewalls get a single deterministic verdict and can rely on automatic server-driven convergence or use the runtime-scoped disable override for incident mitigation.
...
The Rust driver wires the HTTP request header `x-ms-cosmos-sdk-supportedcapabilities` through `SDK_SUPPORTED_CAPABILITIES` in `azure_data_cosmos_driver/src/driver/transport/cosmos_headers.rs`, with `SUPPORTED_CAPABILITIES_BITS` as the source of truth for its value. The header is emitted on every gateway request as the string-encoded decimal bitmask `"11"`.

The driver emits `PartitionMerge` (bit 0, decimal 1), `ChangeFeedWithStartTimePostMerge` (bit 1, decimal 2), and `IgnoreUnknownRntbdTokens` (bit 3, decimal 8). The header value is a string-encoded decimal of the bitwise OR of the capability bits.
...
##### Capability bit composition (Rust = `11`)

The bitmask the Rust driver advertises is **`11`**. It is pinned in `azure_data_cosmos_driver/src/driver/transport/cosmos_headers.rs` with a `const _: () = assert!(SUPPORTED_CAPABILITIES_BITS == 11);` invariant. The bits are:

| Bit | Decimal | Capability                 | Rust advertises | Notes                                                                                                                                                                                                                                                                                                                        |
| --- | ------- | -------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | 1       | `PartitionMerge`           | yes             | Advertises support for routing across merged partition-key ranges.                                                                                                                                                                                                                                                           |
| 1   | 2       | `ChangeFeedWithStartTimePostMerge` | yes      | Advertises change-feed start-time support after a partition merge.                                                                                                                                                                                                                                                           |
...
Total: Rust `11`. The driver only advertises capabilities it implements end-to-end. Adding any further bit requires implementing the corresponding behavior first, then updating the constant in `cosmos_headers.rs` and re-pinning the header-value and Gateway V2 RNTBD metadata tests.

Test coverage asserts that the header value emitted on Gateway 2.0 (and standard Gateway) requests is `"11"`, that Gateway V2 metadata carries `11`, and that point-in-time change feed requests carry RNTBD `IfModifiedSince` (`0x0047`) alongside their continuation.
...
3. **Client opt-out short-circuits probing** — the probe runs whenever `thinClient*Locations` are advertised unless effective `gateway_v2_disabled` is `true`. Explicit disablement and lack of HTTP/2 both short-circuit the entire Gateway 2.0 code path, probe included.
...
- **Timeout policy** — Gateway 2.0 requests use the timeout regime defined in `0005-operation-and-transport-pipelines.md` (single timeout, not bifurcated). Do not introduce Gateway-2.0-specific timeouts in this work; any Gateway 2.0–specific timeout tuning will be addressed in a follow-up.
- **Gateway 2.0 eligibility fallback** — see "Fallback taxonomy" below.
- **Partition-Level Failover interaction** — when PLF (see `0008-partition-level-failover.md`) selects a region, the per-request decision is: if that region's `CosmosEndpoint` exposes a `gateway_v2_url`, the request uses Gateway 2.0; if it does not, the request falls back to standard gateway for that partition until PLF releases its override. PLF chooses the region; Gateway 2.0 is preferred whenever it is available in that region.
...
Gateway 2.0 is **on by default** when the account metadata advertises Gateway 2.0 endpoints and the connectivity probe confirms them. There is no force-enable option; the runtime-scoped disable switch only opts out to Gateway V1.
...
- **Runtime opt-out** — `ConnectionPoolOptions::gateway_v2_disabled` and its base/override environment variables can force Gateway V1. They cannot force Gateway 2.0 or bypass service advertisement, connectivity probing, or HTTP/2; see §3.2.
...
| SDK-supported-capabilities header                      | Yes  |             |     | `x-ms-cosmos-sdk-supportedcapabilities` and Gateway V2 RNTBD metadata emit the exact aggregate of `PartitionMerge`, `ChangeFeedWithStartTimePostMerge`, and `IgnoreUnknownRntbdTokens` (`"11"` / `11`).                                                                                                                                                                                                                                                                                                                                 |
...
- **Q1 — HTTP/2 prior knowledge vs ALPN**: *Resolved*. Gateway 2.0 always uses HTTP/2; the proxy does not accept HTTP/1.x. Rust uses HTTP/2 with prior knowledge on the Gateway 2.0 transport (no ALPN fallback to HTTP/1.x). The broader ALPN default in `0005-operation-and-transport-pipelines.md` does **not** apply to Gateway 2.0; if HTTP/2 negotiation fails, the request fails and the existing retry policies handle it.
