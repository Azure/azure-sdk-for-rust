...
### 3.4 Reconciliation with `0005-operation-and-transport-pipelines.md` §4.2 — **Resolved**
...
[`0010-hub-region-processing-header.md`](0010-hub-region-processing-header.md)
...
`0010-hub-region-processing-header.md` (account-level scope,
...
| Condition | Why |
|---|---|
| Operation is data-plane (`is_dataplane`) | Mirrors the §1.5 scope of `0010-hub-region-processing-header.md`. |
| Account is single-master (`!can_use_multiple_write_locations`) | Mirrors AC-4 of `0010-hub-region-processing-header.md`; multi-master accounts have a separate recovery path and the header is never emitted. |
...
- The session-token retry counter would not advance, breaking the
  ...
  `0010-hub-region-processing-header.md` §7.1.
...
| `shared_hub_region_latch_none_on_multi_master_or_metadata` | Multi-master *or* metadata pipeline; assert `shared_hub_region_latch` is `None` even when the alternate spawns, matching `0010-hub-region-processing-header.md` §5 account-level / §1.5 data-plane gates (§9.6.3). |
...
- Integration into `cosmos_driver.rs` (§8.1) — final integration point
  ...
  `0005-operation-and-transport-pipelines.md` §4.2 (§3.4).
...
8. **Reconciliation with `0005-operation-and-transport-pipelines.md` §4.2** —
