...
>
> **Retained cursor:** `sdk/cosmos/docs/specs/0028-native-feed-cursor.md`
...
- Rename the underlying Rust types so they emit without the redundant `Cosmos` prefix and with the `_t` suffix this spec uses. Concretely (`export.rename`):
  ...
  "CosmosStatusCode"   = "cosmos_status_code_t"
  "CosmosSubStatus"    = "cosmos_sub_status_t"
  ```
...
- **From PR #3347 and follow-ups** — `RuntimeContext` shape and ownership, the `cosmos_error_code_t` value-range layout (FFI / Cosmos / plumbing bands) — **since replaced** by the unified packed `cosmos_status_code_t` in [#4696](https://github.com/Azure/azure-sdk-for-rust/issues/4696) (§3.5.1), `cosmos_string_free` / `cosmos_bytes_free`, the `cosmos_*` symbol prefix convention, the `_t` suffix on type names, the `_unused: u8` placeholder pattern (which this crate **drops** — see §7). The `CallContext` thread-affine error slot from #3347 is **superseded** in this crate by the completion-queue invocation model in §3.1 / §3.6 — errors now flow through completion records, not a per-thread context.
- **New in this crate** — every `cosmos_*` API documented in §3.5 and §4 below, the cbindgen `export.rename` / `item_types` policy from §2.2, the rich `cosmos_error_t` accessor + predicate surface from §3.5.2, the flat versioned request surface from §4.6, the driver cache normative documentation from §4.4.1, the ABI version major-equal / minor-≥ rule from §7.
...
func (d *Driver) ReadItemAsync(ctx context.Context, op *Operation, opts *Options) (*Response, error) {
    ...
    var preErr C.cosmos_status_code_t
...
typedef struct cosmos_cq_options {
     ...
     * `max_capacity` pending completions are rejected pre-flight with a
     * 503 + COSMOS_SUB_STATUS_CLIENT_FFI_QUEUE_FULL packed status. Use
     * `cosmos_cq_wait_writable` (see
     * §3.1.3) to block until space frees, or treat that status as the host
     * SDK's back-pressure signal. 0 = unbounded (the default) — appropriate for
     ...
     * on failure (see §3.5.2). When false, only the packed
     * cosmos_status_code_t is set and the rich payload is NULL. Defaults to true. */
...
```c
cosmos_status_code_t cosmos_<noun>_<verb>(
    /* required handle(s) */,
    /* required scalars */,
...
cosmos_operation_handle_t *cosmos_<noun>_<verb>_submit(
    ...
    cosmos_status_code_t *out_pre_error);            /* synchronous pre-flight error slot, nullable */
```

Behavior:

- On successful **submission** (the request was accepted and will eventually post a completion to `queue`): returns a non-NULL `cosmos_operation_handle_t*` and writes `COSMOS_STATUS_SUCCESS` to `*out_pre_error` (when non-NULL).
...
A **packed 32-bit status** returned by every fallible C function. It carries the driver's canonical `(http_status, sub_status)` pair in a single `int32_t`, so there is exactly one error vocabulary across the whole ABI — the driver's — and no bespoke FFI enum to keep in sync.

**Encoding.** `status = (http_status << 16) | sub_status`:
...
Service (wire) failures pack their real HTTP status and sub-status straight from the driver `CosmosError` via the same encoding — see §6 for the full picture. For **synchronous** entry points the structured detail lives on the owned rich `cosmos_error_t` handed back through `out_error`; for **asynchronous** completions the same detail is flattened directly into the inline fields of `cosmos_completion_t` (`message`, `sub_status`, `activity_id`, …). Either way, the packed status is only for the common dispatch path (switch on `COSMOS_STATUS_HTTP` in C, "expected vs unexpected" branch).
...
Status-code main values follow the same approach via the packed `cosmos_status_code_t` (§3.5.1) plus the standard HTTP status codes the wrapper surfaces through `cosmos_error_status_code(e)` — host SDKs that switch on HTTP status either use their language-standard `HttpStatusCode` enum or define their own constants from `cosmos_error_status_code`'s `uint16_t` return; the wrapper does **not** re-export the standard HTTP code values as `COSMOS_HTTP_*` constants to avoid polluting the header with values that are already standardized across every language.
 ...
 *       — a wire error with HTTP 429 → COSMOS_STATUS_HTTP == 429;
 *       — a wire error with HTTP 404 → COSMOS_STATUS_HTTP == 404;
 *       — a synthetic 503 with sub_status TRANSPORT_GENERATED_503 →
 *         COSMOS_STATUS_SUB == COSMOS_SUB_STATUS_TRANSPORT_GENERATED_503;
 *       — a synthetic 408 with sub_status CLIENT_OPERATION_TIMEOUT →
 *         COSMOS_STATUS_SUB == COSMOS_SUB_STATUS_CLIENT_OPERATION_TIMEOUT.
 *     The packing is identical when include_error_details = false, so host
 ...
 * Returns a 400 + COSMOS_SUB_STATUS_CLIENT_FFI_INVALID_OPTION_VALUE packed status on values exceeding
 * an internal sanity cap (currently 4096); the builder is left unchanged. */
cosmos_status_code_t cosmos_runtime_builder_with_worker_threads(
...
cosmos_status_code_t cosmos_runtime_builder_with_thread_name_prefix(
...
cosmos_status_code_t cosmos_runtime_builder_with_user_agent_suffix(
    cosmos_runtime_builder_t *b, const char *suffix);
cosmos_status_code_t cosmos_runtime_builder_with_workload_id(
    cosmos_runtime_builder_t *b, uint64_t workload_id);
cosmos_status_code_t cosmos_runtime_builder_with_correlation_id(
    cosmos_runtime_builder_t *b, const char *correlation_id);
cosmos_status_code_t cosmos_runtime_builder_with_allow_emulator_invalid_certs(
...
cosmos_status_code_t cosmos_account_ref_with_master_key(
    const char *endpoint,
    const char *key,
...
cosmos_status_code_t cosmos_account_ref_with_resource_token(
    const char *endpoint,
    const char *token,
    cosmos_account_ref_t **out_account,
    cosmos_error_t **out_error);
...
cosmos_status_code_t cosmos_account_ref_clone(
    const cosmos_account_ref_t *account,
...
cosmos_status_code_t cosmos_database_ref_create(
    const cosmos_account_ref_t *account,
    const char *database_id,
    cosmos_database_ref_t **out_database);

cosmos_status_code_t cosmos_database_ref_clone(
...
cosmos_status_code_t cosmos_container_ref_create(
    const cosmos_database_ref_t *database,
    const char *container_id,
    cosmos_container_ref_t **out_container);

cosmos_status_code_t cosmos_container_ref_clone(
...
cosmos_status_code_t cosmos_driver_get_or_create_blocking(
    const cosmos_runtime_t *runtime,
    const cosmos_account_ref_t *account,
    const cosmos_driver_options_t *options,  /* nullable */
...
- **Options are silently ignored on cache hit.** If `cosmos_driver_get_or_create` is called twice for the same endpoint with different `options`, the second call returns the *first* driver and discards the second `options` argument (`runtime.rs:374`). The wrapper would ideally surface an advisory in this case, but the merged `CosmosDriverRuntime::get_or_create_driver` API does **not** expose a "was cached" signal, so cache-hit detection is **deferred** (tracked as a follow-up; see `driver.rs` module docs). Until a driver-side "was cached" signal exists:
  1. The completion is delivered with `outcome = OK` and packed status `COSMOS_STATUS_SUCCESS`; the cached driver is extractable via `cosmos_completion_take_driver`.
  2. No distinguishing advisory is emitted — a cache hit with differing `options` is indistinguishable from a fresh build. The unified error model (§3.5.1) has **no** separate warning band, so once the driver surfaces a cache-hit signal the advisory would ride as inline diagnostic fields on the OK completion (and, for the synchronous `cosmos_driver_get_or_create_blocking` convenience, on `*out_error`) rather than on a distinguished numeric return. Host SDKs that must react to ignored options should, for now, track endpoint→options themselves.
...
cosmos_status_code_t cosmos_partition_key_builder_append_string(
    cosmos_partition_key_builder_t *b, const char *value);
cosmos_status_code_t cosmos_partition_key_builder_append_number(
    cosmos_partition_key_builder_t *b, double value);
cosmos_status_code_t cosmos_partition_key_builder_append_bool(
    cosmos_partition_key_builder_t *b, bool value);
cosmos_status_code_t cosmos_partition_key_builder_append_null(
...
cosmos_status_code_t cosmos_partition_key_builder_append_undefined(
    cosmos_partition_key_builder_t *b);
...
cosmos_status_code_t cosmos_partition_key_builder_build(
...
strict validation rejects mismatches with a `400 + COSMOS_SUB_STATUS_CLIENT_FFI_NULL_ARGUMENT` packed status.
...
typedef struct cosmos_operation_request_t {
    ...
    const cosmos_partition_key_component_t *partition_key_components;
    size_t partition_key_len;            /* inline components take precedence */
    const cosmos_feed_range_t *feed_range; /* optional for query_items */

    const uint8_t *body;                /* NULL iff body_len == 0 */
    size_t body_len;
...
is no operation-builder handle to consume or free.

```c
cosmos_operation_handle_t *cosmos_submit_singleton_operation(
    const cosmos_driver_t *driver,
...
2. **A pre-flight rejection posts no completion.** When the driver / request is
   ...
   capacity, the submit returns NULL and writes a packed `cosmos_status_code_t`
...
3. **Request validation is strict and happens before any work is spawned.** An
   ...
   a `400 + COSMOS_SUB_STATUS_CLIENT_FFI_NULL_ARGUMENT` (or
   `...CLIENT_FFI_INVALID_OPTION_VALUE`) packed status before a Tokio task is spawned.
...
cosmos_status_code_t cosmos_feed_range_full(cosmos_feed_range_t **out_fr);
...
cosmos_status_code_t cosmos_feed_range_for_partition_key(
    const cosmos_container_ref_t *container,
    const cosmos_partition_key_t *pk,
    cosmos_feed_range_t **out_fr);

cosmos_status_code_t cosmos_feed_range_clone(
...
If a host SDK needs to resume a query from a continuation token (the typical EPK-range use case), the feed submit (`cosmos_submit_operation`) already covers that via the `continuation_token` round-trip on `cosmos_operation_request_t` — no `FeedRange` construction from the C side is required.
 ...
 * cosmos_submit_operation, prefer
 ...
 * cosmos_submit_operation, or NULL on the last page / a
 ...
 * only. */
cosmos_status_code_t cosmos_response_body(
...
cosmos_status_code_t cosmos_token_request_complete(
    uint64_t request_id,
    int32_t status,
    const uint8_t *token,
    uintptr_t token_len,
...
1. Rust registers an opaque `request_id` before calling `get_token`. `scope` and the request struct are borrowed only for that call, so the host copies the scope before starting asynchronous work.
2. A nonzero `get_token` return rejects synchronously. Rust removes the pending request; the host must not call `cosmos_token_request_complete`.
3. A zero return accepts the request. The host must call `cosmos_token_request_complete` exactly once, even when token acquisition fails.
4. `cosmos_token_request_complete` atomically removes `request_id`, copies the borrowed token or error buffers, and wakes the credential-owned refresh task. Unknown, late, or duplicate request IDs return `400 / CLIENT_FFI_NULL_ARGUMENT`.
...
A successful completion supplies a non-empty UTF-8 token and a Unix-seconds expiry later than the current time. A failed completion supplies a nonzero status and may supply a UTF-8 error message. The adapter requests exactly one scope (currently `https://cosmos.azure.com/.default`) and makes at most two immediate attempts for each acquisition.

After the first foreground acquisition, Rust schedules background refresh at 50% of the token's remaining lifetime, with a minimum one-minute interval. A successful refresh replaces the cache and schedules the next refresh from the new expiry. A failed background refresh leaves the still-valid token available to callers and retries after half of its remaining lifetime. Background retries stop once that interval would be under one minute; the next request after actual expiry performs foreground acquisition. This matches the default .NET Cosmos v3 policy and avoids adding refresh latency to ordinary requests. Cosmos 401 responses do not trigger credential refresh or request replay; CAE and claims-challenge handling are deferred.

The host may call `cosmos_token_request_complete` synchronously before `get_token` returns or asynchronously from another host thread. Go can call this fixed exported C function directly, without a per-request function-pointer trampoline. An accepted acquisition is owned by the credential rather than by the Cosmos operation that initiated it: cancelling one waiter does not cancel the host request, release provider state, or permit another caller to start a duplicate refresh. Later callers join the same in-flight operation, and the host must still complete every accepted request because v1 does not expose a host cancellation hook or impose an arbitrary token-acquisition timeout.
...
- The wrapper does **not** return 404 / 409 / 412 / 429 as `COSMOS_STATUS_SUCCESS` with a non-2xx `cosmos_response_status_code`. Those surface as completion outcome = ERROR with `cosmos_error_is_from_wire(err) == true` and the appropriate `cosmos_error_status_code(err)`.
...
- `cosmos_driver_get_or_create_submit` → wraps `runtime.get_or_create_driver(account, options).await` and delivers the result as a completion (`cosmos_response_take_driver` on the response). The cache-hit advisory (§4.4.1) is surfaced through the rich `cosmos_error_t` on the OK completion — `cosmos_completion_status` stays `COSMOS_STATUS_SUCCESS`; the unified model has no separate warning code.
...
- `cosmos_driver_free`.
- `c_tests/driver_init.c`: emulator-backed test that creates a driver (via `_blocking` for setup ergonomics), then runs a second `_submit` against the same endpoint with non-NULL `options` and asserts the cache-hit advisory fires on the delivered completion (`status == COSMOS_STATUS_SUCCESS`, `outcome == OK`, advisory rich error present).
...
### Phase 5 — Operation construction *(Goal: represent every operation without chatty factories)*

- `cosmos_operation_request_t` carries the operation kind, references, body,
...
**Done when:** Native Rust and C construction tests pin request sizes, field
offsets, sentinel decoding, strict operation-kind validation, and conversion to
the same `CosmosOperation` fields as native Rust callers.
...
- Cross-partition `read_all_items` and `query_items` use the feed submit path;
    batch uses its operation kind and body contract.
...
`sdk/cosmos/docs/specs/0028-native-feed-cursor.md`. That specification replaces the
...
- Async pager handle: `cosmos_pager_t`, with:
  ...
  - `cosmos_pager_next_submit(pager, queue, user_data, &pre_err)` → returns `cosmos_operation_handle_t*`; each page lands as its own completion (response per page; a `404 + COSMOS_SUB_STATUS_CLIENT_FFI_FEED_EXHAUSTED` packed status on the completion that drains the pager).
...
1. **Single-runtime-per-process enforcement.** The driver's `CosmosDriverRuntime` is process-cardinal in the spirit of `ARCHITECTURE.md`; do we enforce that at the wrapper boundary (rejecting a second `cosmos_runtime_builder_build` with a dedicated `cosmos_status_code_t`), allow N for testing convenience, or rely solely on host-SDK discipline (the §4.1 cardinality advisory)? *Not yet decided — §3.5.1 does not define a `CLIENT_FFI_*` sub-status for the enforcement variant.* (Note: the §4.4.1 `OPTIONS_IGNORED_ON_CACHE_HIT` advisory is **no longer** predicated on this question — it fires unconditionally whenever a cache hit is observed with non-NULL `options`, regardless of runtime cardinality.)
...
17. **Driver-owned buffer pool for request bodies.** §3.3 / §4.6.2 currently specify that `cosmos_operation_with_body` **copies** the caller's `cosmos_bytes_view_t` into wrapper-owned storage, because that contract interoperates cleanly with host-language GCs (pinned .NET buffers, Java `DirectByteBuffer`, Go `[]byte`, etc.) — the caller can release the source memory immediately. Both reviewers (Ashley #12, FabianMeiswinkel #16) flagged that for high-traffic workloads this copy is the obvious hot spot, and that a **driver-owned buffer pool** would let host SDKs write request bodies directly into pool-allocated memory and hand the buffer back via FFI without an intermediate copy. The conceptual API shape:
    ...
    cosmos_status_code_t cosmos_driver_acquire_request_buffer(
        const cosmos_driver_t *driver,
        size_t min_capacity,
    ...
    cosmos_status_code_t cosmos_request_buffer_set_len(
...
18. **Driver→SDK logging callback.** FabianMeiswinkel (review comment #17) raised that the driver currently logs via the `tracing` crate, but host SDK customers expect driver-emitted logs to appear in **their** logger (.NET `ILogger`, Java SLF4J, Go `slog`, Python `logging`, etc.), with their formatting, sink, and filtering. The wrapper today exposes `cosmos_enable_tracing()` (Phase 10) which just bootstraps the driver's internal `tracing-subscriber` and writes to stderr — that is not what host SDKs want. The conceptual API shape:
     ...
     * was not called). */
    cosmos_status_code_t cosmos_runtime_set_log_callback(
...
20. **Handle table for value-type references.** §3.4 documents that `cosmos_account_ref_t*` / `cosmos_database_ref_t*` / `cosmos_container_ref_t*` / `cosmos_partition_key_t*` / `cosmos_feed_range_t*` are heap-allocated handles, and §3.4 (post-#14 revision) notes they're immutable post-build so concurrent reads / clones / frees of **distinct** FFI handles are race-free. Ashley (review comments #5 and #6) flagged a remaining ergonomic / safety concern: nothing in the C ABI prevents a caller from copying a `cosmos_account_ref_t*` value (a raw pointer) around their codebase, leading to (a) double-free if two copies are both `_free`'d, (b) use-after-free if one copy is `_free`'d while another is in use, and (c) (for any future stack-allocated value handles) dangling pointers from value moves. The proposed mitigation is a **handle table**: the wrapper stores `Arc<…>`s in a process-global slab, and the C ABI hands out **integer indices** (`uint64_t handle_id` plus a generation counter to detect use-after-free) instead of raw pointers:
    ...
    cosmos_status_code_t cosmos_database_ref_create_h(
     ...
     * AND lookups on a freed id return a dedicated invalid-handle packed status
