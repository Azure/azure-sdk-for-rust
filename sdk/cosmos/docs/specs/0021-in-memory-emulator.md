            ...
            └── ContainerState
                ...
                └── physical_partitions: Vec<PhysicalPartition>
                    └── PhysicalPartition
                        ...
                        ├── vector_clock_version: AtomicU64  # Topology version (merge and membership)
...
| `vectorClockVersion`  | `u64`         | Topology version (0 initially; incremented on merge and region membership changes, preserved on split)                  |
...
the production `crate::models::effective_partition_key`
...
- `version` — vector clock version (incremented on merge and region membership
  changes)
...
3. **Version check**: if the token's version > partition's current version → 404/1002.
   Higher version means a topology change that hasn't been observed yet.
...
- **Region add/remove**: increments the version on every partition in every
  active region.
...
| Region add/remove          | Incremented on every active partition  |
