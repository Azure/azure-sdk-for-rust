...
- **Summary output has an independent byte cap.** The
  `DiagnosticsOptions::max_summary_size_bytes` knob defaults to **8 KB**, has a
  **4 KB** minimum, and can be set with
...
### 8.1 Transport event visibility

The driver uses `reqwest`, which does not expose the same fine-grained
connection lifecycle callbacks as Reactor Netty. Diagnostics can record when a
transport attempt starts, when response headers arrive, when the response body
finishes, and when transport fails. They cannot currently separate DNS
...
that the current transport cannot observe.
