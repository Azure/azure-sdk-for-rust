...
pub trait ExecutorExt {
    ...
    /// using the specified `CommandResponseProcessor`.
    ///
    /// - Windows: Uses `cmd /C` in the SYSTEMROOT directory
    /// - Everything else: Uses `/bin/sh -c` in the /bin directory
    async fn shell_exec<P>(
        &self,
        env: &Env,
        command: &str,
        _processor_marker: PhantomData<P>,
    ) -> Result<AccessToken>
    where
        P: OutputProcessor;
}
...
#[async_trait::async_trait]
impl<T: Executor + Send + Sync + ?Sized> ExecutorExt for T {
    async fn shell_exec<P>(
        &self,
        env: &Env,
        command: &str,
        ...
        P: OutputProcessor,
    {
        ...
        let status = self.run(OsStr::new(program), &args).await;

        match status {
            Ok(output) if output.status.success() => {
                P::deserialize_token(&String::from_utf8_lossy(&output.stdout))
            }
            Ok(output) => {
                let stderr = String::from_utf8_lossy(&output.stderr);
                let message = if let Some(error_message) = P::get_error_message(&stderr) {
                    error_message.to_string()
                } else if output.status.code() == Some(127)
                    || stderr.contains("' is not recognized")
                {
                    format!("{} not found on path", P::tool_name())
                } else {
                    stderr.to_string()
                };
                Err(Error::with_message(ErrorKind::Credential, || {
                    format!("{} authentication failed: {message}", P::credential_name())
                }))
            }
            Err(e) => {
                let message = format!(
