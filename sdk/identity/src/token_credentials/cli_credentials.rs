...
impl AzureCliCredential {
    ...
    fn get_access_token(resource: Option<&str>) -> Result<CliTokenResponse, azure_core::Error> {
        ...
        match Command::new(program).args(args).output() {
            ...
            Ok(az_output) => {
                ...
                Err(azure_core::Error::GenericErrorWithText(output.to_string()))
            }
            Err(e) => match e.kind() {
                ErrorKind::NotFound => Err(azure_core::Error::GenericErrorWithText(
                    "Azure CLI not installed".to_owned(),
                )),
                _ => Err(azure_core::Error::GenericErrorWithText(e.to_string())),
            },
        }
    }

    /// Returns the current subscription ID from the Azure CLI.
    pub fn get_subscription() -> Result<String, azure_core::Error> {
        let tr = Self::get_access_token(None)?;
        Ok(tr.subscription)
    }

    /// Returns the current tenant ID from the Azure CLI.
    pub fn get_tenant() -> Result<String, azure_core::Error> {
        let tr = Self::get_access_token(None)?;
        Ok(tr.tenant)
    }
}
