...
- `ConsumerClient::close` and `ProducerClient::close` no longer report an error when another object still holds the connection, and the close now runs the orderly teardown instead of skipping it. Both methods took the connection out of its `Arc`, so a handle that the caller still held, most often an `EventReceiver`, made the close fail. Both methods take `self`, so the client was consumed anyway, and `RecoverableConnection::drop` only writes a trace message, which left the connection open with no owner and with no AMQP close sent. A handle that outlives its client now reports that the client is closed on its next call, where it opened a second connection to the service before. `EventProcessor::close` reports a partition client that the application still holds and continues, so one such client no longer stops it from closing the partition clients that follow. ([#4931](https://github.com/Azure/azure-sdk-for-rust/issues/4931))
