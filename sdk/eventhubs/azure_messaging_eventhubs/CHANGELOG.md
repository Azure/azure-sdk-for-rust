...
- `OpenReceiverOptions::owner_level` now reaches the broker. The AMQP receiver dropped the link properties before it attached, so the `com.microsoft:epoch` property never left the client and the service never arbitrated between two consumers on one partition. The crate now takes `azure_core_amqp` 1.2.0-beta.1, which keeps the properties on the link. ([#4888](https://github.com/Azure/azure-sdk-for-rust/issues/4888))
