        ...
        fn from(event_data: EventData) -> Self {
                ...
                || event_data.message_id.is_some()
            {
                ...
                if let Some(content_type) = event_data.content_type {
                    message_properties_builder = message_properties_builder.with_content_type(content_type);
                }
                if let Some(correlation_id) = event_data.correlation_id {
                    message_properties_builder = message_properties_builder.with_correlation_id(correlation_id);
                }
            ...
            if let Some(event_body) = event_data.body {
                message_builder = message_builder.with_body(AmqpMessageBody::Binary(vec![event_body.to_vec()]));
            }
