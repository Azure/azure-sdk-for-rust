...
use azure_core_amqp::{
    ...
    AmqpSession, AmqpSessionApis as _, AmqpSimpleValue, AmqpSymbol,
};
...
pub(crate) struct RecoverableConnection {
    ...
    mgmt_client: AsyncMutex<OnceLock<Arc<AmqpManagement>>>,
...
impl RecoverableConnection {
    ...
    async fn ensure_amqp_management(self: &Arc<Self>) -> Result<Arc<AmqpManagement>> {
        ...
        if mgmt_client.get().is_some() {
            return Ok(mgmt_client.get().unwrap().clone());
        }
        ...
        let access_token = self.authorize_path(&connection, &management_path).await?;
        ...
        mgmt_client.set(management).map_err(|_| {
            error!("Failed to set management client.");
            EventHubsError::from(ErrorKind::MissingManagementClient)
        })?;
        trace!("Management client created.");
        Ok(mgmt_client.get().unwrap().clone())
    }
