...
use azure::core::parsing::{traverse_single_must, traverse_single_optional, traverse,
    from_azure_time, inner_text, traverse_inner_text_must, traverse_inner_text_optional,
    traverse_inner_date_optional, traverse_inner_date_must,
    traverse_single_cast_must, traverse_single_cast_optional,
    traverse_inner_u64_optional};
...
pub fn parse(elem: &Element) -> Result<Blob,core::errors::AzureError> {
    ...
    let content_length = try!(try!(traverse_inner_text_must(elem, &["Properties", "Content-Length"])).parse::<u64>());
    let content_type = try!(traverse_inner_text_must(elem, &["Properties", "Content-Type"]));
    let content_encoding =  try!(traverse_inner_text_optional(elem, &["Properties", "Content-Encoding"]));
    let content_language = try!(traverse_inner_text_optional(elem, &["Properties", "Content-Language"]));
    ...
    let x_ms_blob_sequence_number = try!(traverse_inner_text_optional(elem, &["Properties", "x-ms-blob-sequence-number"]));
    let blob_type = try!(traverse_single_cast_must::<BlobType>(elem, &["Properties", "BlobType"]));
    let lease_status = try!(traverse_single_cast_must::<LeaseStatus>(elem, &["Properties", "LeaseStatus"]));
    let lease_state = try!(traverse_single_cast_must::<LeaseState>(elem, &["Properties", "LeaseState"]));
    let lease_duration = try!(traverse_single_cast_optional::<LeaseDuration>(elem, &["Properties", "LeaseDuration"]));
    let copy_id = try!(traverse_inner_text_optional(elem, &["Properties", "CopyId"]));
    let copy_status = try!(traverse_single_cast_optional::<CopyStatus>(elem, &["Properties", "CopyStatus"]));
    ...
    let copy_completion = try!(traverse_inner_date_optional(elem, &["Properties", "CopyCompletionTime"]));
    let copy_status_description = try!(traverse_inner_text_optional(elem, &["Properties", "CopyStatusDescription"]));
