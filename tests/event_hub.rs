...
fn create_client() -> azure::service_bus::event_hub::Client {
    ...
    let sb_namespace = std::env::var("AZURE_SERVICE_BUS_NAMESPACE").expect("Please set AZURE_SERVICE_BUS_NAMESPACE env variable first!");
     
    let ev_name = std::env::var("AZURE_EVENT_HUB_NAME").expect("Please set AZURE_EVENT_HUB_NAME env variable first!");
   
    azure::service_bus::event_hub::Client::new(&sb_namespace,
                                                                   &ev_name,
                                                                   &policy_name,
                                                                   &policy_key)
