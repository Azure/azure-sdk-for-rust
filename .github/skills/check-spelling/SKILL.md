...
1. **It's a real error** — fix the misspelling directly in the source file. If you cannot confidently determine the correct spelling, ask the user.
...
For words that should be ignored at the service level, add them to the `ignoreWords` array in a `.cspell.json` file under `sdk/{service-directory}` or lower. If the file doesn't exist, create it with an `import` of either a parent `.cspell.json` or `../../.vscode/cspell.json` from the root of the repo.
