...
1. **Prefer the Azure SDK Generation Pipeline when available**
	...
	- Recommend configuring `tspconfig.yaml` directly in `${input:specRepoRef}` and queueing the internal Azure DevOps "SDK Generation Pipeline" (definition ID 7421). Provide the config path, target `api-version`, and commit SHA if the REST spec PR is still open.
