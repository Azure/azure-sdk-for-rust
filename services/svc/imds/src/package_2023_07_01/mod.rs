...
impl Client {
    #[doc = "Get supported API versions."]
    pub fn get_versions(&self) -> get_versions::RequestBuilder {
        get_versions::RequestBuilder { client: self.clone() }
    }
}
pub mod get_versions {
    ...
    impl Response {
        pub async fn into_body(self) -> azure_core::Result<models::Versions> {
            let bytes = self.0.into_body().collect().await?;
            let body: models::Versions = serde_json::from_slice(&bytes)?;
            Ok(body)
        }
    ...
    impl RequestBuilder {
        ...
        fn url(&self) -> azure_core::Result<azure_core::Url> {
            let url = azure_core::Url::parse(&format!("{}/versions", self.client.endpoint(),))?;
            Ok(url)
        }
    }
    impl std::future::IntoFuture for RequestBuilder {
        type Output = azure_core::Result<models::Versions>;
        type IntoFuture = BoxFuture<'static, azure_core::Result<models::Versions>>;
