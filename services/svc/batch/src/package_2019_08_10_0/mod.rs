...
pub mod account {
    ...
    impl Client {
        ...
        #[doc = "Gets the number of Compute Nodes in each state, grouped by Pool."]
...
pub mod job {
    ...
    impl Client {
        ...
        #[doc = "Task counts provide a count of the Tasks by active, running or completed Task state, and a count of Tasks which succeeded or failed. Tasks in the preparing state are counted as running."]
        #[doc = ""]
        #[doc = "Arguments:"]
        #[doc = "* `job_id`: The ID of the Job."]
    ...
    pub mod get_task_counts {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::TaskCounts> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::TaskCounts = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            #[doc = "Send the request and return the response body."]
            pub fn into_future(self) -> futures::future::BoxFuture<'static, azure_core::Result<models::TaskCounts>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
