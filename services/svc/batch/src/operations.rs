...
pub enum Error {
    ...
    #[error(transparent)]
    ComputeNodeExtension_Get(#[from] compute_node_extension::get::Error),
    #[error(transparent)]
    ComputeNodeExtension_List(#[from] compute_node_extension::list::Error),
}
pub mod application {
    use super::models;
        ...
        impl Builder {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::ApplicationListResult, Error>> {
                Box::pin(async move {
                    ...
                    url.query_pairs_mut().append_pair("api-version", "2022-01-01.15.0");
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::TaskCountsResult, Error>> {
                Box::pin(async move {
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::TaskCountsResult =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
...
pub mod compute_node_extension {
    ...
    pub mod get {
        ...
        impl Builder {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::NodeVmExtension, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::NodeVmExtension =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
    ...
    pub mod list {
        ...
        impl Builder {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::NodeVmExtensionList, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                        "{}/pools/{}/nodes/{}/extensions",
                        self.client.endpoint(),
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::NodeVmExtensionList =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
