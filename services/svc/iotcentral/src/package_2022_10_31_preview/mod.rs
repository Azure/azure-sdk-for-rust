...
pub mod deployment_manifests {
        ...
        impl RequestBuilder {
            ...
            pub fn into_stream(self) -> azure_core::Pageable<models::DeploymentManifestCollection, azure_core::error::Error> {
                let make_request = move |continuation: Option<String>| {
                    let this = self.clone();
                    async move {
                        let mut url = azure_core::Url::parse(&format!("{}/deploymentManifests", this.client.endpoint(),))?;
                        let rsp = match continuation {
                            Some(value) => {
                                url.set_path("");
                                url = url.join(&value)?;
    ...
    pub mod get {
        ...
        impl RequestBuilder {
            ...
            #[doc = "Send the request and return the response body."]
            pub fn into_future(self) -> futures::future::BoxFuture<'static, azure_core::Result<models::DeploymentManifest>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
