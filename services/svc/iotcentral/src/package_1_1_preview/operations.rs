    ...
    pub mod list_exports {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::ExportCollection, Error>> {
                Box::pin(async move {
                    let url_str = &format!("{}/dataExport/destinations/{}/exports", &self.client.endpoint, &self.destination_id);
                    let mut url = url::Url::parse(url_str).map_err(Error::ParseUrl)?;
                    let mut req_builder = http::request::Builder::new();
                    req_builder = req_builder.method(http::Method::GET);
                    let credential = self.client.credential();
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::DeviceRelationship, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                        ...
                        &self.client.endpoint, &self.device_id, &self.relationship_id
                    );
                    let mut url = url::Url::parse(url_str).map_err(Error::ParseUrl)?;
                    let mut req_builder = http::request::Builder::new();
                    req_builder = req_builder.method(http::Method::GET);
...
pub mod file_uploads {
    ...
    impl Client {
        ...
        pub fn remove(&self) -> remove::Builder {
            remove::Builder { client: self.base_clone() }
        }
    }
    pub mod get {
        use super::{models, API_VERSION};
