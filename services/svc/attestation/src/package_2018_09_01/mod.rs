...
pub mod policy {
        ...
        impl RequestBuilder {
            ...
            pub fn send(self) -> futures::future::BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    let this = self.clone();
                    async move {
                        let url = azure_core::Url::parse(&format!("{}/operations/policy/updatepolicy", this.client.endpoint(),))?;
                        ...
                        let tee = &this.tee;
                        req.url_mut().query_pairs_mut().append_pair("tee", tee);
    ...
    pub mod get {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::AttestationPolicy> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::AttestationPolicy = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            pub fn send(self) -> futures::future::BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    let this = self.clone();
                    async move {
                        let url = azure_core::Url::parse(&format!("{}/operations/policy/current", this.client.endpoint(),))?;
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::AttestationPolicy>;
            type IntoFuture = futures::future::BoxFuture<'static, azure_core::Result<models::AttestationPolicy>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
        ...
        impl RequestBuilder {
            ...
            pub fn send(self) -> futures::future::BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    let this = self.clone();
                    async move {
                        let url = azure_core::Url::parse(&format!("{}/operations/policy/certificates", this.client.endpoint(),))?;
                        let mut req = azure_core::Request::new(url, azure_core::Method::Get);
                        let credential = this.client.token_credential();
                        let token_response = credential.get_token(&this.client.scopes().join(" ")).await?;
                        req.insert_header(
