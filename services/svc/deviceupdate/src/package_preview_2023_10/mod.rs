...
impl Client {
    ...
    pub fn instance_management_client(&self) -> instance_management::Client {
        instance_management::Client(self.clone())
    }
}
...
pub mod instance_management {
    ...
    pub struct Client(pub(crate) super::Client);
    impl Client {
        #[doc = "Gets the instance limits for active deployments, devices, device classes, and groups."]
        #[doc = ""]
        #[doc = "Arguments:"]
        #[doc = "* `instance_id`: The Device Update for IoT Hub account instance identifier."]
        pub fn get_limits(&self, instance_id: impl Into<String>) -> get_limits::RequestBuilder {
            get_limits::RequestBuilder {
                client: self.0.clone(),
                instance_id: instance_id.into(),
            }
        }
    ...
    pub mod get_limits {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::LimitsResponse> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::LimitsResponse = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                let mut url = azure_core::Url::parse(&format!("{}/deviceUpdate/{}/limits", self.client.endpoint(), &self.instance_id))?;
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
                        .append_pair(azure_core::query_param::API_VERSION, "2023-10-01-preview");
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::LimitsResponse>;
            type IntoFuture = BoxFuture<'static, azure_core::Result<models::LimitsResponse>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
