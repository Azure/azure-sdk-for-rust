...
pub enum Error {
    ...
    #[error(transparent)]
    ValidateClusterUpgrade(#[from] validate_cluster_upgrade::Error),
    #[error(transparent)]
...
pub async fn validate_cluster_upgrade(
    operation_config: &crate::OperationConfig,
    start_cluster_upgrade_description: &models::StartClusterUpgradeDescription,
    timeout: Option<i64>,
) -> std::result::Result<models::ValidateClusterUpgradeResult, validate_cluster_upgrade::Error> {
    let http_client = operation_config.http_client();
    let url_str = &format!("{}/$/ValidateUpgrade", operation_config.base_path(),);
    let mut url = url::Url::parse(url_str).map_err(validate_cluster_upgrade::Error::ParseUrlError)?;
    let mut req_builder = http::request::Builder::new();
    req_builder = req_builder.method(http::Method::POST);
    if let Some(token_credential) = operation_config.token_credential() {
        let token_response = token_credential
            .get_token(operation_config.token_credential_resource())
            .await
            .map_err(validate_cluster_upgrade::Error::GetTokenError)?;
        req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
    }
    url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
    req_builder = req_builder.header("content-type", "application/json");
    let req_body = azure_core::to_json(start_cluster_upgrade_description).map_err(validate_cluster_upgrade::Error::SerializeError)?;
    if let Some(timeout) = timeout {
        url.query_pairs_mut().append_pair("timeout", timeout.to_string().as_str());
    }
    req_builder = req_builder.uri(url.as_str());
    let req = req_builder
        .body(req_body)
        .map_err(validate_cluster_upgrade::Error::BuildRequestError)?;
    let rsp = http_client
        .execute_request(req)
        .await
        .map_err(validate_cluster_upgrade::Error::ExecuteRequestError)?;
    match rsp.status() {
        http::StatusCode::OK => {
            let rsp_body = rsp.body();
            let rsp_value: models::ValidateClusterUpgradeResult = serde_json::from_slice(rsp_body)
                .map_err(|source| validate_cluster_upgrade::Error::DeserializeError(source, rsp_body.clone()))?;
            Ok(rsp_value)
        }
        status_code => {
            let rsp_body = rsp.body();
            let rsp_value: models::FabricError = serde_json::from_slice(rsp_body)
                .map_err(|source| validate_cluster_upgrade::Error::DeserializeError(source, rsp_body.clone()))?;
            Err(validate_cluster_upgrade::Error::DefaultResponse {
                status_code,
                value: rsp_value,
            })
        }
...
pub mod validate_cluster_upgrade {
    use super::{models, API_VERSION};
    #[derive(Debug, thiserror :: Error)]
    pub enum Error {
        #[error("HTTP status code {}", status_code)]
