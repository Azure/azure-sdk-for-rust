...
pub mod web_pub_sub {
    ...
    pub mod generate_client_token {
        ...
        impl RequestBuilder {
            ...
            #[doc = "The type of client. Case-insensitive. If not set, it's \"Default\". For Web PubSub for Socket.IO, only the default value is supported. For Web PubSub, the valid values are 'Default' and 'MQTT'."]
            ...
            pub fn send(self) -> BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    ...
                    async move {
                        ...
                        if let Some(client_type) = &this.client_type {
                            req.url_mut().query_pairs_mut().append_pair("clientType", client_type);
                        }
                        let req_body = azure_core::EMPTY_BODY;
                        req.insert_header(azure_core::headers::CONTENT_LENGTH, "0");
                        req.set_body(req_body);
