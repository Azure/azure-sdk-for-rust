...
azure_core::static_url!(DEFAULT_ENDPOINT, "https://sts.mixedreality.azure.com");
impl ClientBuilder {
...
impl Client {
    ...
    #[doc = ""]
    #[doc = "Arguments:"]
    #[doc = "* `account_id`: The Mixed Reality account identifier."]
    pub fn get_token(&self, account_id: impl Into<String>) -> get_token::RequestBuilder {
...
pub mod get_token {
    ...
    impl Response {
        pub async fn into_body(self) -> azure_core::Result<models::StsTokenResponseMessage> {
            let bytes = self.0.into_body().collect().await?;
            let body: models::StsTokenResponseMessage = serde_json::from_slice(&bytes)?;
            Ok(body)
        }
    ...
    impl<'a> Headers<'a> {
        #[doc = "The service response correlation vector, which will be a new value for every response. Useful when debugging with Microsoft."]
    ...
    impl RequestBuilder {
        #[doc = "The client request correlation vector, which should be set to a new value for each request. Useful when debugging with Microsoft."]
        ...
        fn url(&self) -> azure_core::Result<azure_core::Url> {
            ...
            if !has_api_version_already {
                url.query_pairs_mut()
                    .append_pair(azure_core::query_param::API_VERSION, "2019-02-28-preview");
            }
            Ok(url)
        }
    }
    impl std::future::IntoFuture for RequestBuilder {
        type Output = azure_core::Result<models::StsTokenResponseMessage>;
        type IntoFuture = BoxFuture<'static, azure_core::Result<models::StsTokenResponseMessage>>;
