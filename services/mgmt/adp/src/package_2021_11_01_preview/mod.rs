...
pub mod accounts {
    ...
    pub struct Client(pub(crate) super::Client);
    impl Client {
        #[doc = "Checks that the account name is valid and is not already in use"]
        #[doc = ""]
        #[doc = "Arguments:"]
        #[doc = "* `subscription_id`: The ID of the target subscription."]
        #[doc = "* `parameters`: The checkNameAvailability parameters."]
...
pub mod data_pools {
    ...
    pub mod list {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::DataPoolList> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::DataPoolList = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            pub fn into_stream(self) -> azure_core::Pageable<models::DataPoolList, azure_core::error::Error> {
                let make_request = move |continuation: Option<String>| {
                    let this = self.clone();
                    async move {
                        let mut url = azure_core::Url::parse(&format!(
    ...
    pub mod get {
        ...
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::DataPool> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::DataPool = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            pub fn send(self) -> futures::future::BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    let this = self.clone();
                    async move {
                        let url = azure_core :: Url :: parse (& format ! ("{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.AutonomousDevelopmentPlatform/accounts/{}/dataPools/{}" , this . client . endpoint () , & this . subscription_id , & this . resource_group_name , & this . account_name , & this . data_pool_name)) ? ;
                        let mut req = azure_core::Request::new(url, azure_core::Method::Get);
                        let credential = this.client.token_credential();
                        let token_response = credential.get_token(&this.client.scopes().join(" ")).await?;
                        req.insert_header(
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::DataPool>;
            type IntoFuture = futures::future::BoxFuture<'static, azure_core::Result<models::DataPool>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
