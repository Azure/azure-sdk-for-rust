...
pub enum Error {
    ...
    #[error(transparent)]
    NetworkFunctions_ExecuteRequest(#[from] network_functions::execute_request::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    VendorSkus_ListCredential(#[from] vendor_skus::list_credential::Error),
    #[error(transparent)]
                ...
                Box::pin({
                    ...
                    async move {
                        ...
                        match rsp_status {
                            http::StatusCode::OK => {
                                let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                                let rsp_value: models::SkuCredential =
                                    serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                                Ok(rsp_value)
                            }
                            status_code => {
