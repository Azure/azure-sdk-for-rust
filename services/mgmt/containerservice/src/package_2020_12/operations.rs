...
pub mod maintenance_configurations {
    ...
    pub async fn list_by_managed_cluster(
        ...
        req_builder = req_builder.query(&[("api-version", &operation_config.api_version)]);
        let req = req_builder.build().context(list_by_managed_cluster::BuildRequestError)?;
        let rsp = client.execute(req).await.context(list_by_managed_cluster::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(list_by_managed_cluster::ResponseBytesError)?;
                let rsp_value: MaintenanceConfigurationListResult =
                    serde_json::from_slice(&body).context(list_by_managed_cluster::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(list_by_managed_cluster::ResponseBytesError)?;
                let rsp_value: CloudError = serde_json::from_slice(&body).context(list_by_managed_cluster::DeserializeError { body })?;
    ...
    pub async fn get(
        ...
        let uri_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ContainerService/managedClusters/{}/maintenanceConfigurations/{}",
            &operation_config.base_path, subscription_id, resource_group_name, resource_name, config_name
        );
        let mut req_builder = client.get(uri_str);
        if let Some(token_credential) = &operation_config.token_credential {
            let token_response = token_credential
