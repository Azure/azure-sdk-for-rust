...
pub mod iot_hub_resource {
    ...
    pub async fn get_endpoint_health(
        ...
        resource_group_name: &str,
        iot_hub_name: &str,
    ) -> Result<EndpointHealthDataListResult> {
        let client = &configuration.client;
        let uri_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Devices/IotHubs/{}/routingEndpointsHealth",
            &configuration.base_path, subscription_id, resource_group_name, iot_hub_name
        );
        let mut req_builder = client.get(uri_str);
        if let Some(token) = &configuration.bearer_access_token {
            req_builder = req_builder.bearer_auth(token);
...
pub mod resource_provider_common {
    use crate::{models::*, *};
    pub async fn get_subscription_quota(configuration: &Configuration, subscription_id: &str) -> Result<UserSubscriptionQuotaListResult> {
        let client = &configuration.client;
        let uri_str = &format!(
