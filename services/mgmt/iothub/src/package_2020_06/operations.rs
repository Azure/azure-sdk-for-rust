...
pub mod iot_hub_resource {
    ...
    pub async fn create_event_hub_consumer_group(
        ...
        req_builder = req_builder.query(&[("api-version", &configuration.api_version)]);
        req_builder = req_builder.json(consumer_group_body);
        let req = req_builder.build()?;
        let res = client.execute(req).await?;
        Ok(res.json().await?)
    }
...
pub mod private_link_resources {
    ...
    pub async fn get(
        ...
        resource_name: &str,
        group_id: &str,
    ) -> Result<GroupIdInformation> {
        let client = &configuration.client;
        let uri_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Devices/iotHubs/{}/privateLinkResources/{}",
            &configuration.base_path, subscription_id, resource_group_name, resource_name, group_id
        );
        let mut req_builder = client.get(uri_str);
        if let Some(token) = &configuration.bearer_access_token {
            req_builder = req_builder.bearer_auth(token);
...
pub mod private_endpoint_connections {
    ...
    pub async fn list(
        ...
        resource_group_name: &str,
        resource_name: &str,
    ) -> Result<PrivateEndpointConnectionsList> {
        let client = &configuration.client;
        let uri_str = &format!(
