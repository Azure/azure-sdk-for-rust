...
pub mod iot_hub_resource {
    ...
    pub async fn create_event_hub_consumer_group(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body =
            azure_core::to_json(consumer_group_body).map_err(|source| create_event_hub_consumer_group::Error::SerializeError { source })?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
...
pub mod iot_hub {
    ...
    pub async fn manual_failover(
        ...
        let mut url = url::Url::parse(url_str).map_err(|source| manual_failover::Error::ParseUrlError { source })?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(|source| manual_failover::Error::GetTokenError { source })?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(failover_input).map_err(|source| manual_failover::Error::SerializeError { source })?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
            .map_err(|source| manual_failover::Error::BuildRequestError { source })?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(|source| manual_failover::Error::ExecuteRequestError { source })?;
        match rsp.status() {
            ...
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: ErrorDetails =
                    serde_json::from_slice(rsp_body).map_err(|source| manual_failover::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
