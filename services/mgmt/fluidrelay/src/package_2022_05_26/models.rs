...
pub struct FluidRelayServerProperties {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub storagesku: Option<fluid_relay_server_properties::Storagesku>,
}
...
pub mod fluid_relay_server_properties {
    ...
    impl FromStr for Storagesku {
        type Err = value::Error;
        fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
            Self::deserialize(s.into_deserializer())
        }
    }
    impl<'de> Deserialize<'de> for Storagesku {
        fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
        where
            D: Deserializer<'de>,
        {
    ...
    impl Serialize for Storagesku {
        fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            match self {
                Self::Standard => serializer.serialize_unit_variant("Storagesku", 0u32, "standard"),
                Self::Basic => serializer.serialize_unit_variant("Storagesku", 1u32, "basic"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
