...
pub mod migrate_from_smart_detection {
    ...
    pub mod start_migration {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::MigrationStatusResponse> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::MigrationStatusResponse = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::MigrationStatusResponse>;
            type IntoFuture = futures::future::BoxFuture<'static, azure_core::Result<models::MigrationStatusResponse>>;
