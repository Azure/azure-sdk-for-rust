...
pub mod namespaces {
    ...
    pub mod get_ip_filter_rule {
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::IpFilterRule>;
            type IntoFuture = futures::future::BoxFuture<'static, azure_core::Result<models::IpFilterRule>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
...
pub mod regions {
    ...
    pub mod list_by_sku {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::MessagingRegionsListResult> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::MessagingRegionsListResult = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            pub fn into_stream(self) -> azure_core::Pageable<models::MessagingRegionsListResult, azure_core::error::Error> {
                let make_request = move |continuation: Option<String>| {
                    let this = self.clone();
                    async move {
                        let mut url = azure_core::Url::parse(&format!(
