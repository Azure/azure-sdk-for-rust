...
pub mod namespaces {
    ...
    pub async fn get_messaging_plan(
        ...
        namespace_name: &str,
        subscription_id: &str,
    ) -> std::result::Result<models::MessagingPlan, get_messaging_plan::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        let mut url = url::Url::parse(url_str).map_err(get_messaging_plan::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(get_messaging_plan::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(get_messaging_plan::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(get_messaging_plan::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::MessagingPlan = serde_json::from_slice(rsp_body)
                    .map_err(|source| get_messaging_plan::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
...
pub mod regions {
    ...
    pub async fn list_by_sku(
        ...
        subscription_id: &str,
        sku: &str,
    ) -> std::result::Result<models::MessagingRegionsListResult, list_by_sku::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
