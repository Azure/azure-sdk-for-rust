...
pub mod generate_detailed_cost_report {
    ...
    pub mod create_operation {
        ...
        impl RequestBuilder {
            ...
            pub fn into_future(
                self,
            ) -> futures::future::BoxFuture<'static, azure_core::Result<models::GenerateDetailedCostReportOperationResult>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
...
pub mod generate_detailed_cost_report_operation_status {
    ...
    pub mod get {
        ...
        impl RequestBuilder {
            ...
            pub fn into_future(
                self,
            ) -> futures::future::BoxFuture<'static, azure_core::Result<models::GenerateDetailedCostReportOperationStatuses>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
