...
pub mod machine_learning_compute {
    ...
    pub async fn system_update(
        ...
        req_builder = req_builder.query(&[("api-version", &operation_config.api_version)]);
        req_builder = req_builder.header(reqwest::header::CONTENT_LENGTH, 0);
        let req = req_builder.build().context(system_update::BuildRequestError)?;
        let rsp = client.execute(req).await.context(system_update::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => Ok(system_update::Response::Ok200),
            StatusCode::ACCEPTED => Ok(system_update::Response::Accepted202),
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(system_update::ResponseBytesError)?;
                let rsp_value: MachineLearningServiceError =
                    serde_json::from_slice(&body).context(system_update::DeserializeError { body })?;
