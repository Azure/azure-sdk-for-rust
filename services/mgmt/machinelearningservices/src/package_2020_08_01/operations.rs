        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: NotebookResourceInfo =
                    serde_json::from_slice(rsp_body).map_err(|source| prepare::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
...
pub mod virtual_machine_sizes {
    ...
    pub async fn list(
        ...
        if let Some(compute_type) = compute_type {
            url.query_pairs_mut().append_pair("compute-type", compute_type);
        }
        if let Some(recommended) = recommended {
            url.query_pairs_mut().append_pair("recommended", recommended.to_string().as_str());
        }
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
...
pub mod workspace_connections {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: WorkspaceConnection = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn stop(
        ...
        match rsp.status() {
            ...
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: MachineLearningServiceError =
                    serde_json::from_slice(rsp_body).map_err(|source| stop::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
