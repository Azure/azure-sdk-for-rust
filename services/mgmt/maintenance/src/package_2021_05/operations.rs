...
pub mod apply_updates {
        ...
        match rsp.status() {
            ...
            status_code => {
                ...
                let rsp_value: MaintenanceError =
                    serde_json::from_slice(rsp_body).map_err(|source| get_parent::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
    ...
    pub async fn list(
        operation_config: &crate::OperationConfig,
        subscription_id: &str,
    ) -> std::result::Result<ListApplyUpdate, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ListApplyUpdate = serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
...
pub mod maintenance_configurations {
    ...
    pub mod delete {
        ...
        #[derive(Debug)]
        pub enum Response {
            Ok200(MaintenanceConfiguration),
            NoContent204,
        }
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
