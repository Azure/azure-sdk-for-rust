...
pub mod web_services {
    ...
    pub mod get {
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                ...
                url.set_path(&format!(
                    ...
                    &self.subscription_id, &self.resource_group_name, &self.web_service_name
                ));
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::WebService>;
            type IntoFuture = BoxFuture<'static, azure_core::Result<models::WebService>>;
    ...
    pub mod list_keys {
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::WebServiceKeys>;
            type IntoFuture = BoxFuture<'static, azure_core::Result<models::WebServiceKeys>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
