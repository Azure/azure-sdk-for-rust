...
pub mod workspaces {
    ...
    pub async fn resync_storage_keys(
        ...
        let mut url = url::Url::parse(url_str).context(resync_storage_keys::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        ...
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(resync_storage_keys::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(resync_storage_keys::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => Ok(()),
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: ErrorResponse =
                    serde_json::from_slice(rsp_body).context(resync_storage_keys::DeserializeError { body: rsp_body.clone() })?;
    ...
    pub async fn list_workspace_keys(
        ...
        let mut url = url::Url::parse(url_str).context(list_workspace_keys::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        req_builder = req_builder.header(http::header::CONTENT_LENGTH, 0);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(list_workspace_keys::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(list_workspace_keys::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: WorkspaceKeysResponse =
                    serde_json::from_slice(rsp_body).context(list_workspace_keys::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
