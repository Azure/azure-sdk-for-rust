...
pub enum Error {
    ...
    #[error(transparent)]
    Workspaces_ResyncStorageKeys(#[from] workspaces::resync_storage_keys::Error),
    #[error(transparent)]
    Workspaces_ListWorkspaceKeys(#[from] workspaces::list_workspace_keys::Error),
    #[error(transparent)]
...
pub mod workspaces {
    ...
    ) -> std::result::Result<models::Workspace, create_or_update::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.MachineLearning/workspaces/{}",
            operation_config.base_path(),
    ...
    pub async fn resync_storage_keys(
        ...
        let mut url = url::Url::parse(url_str).map_err(resync_storage_keys::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        ...
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(resync_storage_keys::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        ...
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(resync_storage_keys::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(resync_storage_keys::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => Ok(()),
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: models::ErrorResponse = serde_json::from_slice(rsp_body)
                    .map_err(|source| resync_storage_keys::Error::DeserializeError(source, rsp_body.clone()))?;
                Err(resync_storage_keys::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
    ...
    pub async fn list_workspace_keys(
        ...
        workspace_name: &str,
        resource_group_name: &str,
    ) -> std::result::Result<models::WorkspaceKeysResponse, list_workspace_keys::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        let mut url = url::Url::parse(url_str).map_err(list_workspace_keys::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(list_workspace_keys::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.header(http::header::CONTENT_LENGTH, 0);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(list_workspace_keys::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(list_workspace_keys::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::WorkspaceKeysResponse = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_workspace_keys::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: models::ErrorResponse = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_workspace_keys::Error::DeserializeError(source, rsp_body.clone()))?;
                Err(list_workspace_keys::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
