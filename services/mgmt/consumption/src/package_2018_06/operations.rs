...
pub enum Error {
    ...
    #[error(transparent)]
    CostTags_Get(#[from] cost_tags::get::Error),
    #[error(transparent)]
    CostTags_CreateOrUpdate(#[from] cost_tags::create_or_update::Error),
    #[error(transparent)]
...
pub mod cost_tags {
    ...
    pub async fn get(
        operation_config: &crate::OperationConfig,
        billing_account_id: &str,
    ) -> std::result::Result<models::CostTag, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
    ...
    pub async fn create_or_update(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::CostTag = serde_json::from_slice(rsp_body)
                    .map_err(|source| create_or_update::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(create_or_update::Response::Ok200(rsp_value))
            }
            http::StatusCode::CREATED => {
    ...
    pub mod create_or_update {
        ...
        #[derive(Debug)]
        pub enum Response {
            Ok200(models::CostTag),
            Created201(models::CostTag),
        }
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
...
pub mod tags {
    ...
    pub async fn get(
        operation_config: &crate::OperationConfig,
        billing_account_id: &str,
    ) -> std::result::Result<models::TagsResult, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
