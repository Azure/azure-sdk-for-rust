...
pub mod workspaces {
    ...
    pub async fn list_by_accounts(
        ...
        account_name: &str,
        resource_group_name: &str,
    ) -> std::result::Result<models::WorkspaceListResult, list_by_accounts::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.MachineLearningExperimentation/accounts/{}/workspaces",
            operation_config.base_path(),
        ...
        let mut url = url::Url::parse(url_str).map_err(list_by_accounts::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(list_by_accounts::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(list_by_accounts::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(list_by_accounts::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::WorkspaceListResult = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_by_accounts::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: models::ErrorResponse = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_by_accounts::Error::DeserializeError(source, rsp_body.clone()))?;
                Err(list_by_accounts::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
...
pub mod projects {
    ...
    pub async fn list_by_workspace(
        ...
        workspace_name: &str,
        resource_group_name: &str,
    ) -> std::result::Result<models::ProjectListResult, list_by_workspace::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
