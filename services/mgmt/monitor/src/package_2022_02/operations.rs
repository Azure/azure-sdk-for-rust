...
pub enum Error {
    ...
    DataCollectionRuleAssociations_ListByDataCollectionEndpoint(
        #[from] data_collection_rule_associations::list_by_data_collection_endpoint::Error,
    ),
    #[error(transparent)]
...
pub mod action_groups {
    ...
    pub mod post_test_notifications {
        use super::models;
        type Response = models::TestNotificationResponse;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
            DefaultResponse {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<Response, Error>> {
                Box::pin({
                    ...
                    async move {
                        ...
                        url.query_pairs_mut().append_pair("api-version", "2021-09-01");
                        req_builder = req_builder.header("content-type", "application/json");
                        let req_body = azure_core::to_json(&this.notification_request).map_err(Error::Serialize)?;
                        req_builder = req_builder.uri(url.as_str());
                        let req = req_builder.body(req_body).map_err(Error::BuildRequest)?;
                        let rsp = this.client.send(req).await.map_err(Error::SendRequest)?;
                        let (rsp_status, rsp_headers, rsp_stream) = rsp.deconstruct();
    ...
    pub mod get_test_notifications {
        use super::models;
        type Response = models::TestNotificationDetailsResponse;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
            DefaultResponse {
                    ...
                    async move {
                        ...
                        let rsp = match continuation {
                            Some(token) => {
                                ...
                                let has_api_version_already = url.query_pairs().any(|(k, _)| k == "api-version");
                                if !has_api_version_already {
                                    url.query_pairs_mut().append_pair("api-version", "2019-10-17-preview");
                                }
                                req_builder = req_builder.uri(url.as_str());
                                req_builder = req_builder.method(http::Method::GET);
                                let credential = this.client.token_credential();
...
pub mod data_collection_rule_associations {
    ...
    impl Client {
        ...
        pub fn list_by_data_collection_endpoint(
            ...
            resource_group_name: impl Into<String>,
            data_collection_endpoint_name: impl Into<String>,
        ) -> list_by_data_collection_endpoint::Builder {
            list_by_data_collection_endpoint::Builder {
                client: self.0.clone(),
                subscription_id: subscription_id.into(),
                resource_group_name: resource_group_name.into(),
