...
pub mod metrics {
    ...
    pub mod list {
        use super::models;
        type Response = models::MetricCollection;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
...
pub mod service_diagnostic_settings {
    ...
    pub mod get {
        use super::models;
        type Response = models::ServiceDiagnosticSettingsResource;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
            DefaultResponse {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<Response, Error>> {
                Box::pin({
                    ...
                    async move {
                        ...
                        url.query_pairs_mut().append_pair("api-version", "2016-09-01");
                        req_builder = req_builder.header("content-type", "application/json");
                        let req_body = azure_core::to_json(&this.service_diagnostic_settings_resource).map_err(Error::Serialize)?;
                        req_builder = req_builder.uri(url.as_str());
                        let req = req_builder.body(req_body).map_err(Error::BuildRequest)?;
                        let rsp = this.client.send(req).await.map_err(Error::SendRequest)?;
                        let (rsp_status, rsp_headers, rsp_stream) = rsp.deconstruct();
