    ...
    pub async fn list(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: MetricCollection = serde_json::from_slice(&body).context(list::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(list::ResponseBytesError)?;
...
pub mod service_diagnostic_settings {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: ServiceDiagnosticSettingsResource = serde_json::from_slice(&body).context(get::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(get::ResponseBytesError)?;
    ...
    pub async fn update(
        ...
        req_builder = req_builder.query(&[("api-version", &operation_config.api_version)]);
        req_builder = req_builder.json(service_diagnostic_settings_resource);
