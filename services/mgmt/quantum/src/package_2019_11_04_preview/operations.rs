...
pub mod workspaces {
    ...
    pub async fn get(
        ...
        subscription_id: &str,
        workspace_name: &str,
    ) -> std::result::Result<QuantumWorkspace, get::Error> {
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: QuantumWorkspace = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn create_or_update(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(quantum_workspace).map_err(|source| create_or_update::Error::SerializeError { source })?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
        ...
        match rsp.status() {
            http::StatusCode::CREATED => {
                let rsp_body = rsp.body();
                let rsp_value: QuantumWorkspace =
    ...
    pub mod create_or_update {
        ...
        #[derive(Debug)]
        pub enum Response {
            Created201(QuantumWorkspace),
            Ok200(QuantumWorkspace),
        }
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("DefaultResponse")]
    ...
    pub async fn update_tags(
    ...
    ) -> std::result::Result<QuantumWorkspace, update_tags::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Quantum/workspaces/{}",
            operation_config.base_path(),
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(workspace_tags).map_err(|source| update_tags::Error::SerializeError { source })?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
...
pub mod offerings {
    ...
    pub async fn list(
        ...
        subscription_id: &str,
        location_name: &str,
    ) -> std::result::Result<OfferingsListResult, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: OfferingsListResult = serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
...
pub mod operations {
    ...
    pub async fn list(operation_config: &crate::OperationConfig) -> std::result::Result<OperationsList, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!("{}/providers/Microsoft.Quantum/operations", operation_config.base_path(),);
        let mut url = url::Url::parse(url_str).map_err(|source| list::Error::ParseUrlError { source })?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
