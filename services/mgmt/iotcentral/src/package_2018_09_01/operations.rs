...
pub mod apps {
    ...
    pub async fn check_subdomain_availability(
        ...
        let mut url = url::Url::parse(url_str).context(check_subdomain_availability::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(operation_inputs).context(check_subdomain_availability::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
            .context(check_subdomain_availability::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: AppAvailabilityInfo =
                    serde_json::from_slice(rsp_body).context(check_subdomain_availability::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
    ...
    pub async fn list_templates(
        ...
        let mut url = url::Url::parse(url_str).context(list_templates::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(list_templates::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(list_templates::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: AppTemplatesResult =
                    serde_json::from_slice(rsp_body).context(list_templates::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
