...
pub mod ea_subscription_list_migration_date {
    ...
    pub mod post {
        use super::models;
        type Response = models::EaSubscriptionMigrationDate;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
...
pub mod component_current_pricing_plan {
    ...
    pub mod get {
        use super::models;
        type Response = models::ApplicationInsightsComponentPricingPlan;
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("Unexpected HTTP status code {}", status_code)]
            UnexpectedResponse { status_code: http::StatusCode, body: bytes::Bytes },
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<Response, Error>> {
                Box::pin({
                    ...
                    async move {
                        ...
                        let req_body = azure_core::to_json(&this.pricing_plan_properties).map_err(Error::Serialize)?;
                        req_builder = req_builder.uri(url.as_str());
                        let req = req_builder.body(req_body).map_err(Error::BuildRequest)?;
                        let rsp = this.client.send(req).await.map_err(Error::SendRequest)?;
                        let (rsp_status, rsp_headers, rsp_stream) = rsp.deconstruct();
