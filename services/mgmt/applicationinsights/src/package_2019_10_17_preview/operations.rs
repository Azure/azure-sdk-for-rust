...
pub mod workbook_templates {
    ...
    pub async fn list_by_resource_group(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: WorkbookTemplatesListResult =
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: WorkbookTemplate = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn create_or_update(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body =
            azure_core::to_json(workbook_template_properties).map_err(|source| create_or_update::Error::SerializeError { source })?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
    ...
    pub async fn update(
        ...
        let req_body = if let Some(workbook_template_update_parameters) = workbook_template_update_parameters {
            azure_core::to_json(workbook_template_update_parameters).map_err(|source| update::Error::SerializeError { source })?
        } else {
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: WorkbookTemplate = serde_json::from_slice(rsp_body).map_err(|source| update::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
