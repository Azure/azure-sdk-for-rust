...
pub mod connected_cluster {
    ...
    pub async fn list_cluster_user_credential(
        ...
        resource_group_name: &str,
        cluster_name: &str,
        properties: &models::ListClusterUserCredentialProperties,
    ) -> std::result::Result<models::CredentialResults, list_cluster_user_credential::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        req_builder = req_builder.header("content-type", "application/json");
        let req_body = azure_core::to_json(properties).map_err(list_cluster_user_credential::Error::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder
            .body(req_body)
