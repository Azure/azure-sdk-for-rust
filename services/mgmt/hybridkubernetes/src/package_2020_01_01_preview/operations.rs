...
pub mod connected_cluster {
    ...
    pub async fn list_cluster_user_credentials(
        ...
        if let Some(client_proxy) = client_proxy {
            url.query_pairs_mut().append_pair("ClientProxy", client_proxy.to_string().as_str());
        }
        let req_body = if let Some(client_authentication_details) = client_authentication_details {
            azure_core::to_json(client_authentication_details)
                .map_err(|source| list_cluster_user_credentials::Error::SerializeError { source })?
        } else {
