...
pub mod looking_glass {
    ...
    pub async fn invoke(
        ...
        destination_ip: &str,
        subscription_id: &str,
    ) -> std::result::Result<LookingGlassOutput, invoke::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: LookingGlassOutput =
...
pub mod connection_monitor_tests {
    ...
    pub async fn get(
        ...
        subscription_id: &str,
    ) -> std::result::Result<ConnectionMonitorTest, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ConnectionMonitorTest =
                    serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
    ...
    pub async fn create_or_update(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ConnectionMonitorTest = serde_json::from_slice(rsp_body)
                    .map_err(|source| create_or_update::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(create_or_update::Response::Ok200(rsp_value))
            }
            http::StatusCode::CREATED => {
    ...
    pub mod create_or_update {
        ...
        #[derive(Debug)]
        pub enum Response {
            Ok200(ConnectionMonitorTest),
            Created201(ConnectionMonitorTest),
        }
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
    ...
    pub async fn list_by_peering_service(
        ...
        peering_service_name: &str,
        subscription_id: &str,
    ) -> std::result::Result<ConnectionMonitorTestListResult, list_by_peering_service::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Peering/peeringServices/{}/connectionMonitorTests",
            operation_config.base_path(),
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ConnectionMonitorTestListResult = serde_json::from_slice(rsp_body)
