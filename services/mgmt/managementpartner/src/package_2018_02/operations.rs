...
pub mod partner {
    ...
    pub async fn get(operation_config: &crate::OperationConfig, partner_id: &str) -> std::result::Result<PartnerResponse, get::Error> {
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: PartnerResponse = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn create(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: PartnerResponse = serde_json::from_slice(rsp_body).map_err(|source| create::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn update(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: PartnerResponse = serde_json::from_slice(rsp_body).map_err(|source| update::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
