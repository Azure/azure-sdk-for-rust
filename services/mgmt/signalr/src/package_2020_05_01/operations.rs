...
pub mod signal_r_private_endpoint_connections {
    ...
    pub async fn get(
        ...
        let uri_str = &format!(
            ...
            &configuration.base_path, subscription_id, resource_group_name, resource_name, private_endpoint_connection_name
        );
        let mut req_builder = client.get(uri_str);
        if let Some(token) = &configuration.bearer_access_token {
            req_builder = req_builder.bearer_auth(token);
...
pub mod signal_r_private_link_resources {
    ...
    pub async fn list(
        ...
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(list::ResponseBytesError)?;
                let rsp_value: PrivateLinkResourceList = serde_json::from_slice(&body).context(list::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(list::ResponseBytesError)?;
