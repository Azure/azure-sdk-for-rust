...
pub mod guest_usages {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: GuestUsagesResource = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn create(
        ...
        let req_body = if let Some(resource) = resource {
            azure_core::to_json(resource).map_err(|source| create::Error::SerializeError { source })?
        } else {
    ...
    pub async fn update(
        ...
        let req_body = if let Some(resource_patch) = resource_patch {
            azure_core::to_json(resource_patch).map_err(|source| update::Error::SerializeError { source })?
        } else {
            bytes::Bytes::from_static(azure_core::EMPTY_BODY)
        };
        req_builder = req_builder.uri(url.as_str());
