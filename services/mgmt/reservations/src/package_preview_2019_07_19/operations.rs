...
pub enum Error {
    ...
    #[error(transparent)]
    Reservation_Archive(#[from] reservation::archive::Error),
    #[error(transparent)]
    Reservation_Unarchive(#[from] reservation::unarchive::Error),
    #[error(transparent)]
...
pub mod reservation {
    ...
    pub mod available_scopes {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::Properties, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::Properties =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
