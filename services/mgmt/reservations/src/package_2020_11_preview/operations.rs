    ...
    pub async fn change_directory(
        ...
        if let Some(token_credential) = &operation_config.token_credential {
            let token_response = token_credential
                .get_token(&operation_config.token_credential_resource)
                .await
                .context(change_directory::GetTokenError)?;
            req_builder = req_builder.bearer_auth(token_response.token.secret());
        }
        req_builder = req_builder.query(&[("api-version", &operation_config.api_version)]);
        req_builder = req_builder.json(body);
        let req = req_builder.build().context(change_directory::BuildRequestError)?;
        let rsp = client.execute(req).await.context(change_directory::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(change_directory::ResponseBytesError)?;
                let rsp_value: ChangeDirectoryResponse =
                    serde_json::from_slice(&body).context(change_directory::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(change_directory::ResponseBytesError)?;
                let rsp_value: Error = serde_json::from_slice(&body).context(change_directory::DeserializeError { body })?;
                change_directory::DefaultResponse {
                    status_code,
                    value: rsp_value,
                }
                .fail()
