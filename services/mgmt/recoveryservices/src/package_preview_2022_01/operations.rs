...
pub enum Error {
    ...
    #[error(transparent)]
    RecoveryServices_Capabilities(#[from] recovery_services::capabilities::Error),
    #[error(transparent)]
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::VaultCertificateResponse, Error>> {
                Box::pin(async move {
                    ...
                    url.query_pairs_mut().append_pair("api-version", "2022-01-31-preview");
...
pub mod recovery_services {
    ...
    pub mod capabilities {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::CapabilitiesResponse, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::CapabilitiesResponse =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
