...
pub mod jobs {
    ...
    pub async fn mark_devices_shipped(
        operation_config: &crate::OperationConfig,
        job_name: &str,
        subscription_id: &str,
        resource_group_name: &str,
        mark_devices_shipped_request: &models::MarkDevicesShippedRequest,
    ) -> std::result::Result<(), mark_devices_shipped::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        let mut url = url::Url::parse(url_str).map_err(mark_devices_shipped::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        ...
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(mark_devices_shipped::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        req_builder = req_builder.header("content-type", "application/json");
        let req_body = azure_core::to_json(mark_devices_shipped_request).map_err(mark_devices_shipped::Error::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(mark_devices_shipped::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(mark_devices_shipped::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::NO_CONTENT => Ok(()),
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: models::ApiError = serde_json::from_slice(rsp_body)
                    .map_err(|source| mark_devices_shipped::Error::DeserializeError(source, rsp_body.clone()))?;
                Err(mark_devices_shipped::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
