...
pub mod jobs {
    ...
    pub async fn book_shipment_pick_up(
        ...
        match rsp.status() {
            ...
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(book_shipment_pick_up::ResponseBytesError)?;
                book_shipment_pick_up::UnexpectedResponse { status_code, body: body }.fail()
            }
        }
    }
...
pub mod service {
    ...
    pub async fn list_available_skus(
        ...
        let req = req_builder.build().context(list_available_skus::BuildRequestError)?;
        let rsp = client.execute(req).await.context(list_available_skus::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(list_available_skus::ResponseBytesError)?;
                let rsp_value: AvailableSkusResult =
                    serde_json::from_slice(&body).context(list_available_skus::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(list_available_skus::ResponseBytesError)?;
                list_available_skus::UnexpectedResponse { status_code, body: body }.fail()
            }
        }
    }
    ...
    pub async fn validate_address(
        ...
        match rsp.status() {
            ...
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(validate_address::ResponseBytesError)?;
                validate_address::UnexpectedResponse { status_code, body: body }.fail()
            }
        }
    }
