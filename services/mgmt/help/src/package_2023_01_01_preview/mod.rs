        ...
        impl RequestBuilder {
            pub fn into_stream(self) -> azure_core::Pageable<models::OperationListResult, azure_core::error::Error> {
                let make_request = move |continuation: Option<String>| {
                    let this = self.clone();
                    async move {
                        let mut url = azure_core::Url::parse(&format!("{}/providers/Microsoft.Help/operations", this.client.endpoint(),))?;
...
pub mod diagnostics {
    ...
    pub mod get {
                ...
                Box::pin({
                    ...
                    async move {
                        let url = azure_core::Url::parse(&format!(
                            ...
                            this.client.endpoint(),
                            &this.scope,
                            &this.diagnostics_resource_name
                        ))?;
                        let mut req = azure_core::Request::new(url, azure_core::Method::Get);
                        let credential = this.client.token_credential();
                        let token_response = credential.get_token(&this.client.scopes().join(" ")).await?;
        ...
        impl std::future::IntoFuture for RequestBuilder {
            ...
            type IntoFuture = futures::future::BoxFuture<'static, azure_core::Result<models::DiagnosticResource>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
