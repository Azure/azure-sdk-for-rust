...
pub mod management_group_subscriptions {
    ...
    pub async fn get_subscription(
        ...
        let req = req_builder.build().context(get_subscription::BuildRequestError)?;
        let rsp = client.execute(req).await.context(get_subscription::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(get_subscription::ResponseBytesError)?;
                let rsp_value: SubscriptionUnderManagementGroup =
                    serde_json::from_slice(&body).context(get_subscription::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(get_subscription::ResponseBytesError)?;
                let rsp_value: ErrorResponse = serde_json::from_slice(&body).context(get_subscription::DeserializeError { body })?;
        ...
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp
                    .bytes()
                    .await
                    .context(get_subscriptions_under_management_group::ResponseBytesError)?;
                let rsp_value: ListSubscriptionUnderManagementGroup =
                    serde_json::from_slice(&body).context(get_subscriptions_under_management_group::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp
