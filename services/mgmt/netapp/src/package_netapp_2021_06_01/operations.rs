...
pub mod net_app_resource_quota_limits {
    ...
    pub async fn list(
        ...
        subscription_id: &str,
        location: &str,
    ) -> std::result::Result<SubscriptionQuotaItemList, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: SubscriptionQuotaItemList =
                    serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => Err(list::Error::DefaultResponse { status_code }),
    ...
    pub async fn get(
    ...
    ) -> std::result::Result<SubscriptionQuotaItem, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/providers/Microsoft.NetApp/locations/{}/quotaLimits/{}",
            operation_config.base_path(),
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: SubscriptionQuotaItem =
