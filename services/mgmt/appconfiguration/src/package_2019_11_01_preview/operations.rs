...
pub mod configuration_stores {
    ...
    ) -> std::result::Result<models::ConfigurationStore, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.AppConfiguration/configurationStores/{}",
            operation_config.base_path(),
    ...
    ) -> std::result::Result<models::ApiKey, regenerate_key::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
    ...
    pub async fn list_key_value(
    ...
    ) -> std::result::Result<models::KeyValue, list_key_value::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        let mut url = url::Url::parse(url_str).map_err(list_key_value::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(list_key_value::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        req_builder = req_builder.header("content-type", "application/json");
        let req_body = azure_core::to_json(list_key_value_parameters).map_err(list_key_value::Error::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).map_err(list_key_value::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(list_key_value::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::KeyValue =
                    serde_json::from_slice(rsp_body).map_err(|source| list_key_value::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: models::Error =
                    serde_json::from_slice(rsp_body).map_err(|source| list_key_value::Error::DeserializeError(source, rsp_body.clone()))?;
                Err(list_key_value::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
...
pub mod operations {
    ...
    ) -> std::result::Result<models::NameAvailabilityStatus, check_name_availability::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
...
pub mod private_endpoint_connections {
    ...
    pub async fn list_by_configuration_store(
        ...
        resource_group_name: &str,
        config_store_name: &str,
    ) -> std::result::Result<models::PrivateEndpointConnectionListResult, list_by_configuration_store::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
...
pub mod private_link_resources {
    ...
    pub async fn list_by_configuration_store(
        ...
        resource_group_name: &str,
        config_store_name: &str,
    ) -> std::result::Result<models::PrivateLinkResourceListResult, list_by_configuration_store::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
