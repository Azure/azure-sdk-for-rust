...
#[doc = "Indicates if the image template needs to be built on create/update"]
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, Default)]
pub struct ImageTemplateAutoRun {
    #[doc = "Enabling this field will trigger an automatic build on image template creation or update."]
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub state: Option<image_template_auto_run::State>,
}
impl ImageTemplateAutoRun {
    pub fn new() -> Self {
        Self::default()
    }
}
pub mod image_template_auto_run {
    use super::*;
    #[doc = "Enabling this field will trigger an automatic build on image template creation or update."]
    #[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
    pub enum State {
...
pub struct ImageTemplateProperties {
    ...
    #[doc = "Indicates if the image template needs to be built on create/update"]
    #[serde(rename = "autoRun", default, skip_serializing_if = "Option::is_none")]
    pub auto_run: Option<ImageTemplateAutoRun>,
    #[doc = "Tags that will be applied to the resource group and/or resources created by the service."]
    #[serde(rename = "managedResourceTags", default, skip_serializing_if = "Option::is_none")]
    pub managed_resource_tags: Option<serde_json::Value>,
}
...
pub struct VirtualNetworkConfig {
    #[doc = "Resource id of a pre-existing subnet on which the build VM and validation VM will be deployed"]
    #[serde(rename = "subnetId", default, skip_serializing_if = "Option::is_none")]
    pub subnet_id: Option<String>,
    #[doc = "Resource id of a pre-existing subnet on which Azure Container Instance will be deployed for Isolated Builds. This field may be specified only if `subnetId` is also specified and must be on the same Virtual Network as the subnet specified in `subnetId`."]
    #[serde(rename = "containerInstanceSubnetId", default, skip_serializing_if = "Option::is_none")]
    pub container_instance_subnet_id: Option<String>,
    #[doc = "Size of the proxy virtual machine used to pass traffic to the build VM and validation VM. This must not be specified if `containerInstanceSubnetId` is specified because no proxy virtual machine is deployed in that case. Omit or specify empty string to use the default (Standard_A1_v2)."]
