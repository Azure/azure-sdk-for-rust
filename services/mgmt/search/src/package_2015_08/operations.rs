...
pub mod query_keys {
    ...
    pub async fn list_by_search_service_get(
        ...
        let mut url = url::Url::parse(url_str).map_err(|source| list_by_search_service_get::Error::ParseUrlError { source })?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(|source| list_by_search_service_get::Error::GetTokenError { source })?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        if let Some(x_ms_client_request_id) = x_ms_client_request_id {
        ...
        let req = req_builder
            .body(req_body)
            .map_err(|source| list_by_search_service_get::Error::BuildRequestError { source })?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(|source| list_by_search_service_get::Error::ExecuteRequestError { source })?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ListQueryKeysResult =
                    serde_json::from_slice(rsp_body).map_err(|source| list_by_search_service_get::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
                Ok(rsp_value)
