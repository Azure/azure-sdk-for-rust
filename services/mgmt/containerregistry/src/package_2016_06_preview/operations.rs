...
pub enum Error {
    ...
    #[error(transparent)]
    Registries_GetProperties(#[from] registries::get_properties::Error),
    #[error(transparent)]
    Registries_CreateOrUpdate(#[from] registries::create_or_update::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    Registries_GetCredentials(#[from] registries::get_credentials::Error),
    #[error(transparent)]
    Registries_RegenerateCredentials(#[from] registries::regenerate_credentials::Error),
}
pub mod registries {
    use super::{models, API_VERSION};
    ...
    pub async fn get_properties(
        ...
        registry_name: &str,
    ) -> std::result::Result<models::Registry, get_properties::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
    ...
    pub async fn create_or_update(
        ...
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        req_builder = req_builder.header("content-type", "application/json");
        let req_body = azure_core::to_json(registry).map_err(create_or_update::Error::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
    ...
    pub async fn get_credentials(
        ...
        resource_group_name: &str,
        registry_name: &str,
    ) -> std::result::Result<models::RegistryCredentials, get_credentials::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::RegistryCredentials = serde_json::from_slice(rsp_body)
    ...
    pub async fn regenerate_credentials(
        ...
        resource_group_name: &str,
        registry_name: &str,
    ) -> std::result::Result<models::RegistryCredentials, regenerate_credentials::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        let mut url = url::Url::parse(url_str).map_err(regenerate_credentials::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(regenerate_credentials::Error::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", super::API_VERSION);
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        ...
        let req = req_builder
            .body(req_body)
            .map_err(regenerate_credentials::Error::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(regenerate_credentials::Error::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: models::RegistryCredentials = serde_json::from_slice(rsp_body)
                    .map_err(|source| regenerate_credentials::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
