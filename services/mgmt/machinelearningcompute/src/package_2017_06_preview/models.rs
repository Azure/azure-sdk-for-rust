...
impl FromStr for SystemServices {
    type Err = value::Error;
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        Self::deserialize(s.into_deserializer())
    }
}
impl<'de> Deserialize<'de> for SystemServices {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
...
impl Serialize for SystemServices {
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match self {
            Self::Scoring => serializer.serialize_unit_variant("SystemServices", 0u32, "Scoring"),
            Self::Batch => serializer.serialize_unit_variant("SystemServices", 1u32, "Batch"),
            Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
        }
    }
}
...
pub mod update_system_response {
    ...
    impl Serialize for UpdateStatus {
        ...
        {
            match self {
                Self::InProgress => serializer.serialize_unit_variant("UpdateStatus", 0u32, "InProgress"),
                Self::Completed => serializer.serialize_unit_variant("UpdateStatus", 1u32, "Completed"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
