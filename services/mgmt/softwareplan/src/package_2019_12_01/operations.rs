    ...
    pub async fn register(operation_config: &crate::OperationConfig, subscription_id: &str) -> std::result::Result<(), register::Error> {
        ...
        match rsp.status() {
            ...
            status_code => {
                ...
                let rsp_value: Error = serde_json::from_slice(rsp_body).map_err(|source| register::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
...
pub mod hybrid_use_benefit {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: HybridUseBenefitModel = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
