...
pub struct VirtualNetworkLinkProperties {
    ...
    #[doc = "The resolution policy on the virtual network link. Only applicable for virtual network links to privatelink zones, and for A,AAAA,CNAME queries. When set to 'NxDomainRedirect', Azure DNS resolver falls back to public resolution if private dns query resolution results in non-existent domain response."]
    #[serde(rename = "resolutionPolicy", default, skip_serializing_if = "Option::is_none")]
    pub resolution_policy: Option<virtual_network_link_properties::ResolutionPolicy>,
...
pub mod virtual_network_link_properties {
    ...
    impl<'de> Deserialize<'de> for ResolutionPolicy {
        fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
        where
            D: Deserializer<'de>,
        {
    ...
    impl Serialize for ResolutionPolicy {
        fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            match self {
                Self::Default => serializer.serialize_unit_variant("ResolutionPolicy", 0u32, "Default"),
                Self::NxDomainRedirect => serializer.serialize_unit_variant("ResolutionPolicy", 1u32, "NxDomainRedirect"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
