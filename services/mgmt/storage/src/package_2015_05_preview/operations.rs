    ...
    pub async fn list_keys(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: StorageAccountKeys = serde_json::from_slice(&body).context(list_keys::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(list_keys::ResponseBytesError)?;
    ...
    pub async fn regenerate_key(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: StorageAccountKeys = serde_json::from_slice(&body).context(regenerate_key::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(regenerate_key::ResponseBytesError)?;
