...
pub mod workspaces {
    ...
    pub mod delete {
        ...
        pub struct RequestBuilder {
            ...
            pub(crate) workspace_name: String,
            pub(crate) subscription_id: String,
            pub(crate) retain_uc_data: Option<bool>,
        }
        impl RequestBuilder {
            ...
            pub fn retain_uc_data(mut self, retain_uc_data: bool) -> Self {
                self.retain_uc_data = Some(retain_uc_data);
                self
            }
            #[doc = "Returns a future that sends the request and returns a [`Response`] object that provides low-level access to full response details."]
            #[doc = ""]
            ...
            pub fn send(self) -> BoxFuture<'static, azure_core::Result<Response>> {
                Box::pin({
                    ...
                    async move {
                        ...
                        let bearer_token = this.client.bearer_token().await?;
                        req.insert_header(azure_core::headers::AUTHORIZATION, format!("Bearer {}", bearer_token.secret()));
                        if let Some(retain_uc_data) = &this.retain_uc_data {
                            req.url_mut()
                                .query_pairs_mut()
                                .append_pair("retainUcData", &retain_uc_data.to_string());
                        }
                        let req_body = azure_core::EMPTY_BODY;
                        req.set_body(req_body);
                        Ok(Response(this.client.send(&mut req).await?))
