...
pub mod enrollment_accounts {
    ...
    pub async fn get(configuration: &crate::Configuration, name: &str) -> std::result::Result<EnrollmentAccount, get::Error> {
        let client = &configuration.client;
        let uri_str = &format!(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: EnrollmentAccount = serde_json::from_slice(&body).context(get::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(get::ResponseBytesError)?;
