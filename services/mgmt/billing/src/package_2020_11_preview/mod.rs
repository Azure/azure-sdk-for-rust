...
pub mod promotion {
    ...
    pub mod get {
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                let mut url = self.client.endpoint().clone();
                url.set_path(&format!("/providers/Microsoft.Billing/promotions/{}", &self.promotion_id));
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                ...
                url.set_path(&format!(
                    ...
                    &self.subscription_id, &self.promotion_sku_id
                ));
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
...
pub mod promotions {
    ...
    pub mod list {
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                let mut url = self.client.endpoint().clone();
                url.set_path(&format!("/providers/Microsoft.Billing/promotions",));
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
                        .append_pair(azure_core::query_param::API_VERSION, "2020-11-01-preview");
