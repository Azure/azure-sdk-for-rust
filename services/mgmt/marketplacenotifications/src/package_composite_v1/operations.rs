                    ...
                    async move {
                        ...
                        let rsp = match continuation {
                            ...
                            None => {
                                ...
                                req_builder =
                                    req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
                                url.query_pairs_mut().append_pair("api-version", "2021-03-03");
                                let principal_id = &this.principal_id;
                        ...
                        match rsp_status {
                            http::StatusCode::OK => {
                                ...
                                let rsp_value: models::NotificationList = serde_json::from_slice(&rsp_body)?;
                                Ok(rsp_value)
                            }
        ...
        impl Builder {
            pub fn into_stream(self) -> azure_core::Pageable<Response, azure_core::error::Error> {
                let make_request = move |continuation: Option<azure_core::prelude::Continuation>| {
                    let this = self.clone();
                    async move {
                        let url_str = &format!("{}/providers/Microsoft.MarketplaceNotifications/operations", this.client.endpoint(),);
                        let mut url = url::Url::parse(url_str).context(azure_core::error::ErrorKind::Other, "build request")?;
                        let mut req_builder = http::request::Builder::new();
                        let rsp = match continuation {
                            Some(token) => {
