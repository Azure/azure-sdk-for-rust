...
pub mod cluster_extension_type {
    ...
    ) -> std::result::Result<ExtensionType, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
...
pub mod cluster_extension_types {
    ...
    pub async fn list(
        ...
        cluster_rp: &str,
        cluster_name: &str,
    ) -> std::result::Result<ExtensionTypeList, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = & format ! ("{}/subscriptions/{}/resourceGroups/{}/providers/{}/connectedClusters/{}/providers/Microsoft.KubernetesConfiguration/extensionTypes" , operation_config . base_path () , subscription_id , resource_group_name , cluster_rp , cluster_name) ;
        let mut url = url::Url::parse(url_str).map_err(list::Error::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ExtensionTypeList =
                    serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
...
pub mod extension_type_versions {
    ...
    pub async fn list(
        ...
        location: &str,
        extension_type_name: &str,
    ) -> std::result::Result<ExtensionVersionList, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ExtensionVersionList =
                    serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
