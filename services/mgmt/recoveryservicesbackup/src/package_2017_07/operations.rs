    ...
    pub async fn get(
        ...
        match rsp.status() {
            StatusCode::OK => {
                ...
                let rsp_value: AadPropertiesResource = serde_json::from_slice(&body).context(get::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(get::ResponseBytesError)?;
...
pub mod crr_operation_results {
    ...
    pub async fn get(
        ...
        let uri_str = &format!(
            ...
            &operation_config.base_path, subscription_id, azure_region, operation_id
        );
        let mut req_builder = client.get(uri_str);
        if let Some(token) = &operation_config.bearer_access_token {
            req_builder = req_builder.bearer_auth(token);
...
pub mod recovery_points {
    ...
    pub async fn get_access_token(
        ...
        req_builder = req_builder.query(&[("api-version", &operation_config.api_version)]);
        let req = req_builder.build().context(get_access_token::BuildRequestError)?;
        let rsp = client.execute(req).await.context(get_access_token::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(get_access_token::ResponseBytesError)?;
                let rsp_value: CrrAccessTokenResource =
                    serde_json::from_slice(&body).context(get_access_token::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(get_access_token::ResponseBytesError)?;
                get_access_token::UnexpectedResponse { status_code, body: body }.fail()
            }
        }
    }
