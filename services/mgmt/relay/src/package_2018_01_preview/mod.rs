...
pub mod operation_status_private_endpoint_connections {
    ...
    pub mod get {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::OperationStatusItem> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::OperationStatusItem = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                let mut url = self.client.endpoint().clone();
                url . set_path (& format ! ("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Relay/namespaces/{}/privateEndpointConnections/{}/operationStatus/{}" , & self . subscription_id , & self . resource_group_name , & self . namespace_name , & self . private_endpoint_connection_name , & self . private_endpoint_connection_name)) ;
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
                        .append_pair(azure_core::query_param::API_VERSION, "2018-01-01-preview");
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::OperationStatusItem>;
            type IntoFuture = BoxFuture<'static, azure_core::Result<models::OperationStatusItem>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
...
impl Client {
    ...
    pub fn private_link_resources_get(
        ...
        namespace_name: impl Into<String>,
        subscription_id: impl Into<String>,
    ) -> private_link_resources_get::RequestBuilder {
        private_link_resources_get::RequestBuilder {
            client: self.clone(),
            resource_group_name: resource_group_name.into(),
            namespace_name: namespace_name.into(),
