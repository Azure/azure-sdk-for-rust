...
pub mod budget {
    ...
    pub async fn get(
        ...
        scope: &str,
        budget_name: &str,
    ) -> std::result::Result<BudgetModel, get::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/{}/providers/Microsoft.CostManagement/budgets/{}",
            operation_config.base_path(),
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: BudgetModel = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
