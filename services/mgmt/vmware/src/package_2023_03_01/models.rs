...
impl Serialize for ClusterProvisioningState {
        ...
        S: Serializer,
    {
        match self {
            ...
            Self::Canceled => serializer.serialize_unit_variant("ClusterProvisioningState", 5u32, "Canceled"),
            Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
        }
    }
}
...
pub mod private_cloud_properties {
    ...
    #[doc = "Flag to indicate whether the private cloud has the quota for provisioned NSX Public IP count raised from 64 to 1024"]
    #[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
    #[serde(remote = "NsxPublicIpQuotaRaised")]
    pub enum NsxPublicIpQuotaRaised {
        Enabled,
        Disabled,
        #[serde(skip_deserializing)]
        UnknownValue(String),
    }
    impl FromStr for NsxPublicIpQuotaRaised {
        type Err = value::Error;
        fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
            Self::deserialize(s.into_deserializer())
        }
    }
    impl<'de> Deserialize<'de> for NsxPublicIpQuotaRaised {
        fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
        where
            D: Deserializer<'de>,
        {
    ...
    impl Serialize for NsxPublicIpQuotaRaised {
        fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            match self {
                Self::Enabled => serializer.serialize_unit_variant("NsxPublicIpQuotaRaised", 0u32, "Enabled"),
                Self::Disabled => serializer.serialize_unit_variant("NsxPublicIpQuotaRaised", 1u32, "Disabled"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
...
pub struct PrivateCloudUpdateProperties {
    ...
    #[doc = "Array of additional networks noncontiguous with networkBlock. Networks must be unique and non-overlapping across VNet in your subscription, on-premise, and this privateCloud networkBlock attribute. Make sure the CIDR format conforms to (A.B.C.D/X)."]
    #[serde(
