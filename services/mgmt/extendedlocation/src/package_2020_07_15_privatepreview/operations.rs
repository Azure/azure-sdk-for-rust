...
pub mod custom_locations {
    ...
    pub async fn get_enabled_resource_type(
        operation_config: &crate::OperationConfig,
        subscription_id: &str,
        resource_group_name: &str,
        resource_name: &str,
        enabled_resource_type_name: &str,
    ) -> std::result::Result<EnabledResourceType, get_enabled_resource_type::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ExtendedLocation/customLocations/{}/enabledResourceTypes/{}",
            operation_config.base_path(),
            ...
            resource_group_name,
            resource_name,
            enabled_resource_type_name
        );
        let mut url = url::Url::parse(url_str).map_err(|source| get_enabled_resource_type::Error::ParseUrlError { source })?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(|source| get_enabled_resource_type::Error::GetTokenError { source })?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        ...
        let req = req_builder
            .body(req_body)
            .map_err(|source| get_enabled_resource_type::Error::BuildRequestError { source })?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(|source| get_enabled_resource_type::Error::ExecuteRequestError { source })?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: EnabledResourceType =
                    serde_json::from_slice(rsp_body).map_err(|source| get_enabled_resource_type::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
                Ok(rsp_value)
            ...
            status_code => {
                ...
                Err(get_enabled_resource_type::Error::DefaultResponse {
                    status_code,
                    value: rsp_value,
                })
            }
    ...
    pub mod get_enabled_resource_type {
        use crate::{models, models::*};
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
