...
pub mod event_hub_connections {
    ...
    pub async fn eventhub_connection_validation(
        ...
        let mut url = url::Url::parse(url_str).context(eventhub_connection_validation::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        let req = req_builder
            .body(req_body)
            .context(eventhub_connection_validation::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: EventHubConnectionValidationListResult = serde_json::from_slice(rsp_body)
                    .context(eventhub_connection_validation::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                eventhub_connection_validation::UnexpectedResponse {
                    status_code,
                    body: rsp_body.clone(),
                }
                .fail()
