...
pub mod start_container {
    ...
    pub async fn launch_exec(
        ...
        let mut url = url::Url::parse(url_str).context(launch_exec::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        ...
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(launch_exec::BuildRequestError)?;
        let rsp = http_client.execute_request(req).await.context(launch_exec::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ContainerExecResponse =
                    serde_json::from_slice(rsp_body).context(launch_exec::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                launch_exec::UnexpectedResponse {
                    status_code,
                    body: rsp_body.clone(),
                }
                .fail()
