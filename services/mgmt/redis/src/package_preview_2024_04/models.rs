...
pub struct RedisCommonProperties {
    #[doc = "All Redis Settings. Few possible keys: rdb-backup-enabled,rdb-storage-connection-string,rdb-backup-frequency,maxmemory-delta, maxmemory-policy,notify-keyspace-events, aof-backup-enabled, aof-storage-connection-string-0, aof-storage-connection-string-1 etc."]
    ...
    #[doc = "Authentication to Redis through access keys is disabled when set as true. Default value is false."]
    #[serde(rename = "disableAccessKeyAuthentication", default, skip_serializing_if = "Option::is_none")]
    pub disable_access_key_authentication: Option<bool>,
    #[doc = "Optional: Specifies how availability zones are allocated to the Redis cache. 'Automatic' enables zone redundancy and Azure will select zones based on regional availability and capacity. 'UserDefined' will select availability zones passed in by you using the 'zones' parameter. 'NoZones' will produce a non-zonal cache. If 'zonalAllocation' is not passed, it will be set to 'UserDefined' when zones are passed in, otherwise, will be set to 'NoZones'"]
    #[serde(rename = "zonalAllocationPolicy", default, skip_serializing_if = "Option::is_none")]
    pub zonal_allocation_policy: Option<redis_common_properties::ZonalAllocationPolicy>,
}
...
pub mod redis_common_properties {
    ...
    #[doc = "Optional: Specifies how availability zones are allocated to the Redis cache. 'Automatic' enables zone redundancy and Azure will select zones based on regional availability and capacity. 'UserDefined' will select availability zones passed in by you using the 'zones' parameter. 'NoZones' will produce a non-zonal cache. If 'zonalAllocation' is not passed, it will be set to 'UserDefined' when zones are passed in, otherwise, will be set to 'NoZones'"]
    #[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
    #[serde(remote = "ZonalAllocationPolicy")]
    pub enum ZonalAllocationPolicy {
        Automatic,
        UserDefined,
        NoZones,
        #[serde(skip_deserializing)]
    ...
    impl FromStr for ZonalAllocationPolicy {
        type Err = value::Error;
        fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
            Self::deserialize(s.into_deserializer())
        }
    }
    impl<'de> Deserialize<'de> for ZonalAllocationPolicy {
        fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
        where
            D: Deserializer<'de>,
        {
    ...
    impl Serialize for ZonalAllocationPolicy {
        fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            match self {
                Self::Automatic => serializer.serialize_unit_variant("ZonalAllocationPolicy", 0u32, "Automatic"),
                Self::UserDefined => serializer.serialize_unit_variant("ZonalAllocationPolicy", 1u32, "UserDefined"),
                Self::NoZones => serializer.serialize_unit_variant("ZonalAllocationPolicy", 2u32, "NoZones"),
                Self::UnknownValue(s) => serializer.serialize_str(s.as_str()),
            }
        }
    }
