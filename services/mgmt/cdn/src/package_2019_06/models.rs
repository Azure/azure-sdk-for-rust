...
pub struct UrlRewriteActionParameters {
    ...
    #[doc = "define a request URI pattern that identifies the type of requests that may be rewritten. Currently, source pattern uses a prefix-based match. To match all URL paths, use \"/\" as the source pattern value. To match only the root directory and re-write this path, use the origin path field"]
    #[serde(rename = "sourcePattern")]
    pub source_pattern: String,
    #[doc = "Define the destination path for be used in the rewrite. This will overwrite the source pattern "]
    pub destination: String,
    #[doc = "If True, the remaining path after the source pattern will be appended to the new destination path.  "]
