...
pub mod manage_inventory_metadata {
    ...
    impl RequestBuilder {
        ...
        fn url(&self) -> azure_core::Result<azure_core::Url> {
            let mut url = self.client.endpoint().clone();
            url . set_path (& format ! ("/subscriptions/{}/providers/Microsoft.EdgeOrderPartner/locations/{}/productFamilies/{}/inventories/{}/manageInventoryMetadata" , & self . subscription_id , & self . location , & self . family_identifier , & self . serial_number)) ;
            let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
            if !has_api_version_already {
                url.query_pairs_mut()
                    .append_pair(azure_core::query_param::API_VERSION, "2020-12-01-preview");
    ...
    impl RequestBuilder {
        ...
        fn url(&self) -> azure_core::Result<azure_core::Url> {
            ...
            url.set_path(&format!(
                ...
                &self.subscription_id, &self.location, &self.family_identifier, &self.serial_number
            ));
            let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
            if !has_api_version_already {
                url.query_pairs_mut()
...
pub mod search_inventories {
    ...
    pub struct Response(azure_core::Response);
    impl Response {
        pub async fn into_body(self) -> azure_core::Result<models::PartnerInventoryList> {
            let bytes = self.0.into_body().collect().await?;
            let body: models::PartnerInventoryList = serde_json::from_slice(&bytes)?;
            Ok(body)
        }
        pub fn into_raw_response(self) -> azure_core::Response {
            self.0
    ...
    impl RequestBuilder {
        pub fn into_stream(self) -> azure_core::Pageable<models::PartnerInventoryList, azure_core::error::Error> {
            let make_request = move |continuation: Option<String>| {
                let this = self.clone();
                async move {
                    let mut url = this.url()?;
