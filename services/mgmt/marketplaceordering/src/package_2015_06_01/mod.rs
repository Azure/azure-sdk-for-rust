...
pub mod marketplace_agreements {
    ...
    pub mod sign {
        ...
        impl RequestBuilder {
            ...
            #[doc = "Send the request and return the response body."]
            pub fn into_future(self) -> futures::future::BoxFuture<'static, azure_core::Result<models::OldAgreementTerms>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
    ...
    pub mod list {
        ...
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::AgreementTermsList> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::AgreementTermsList = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl RequestBuilder {
            ...
            #[doc = "Send the request and return the response body."]
            pub fn into_future(self) -> futures::future::BoxFuture<'static, azure_core::Result<models::AgreementTermsList>> {
                Box::pin(async move { self.send().await?.into_body().await })
            }
        }
    }
