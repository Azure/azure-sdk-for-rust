...
pub mod template_specs {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: TemplateSpec = serde_json::from_slice(rsp_body).context(get::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
    ...
    pub async fn create_or_update(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(template_spec).context(create_or_update::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
    ...
    pub async fn update(
        ...
        let req_body = if let Some(template_spec) = template_spec {
            azure_core::to_json(template_spec).context(update::SerializeError)?
        } else {
...
pub mod template_spec_versions {
    ...
    pub async fn create_or_update(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(template_spec_version_model).context(create_or_update::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(create_or_update::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
    ...
    pub async fn update(
        ...
        let req_body = if let Some(template_spec_version_update_model) = template_spec_version_update_model {
            azure_core::to_json(template_spec_version_update_model).context(update::SerializeError)?
        } else {
            bytes::Bytes::from_static(azure_core::EMPTY_BODY)
        };
        req_builder = req_builder.uri(url.as_str());
