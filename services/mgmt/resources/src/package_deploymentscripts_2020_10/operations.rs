...
pub mod deployment_scripts {
    ...
    pub async fn create(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = azure_core::to_json(deployment_script).context(create::SerializeError)?;
        req_builder = req_builder.uri(url.as_str());
    ...
    pub async fn update(
        ...
        let req_body = if let Some(deployment_script) = deployment_script {
            azure_core::to_json(deployment_script).context(update::SerializeError)?
        } else {
    ...
    pub async fn get_logs(
        ...
        let mut url = url::Url::parse(url_str).context(get_logs::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(get_logs::BuildRequestError)?;
        let rsp = http_client.execute_request(req).await.context(get_logs::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ScriptLogsList =
                    serde_json::from_slice(rsp_body).context(get_logs::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
    ...
    pub async fn get_logs_default(
        ...
        let mut url = url::Url::parse(url_str).context(get_logs_default::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::GET);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
        ...
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(get_logs_default::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(get_logs_default::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ScriptLog =
                    serde_json::from_slice(rsp_body).context(get_logs_default::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
