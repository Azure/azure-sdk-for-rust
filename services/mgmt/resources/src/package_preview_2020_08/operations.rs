...
pub mod applications {
    ...
    pub async fn update_by_id(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = if let Some(parameters) = parameters {
            azure_core::to_json(parameters).context(update_by_id::SerializeError)?
        } else {
            bytes::Bytes::from_static(azure_core::EMPTY_BODY)
        };
        req_builder = req_builder.uri(url.as_str());
    ...
    pub async fn refresh_permissions(
        ...
        let mut url = url::Url::parse(url_str).context(refresh_permissions::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        ...
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(refresh_permissions::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(refresh_permissions::ExecuteRequestError)?;
        match rsp.status() {
            ...
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: CloudError =
                    serde_json::from_slice(rsp_body).context(refresh_permissions::DeserializeError { body: rsp_body.clone() })?;
