...
pub async fn list_operations(
    ...
    match rsp.status() {
        ...
        status_code => {
            let rsp_body = rsp.body();
            let rsp_value: CloudError = serde_json::from_slice(rsp_body).map_err(|source| list_operations::Error::DeserializeError {
                source,
                body: rsp_body.clone(),
            })?;
...
pub mod applications {
    ...
    pub async fn get_by_id(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: Application = serde_json::from_slice(rsp_body).map_err(|source| get_by_id::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn update_by_id(
        ...
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = if let Some(parameters) = parameters {
            azure_core::to_json(parameters).map_err(|source| update_by_id::Error::SerializeError { source })?
        } else {
            bytes::Bytes::from_static(azure_core::EMPTY_BODY)
        };
        req_builder = req_builder.uri(url.as_str());
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: Application = serde_json::from_slice(rsp_body).map_err(|source| update_by_id::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
    ...
    pub async fn refresh_permissions(
        ...
        let mut url = url::Url::parse(url_str).map_err(|source| refresh_permissions::Error::ParseUrlError { source })?;
        let mut req_builder = http::request::Builder::new();
        ...
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .map_err(|source| refresh_permissions::Error::GetTokenError { source })?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        ...
        let req = req_builder
            .body(req_body)
            .map_err(|source| refresh_permissions::Error::BuildRequestError { source })?;
        let rsp = http_client
            .execute_request(req)
            .await
            .map_err(|source| refresh_permissions::Error::ExecuteRequestError { source })?;
        match rsp.status() {
            ...
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: CloudError =
                    serde_json::from_slice(rsp_body).map_err(|source| refresh_permissions::Error::DeserializeError {
                        source,
                        body: rsp_body.clone(),
                    })?;
...
pub mod application_definitions {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ApplicationDefinition = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
...
pub mod jit_requests {
    ...
    pub async fn get(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: JitRequestDefinition = serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
