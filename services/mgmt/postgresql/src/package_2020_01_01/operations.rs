...
pub mod server_keys {
    ...
    pub async fn list(
        ...
        resource_group_name: &str,
        server_name: &str,
    ) -> std::result::Result<ServerKeyListResult, list::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: ServerKeyListResult = serde_json::from_slice(rsp_body).map_err(|source| list::Error::DeserializeError {
                    source,
                    body: rsp_body.clone(),
                })?;
                Ok(rsp_value)
