...
pub mod migrations {
    ...
    pub async fn get(
        ...
        migration_name: &str,
    ) -> std::result::Result<MigrationResource, get::Error> {
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                ...
                let rsp_value: MigrationResource =
                    serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: ServiceError =
                    serde_json::from_slice(rsp_body).map_err(|source| get::Error::DeserializeError(source, rsp_body.clone()))?;
    ...
    pub mod create {
        ...
        #[derive(Debug)]
        pub enum Response {
            Ok200(MigrationResource),
            Created201(MigrationResource),
        }
        #[derive(Debug, thiserror :: Error)]
        pub enum Error {
            #[error("HTTP status code {}", status_code)]
    ...
    ) -> std::result::Result<MigrationResource, update::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DBforPostgreSQL/flexibleServers/{}/migrations/{}",
            operation_config.base_path(),
    ...
    pub async fn list_by_target_server(
    ...
    ) -> std::result::Result<MigrationResourceListResult, list_by_target_server::Error> {
        let http_client = operation_config.http_client();
        let url_str = &format!(
            "{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DBforPostgreSQL/flexibleServers/{}/migrations",
            operation_config.base_path(),
        ...
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: MigrationResourceListResult = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_by_target_server::Error::DeserializeError(source, rsp_body.clone()))?;
                Ok(rsp_value)
            }
            status_code => {
                let rsp_body = rsp.body();
                let rsp_value: ServiceError = serde_json::from_slice(rsp_body)
                    .map_err(|source| list_by_target_server::Error::DeserializeError(source, rsp_body.clone()))?;
...
pub async fn check_migration_name_availability(
...
) -> std::result::Result<MigrationNameAvailabilityResource, check_migration_name_availability::Error> {
    let http_client = operation_config.http_client();
    let url_str = &format!(
    ...
    match rsp.status() {
        http::StatusCode::OK => {
            let rsp_body = rsp.body();
            let rsp_value: MigrationNameAvailabilityResource = serde_json::from_slice(rsp_body)
