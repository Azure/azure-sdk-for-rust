...
pub mod event_subscriptions {
    ...
    pub async fn get_delivery_attributes(
        ...
        let mut url = url::Url::parse(url_str).context(get_delivery_attributes::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
            let token_response = token_credential
                .get_token(operation_config.token_credential_resource())
                .await
                .context(get_delivery_attributes::GetTokenError)?;
            req_builder = req_builder.header(http::header::AUTHORIZATION, format!("Bearer {}", token_response.token.secret()));
        }
        url.query_pairs_mut().append_pair("api-version", operation_config.api_version());
        let req_body = bytes::Bytes::from_static(azure_core::EMPTY_BODY);
        req_builder = req_builder.header(http::header::CONTENT_LENGTH, 0);
        req_builder = req_builder.uri(url.as_str());
        let req = req_builder.body(req_body).context(get_delivery_attributes::BuildRequestError)?;
        let rsp = http_client
            .execute_request(req)
            .await
            .context(get_delivery_attributes::ExecuteRequestError)?;
        match rsp.status() {
            http::StatusCode::OK => {
                let rsp_body = rsp.body();
                let rsp_value: DeliveryAttributeListResult =
                    serde_json::from_slice(rsp_body).context(get_delivery_attributes::DeserializeError { body: rsp_body.clone() })?;
                Ok(rsp_value)
            }
            status_code => get_delivery_attributes::DefaultResponse { status_code }.fail(),
        }
    }
...
pub mod partner_topic_event_subscriptions {
    ...
    pub async fn get_delivery_attributes(
        ...
        let http_client = operation_config.http_client();
        let url_str = & format ! ("{}/subscriptions/{}/resourceGroups/{}/providers/Microsoft.EventGrid/partnerTopics/{}/eventSubscriptions/{}/getDeliveryAttributes" , operation_config . base_path () , subscription_id , resource_group_name , partner_topic_name , event_subscription_name) ;
        let mut url = url::Url::parse(url_str).context(get_delivery_attributes::ParseUrlError)?;
        let mut req_builder = http::request::Builder::new();
        req_builder = req_builder.method(http::Method::POST);
        if let Some(token_credential) = operation_config.token_credential() {
