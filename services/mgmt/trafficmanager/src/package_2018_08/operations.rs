...
pub mod geographic_hierarchies {
    ...
    impl Client {
        ...
        pub fn get_default(&self) -> get_default::Builder {
            get_default::Builder { client: self.base_clone() }
        }
    }
    pub mod get_default {
        use super::{models, API_VERSION};
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::HeatMapModel, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                        ...
                        &self.client.endpoint, &self.subscription_id, &self.resource_group_name, &self.profile_name, &self.heat_map_type
                    );
                    let mut url = url::Url::parse(url_str).map_err(Error::ParseUrl)?;
                    let mut req_builder = http::request::Builder::new();
                    req_builder = req_builder.method(http::Method::GET);
...
pub mod traffic_manager_user_metrics_keys {
    ...
    pub mod get {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::UserMetricsModel, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::UserMetricsModel =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
