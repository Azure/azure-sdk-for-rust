...
pub enum Error {
    ...
    #[error(transparent)]
    AscOperations_Get(#[from] asc_operations::get::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    StorageTargets_DnsRefresh(#[from] storage_targets::dns_refresh::Error),
    #[error(transparent)]
    Caches_DebugInfo(#[from] caches::debug_info::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    StorageTarget_Flush(#[from] storage_target::flush::Error),
    #[error(transparent)]
    StorageTarget_Suspend(#[from] storage_target::suspend::Error),
    #[error(transparent)]
    StorageTarget_Resume(#[from] storage_target::resume::Error),
    #[error(transparent)]
...
pub mod asc_operations {
    ...
    pub mod get {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::AscOperation, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::AscOperation =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
            ...
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<Response, Error>> {
                Box::pin(async move {
                    ...
                    if let Some(force) = &self.force {
                        url.query_pairs_mut().append_pair("force", force);
                    }
                    let req_body = azure_core::EMPTY_BODY;
                    req_builder = req_builder.uri(url.as_str());
                    let req = req_builder.body(req_body).map_err(Error::BuildRequest)?;
