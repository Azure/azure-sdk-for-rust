...
pub enum Error {
    ...
    #[error(transparent)]
    AscUsages_List(#[from] asc_usages::list::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    StorageTarget_Invalidate(#[from] storage_target::invalidate::Error),
    #[error(transparent)]
...
pub mod asc_usages {
    ...
    pub mod list {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::ResourceUsagesListResult, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::ResourceUsagesListResult =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
