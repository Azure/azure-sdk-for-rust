...
pub mod front_door_name_availability {
    ...
    pub async fn check(
        ...
        let req = req_builder.build().context(check::BuildRequestError)?;
        let rsp = client.execute(req).await.context(check::ExecuteRequestError)?;
        match rsp.status() {
            StatusCode::OK => {
                let body: bytes::Bytes = rsp.bytes().await.context(check::ResponseBytesError)?;
                let rsp_value: CheckNameAvailabilityOutput = serde_json::from_slice(&body).context(check::DeserializeError { body })?;
                Ok(rsp_value)
            }
            status_code => {
                let body: bytes::Bytes = rsp.bytes().await.context(check::ResponseBytesError)?;
                let rsp_value: ErrorResponse = serde_json::from_slice(&body).context(check::DeserializeError { body })?;
