...
pub mod accounts {
    ...
    impl Client {
        ...
        pub fn create(
            ...
            resource_group_name: impl Into<String>,
            account_name: impl Into<String>,
            account: impl Into<models::CognitiveServicesAccount>,
            subscription_id: impl Into<String>,
        ) -> create::Builder {
            create::Builder {
                client: self.0.clone(),
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::CognitiveServicesAccount, Error>> {
                Box::pin(async move {
                    ...
                    url.query_pairs_mut().append_pair("api-version", "2017-04-18");
    ...
    pub mod create {
        ...
        pub struct Builder {
            ...
            pub(crate) resource_group_name: String,
            pub(crate) account_name: String,
            pub(crate) account: models::CognitiveServicesAccount,
            pub(crate) subscription_id: String,
        }
        impl Builder {
...
pub mod check_sku_availability {
    ...
    impl Builder {
        pub fn into_future(
            self,
        ) -> futures::future::BoxFuture<'static, std::result::Result<models::CheckSkuAvailabilityResultList, Error>> {
            Box::pin(async move {
                let url_str = &format!(
                ...
                match rsp_status {
                    http::StatusCode::OK => {
                        let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                        let rsp_value: models::CheckSkuAvailabilityResultList =
                            serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                        Ok(rsp_value)
                    }
                    status_code => {
...
pub mod check_domain_availability {
    ...
    impl Builder {
        pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::CheckDomainAvailabilityResult, Error>> {
            Box::pin(async move {
                let url_str = &format!(
                ...
                match rsp_status {
                    http::StatusCode::OK => {
                        let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                        let rsp_value: models::CheckDomainAvailabilityResult =
                            serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                        Ok(rsp_value)
                    }
                    status_code => {
