...
pub enum Error {
    ...
    #[error(transparent)]
    Accounts_ListModels(#[from] accounts::list_models::Error),
    #[error(transparent)]
...
pub mod accounts {
    ...
    pub mod list_models {
        ...
        impl Builder {
            pub fn into_future(self) -> futures::future::BoxFuture<'static, std::result::Result<models::AccountModelListResult, Error>> {
                Box::pin(async move {
                    let url_str = &format!(
                    ...
                    match rsp_status {
                        http::StatusCode::OK => {
                            let rsp_body = azure_core::collect_pinned_stream(rsp_stream).await.map_err(Error::ResponseBytes)?;
                            let rsp_value: models::AccountModelListResult =
                                serde_json::from_slice(&rsp_body).map_err(|source| Error::Deserialize(source, rsp_body.clone()))?;
                            Ok(rsp_value)
                        }
                        status_code => {
