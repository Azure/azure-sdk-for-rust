...
pub mod premium_messaging_regions {
    ...
    pub mod list {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::PremiumMessagingRegionsListResult> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::PremiumMessagingRegionsListResult = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
        }
        #[derive(Clone)]
        ...
        impl RequestBuilder {
            pub fn into_stream(self) -> azure_core::Pageable<models::PremiumMessagingRegionsListResult, azure_core::error::Error> {
                let make_request = move |continuation: Option<String>| {
                    let this = self.clone();
                    async move {
                        let mut url = azure_core::Url::parse(&format!(
...
pub mod regions {
    ...
    impl Client {
        ...
        pub fn list_by_sku(&self, subscription_id: impl Into<String>, sku: impl Into<String>) -> list_by_sku::RequestBuilder {
